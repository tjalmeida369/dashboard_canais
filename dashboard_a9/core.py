...
def renderizar_html_tabela_dashboard(html: object, height: int = 520) -> None:
    """Renderiza tabelas HTML completas sem transformar <tr>/<td> em texto."""
    if html is None:
        return
    html_texto = str(html)
    if not html_texto.strip():
    ...
    if callable(st_html):
        ...
        components.html(html_texto, height=height, scrolling=True)
...
def _is_truthy_env(valor: str | None) -> bool | None:
...
_parquet_only_env = _is_truthy_env(os.getenv("DASHBOARD_PARQUET_ONLY"))
DASHBOARD_PARQUET_ONLY = (
    _parquet_only_env
    if _parquet_only_env is not None
    else (
        str(DASHBOARD_APP_DIR).replace("\\", "/").startswith("/mount/src/")
        or _is_truthy_env(os.getenv("STREAMLIT_CLOUD")) is True
        or _is_truthy_env(os.getenv("STREAMLIT_SHARING")) is True
    )
)


def _bloquear_fallback_pesado(path_obj: Path) -> bool:
    return bool(DASHBOARD_PARQUET_ONLY and Path(path_obj).suffix.lower() != ".parquet")
...
def resolver_arquivo_preprocessado(nome_arquivo: str | Path, *fallbacks: str | Path) -> Path:
    ...
    if DASHBOARD_PARQUET_ONLY:
        candidatos = [c for c in candidatos if Path(c).suffix.lower() == ".parquet"]
...
def obter_bases_funil_home() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if base_funil_home is None or getattr(base_funil_home, "empty", True):
        try:
            perf_path_ref = Path(globals().get("BASE_PERFORMANCE_FILE_PATH", BASE_PERFORMANCE_FILE_PATH))
            if perf_path_ref.exists():
                base_funil_home = load_base_performance_data(str(perf_path_ref), perf_path_ref.stat().st_mtime)
        except Exception:
            base_funil_home = pd.DataFrame()
    if (
        (base_funil_home is None or getattr(base_funil_home, "empty", True)) and
        not DASHBOARD_PARQUET_ONLY and
        "preparar_base_performance" in globals() and
        "df" in globals()
    ):
        try:
    ...
    if df_cotacoes_home is None or getattr(df_cotacoes_home, "empty", True):
        try:
            ...
            if cotacoes_path_ref:
                cotacoes_path = (
                    Path(cotacoes_path_ref)
                    if DASHBOARD_PARQUET_ONLY
                    else resolver_arquivo_dashboard(cotacoes_path_ref, "RelatorioFluxoVidaCotacao.xlsx")
                )
...
st.markdown(
    """
    <style>
    /* A9 TABLE GRID STANDARD - grade uniforme sem fundo condicional nas variações */
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table,
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .ff-table-box table {
        border-collapse: collapse !important;
        border-spacing: 0 !important;
    }

    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] thead th,
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table thead th,
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .ff-table-box table thead th {
        border-right: 1px solid rgba(255, 255, 255, 0.20) !important;
        border-bottom: 1px solid rgba(61, 7, 4, 0.55) !important;
    }

    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td,
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total) td,
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .ff-table-box table tbody tr:not(.linha-total) td {
    ...
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(odd):not([class*="total"]):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(odd):not([class*="total"]):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(odd):not([class*="total"]):not(.linha-total) td[class*="neutro"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(odd):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(odd):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(odd):not(.linha-total) td[class*="neutro"] {
        background: #FFFFFF !important;
    ...
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(even):not([class*="total"]):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(even):not([class*="total"]):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(even):not([class*="total"]):not(.linha-total) td[class*="neutro"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(even):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(even):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(even):not(.linha-total) td[class*="neutro"] {
    ...
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total):hover td[class*="positivo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total):hover td[class*="negativo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total):hover td[class*="neutro"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total):hover td[class*="positivo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total):hover td[class*="negativo"],
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total):hover td[class*="neutro"] {
    ...
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr[class*="total"] td,
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr.linha-total td {
        border-right: 1px solid rgba(255, 255, 255, 0.16) !important;
        border-bottom: 1px solid rgba(255, 255, 255, 0.16) !important;
    }

    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] .mom-pill,
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] .mom-chip-funil,
    html body :is(#a9-table-grid-standard, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] .mom-chip-migracoes {
        border: 0 !important;
        box-shadow: none !important;
        outline: none !important;
    }
...
st.markdown(
    """
    <style>
    /* A9 CLEAN VARIATIONS - sem fundo condicional, mantendo setas e cores */
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total):hover td[class*="positivo"],
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td.status-superavit,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total) td[class*="positivo"] {
    ...
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total):hover td[class*="negativo"],
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td.status-gap,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total) td[class*="negativo"] {
        color: #B42318 !important;
    ...
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td[class*="neutro"],
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total):hover td[class*="neutro"],
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total) td[class*="neutro"] {
    ...
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td[class*="positivo"]::before,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td.status-superavit::before {
        color: #087443 !important;
        -webkit-text-fill-color: #087443 !important;
    }

    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td[class*="negativo"]::before,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td.status-gap::before {
        color: #B42318 !important;
        -webkit-text-fill-color: #B42318 !important;
    }

    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] .mom-pill,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] .mom-chip-funil,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] .mom-chip-migracoes,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .ff-table-box table tbody td.ff-mom-cell .mom-pill {
        background: transparent !important;
        background-color: transparent !important;
        background-image: none !important;
    ...
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] .mom-up,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .ff-table-box table .mom-up {
        color: #087443 !important;
        -webkit-text-fill-color: #087443 !important;
    }

    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] .mom-down,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .ff-table-box table .mom-down {
        color: #B42318 !important;
        -webkit-text-fill-color: #B42318 !important;
    }

    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody td,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody td,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .ff-table-box table tbody td {
        font-size: clamp(10.2px, 0.76vw, 12.0px) !important;
        line-height: 1.16 !important;
    }

    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] thead th,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .conv-table thead th,
    html body :is(#a9-clean-variation-override, div[id], div[class*="tabela"], div[class*="table"]) .ff-table-box table thead th {
        font-size: clamp(9.4px, 0.70vw, 11.0px) !important;
...
st.markdown(
    """
    <style>
    /* VARIATION COLORS ABSOLUTE FINAL - somente tabelas */
    body table[class*="tabela"] tbody tr:not([class*="total"]) td[class*="positivo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):nth-child(even) td[class*="positivo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):nth-child(odd) td[class*="positivo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):hover td[class*="positivo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-superavit,
    body table[class*="tabela"] tbody tr:not([class*="total"]):hover td.status-superavit,
    body .conv-table tbody tr:not(.linha-total) td[class*="positivo"],
    body .conv-table tbody tr:not(.linha-total):hover td[class*="positivo"] {
        color: #087443 !important;
        -webkit-text-fill-color: #087443 !important;
        background: transparent !important;
        background-image: none !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td[class*="negativo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):nth-child(even) td[class*="negativo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):nth-child(odd) td[class*="negativo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):hover td[class*="negativo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-gap,
    body table[class*="tabela"] tbody tr:not([class*="total"]):hover td.status-gap,
    body .conv-table tbody tr:not(.linha-total) td[class*="negativo"],
    body .conv-table tbody tr:not(.linha-total):hover td[class*="negativo"] {
        color: #B42318 !important;
        -webkit-text-fill-color: #B42318 !important;
        background: transparent !important;
        background-image: none !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td[class*="neutro"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):nth-child(even) td[class*="neutro"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):nth-child(odd) td[class*="neutro"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):hover td[class*="neutro"],
    body .conv-table tbody tr:not(.linha-total) td[class*="neutro"],
    body .conv-table tbody tr:not(.linha-total):hover td[class*="neutro"] {
        color: #475467 !important;
        -webkit-text-fill-color: #475467 !important;
        background: transparent !important;
        background-image: none !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td[class*="positivo"]::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-superavit::before {
        content: "▲" !important;
        position: absolute !important;
        left: 5px !important;
        top: 50% !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td[class*="negativo"]::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-gap::before {
        content: "▼" !important;
        position: absolute !important;
        left: 5px !important;
        top: 50% !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td[class*="neutro"]::before {
        content: "" !important;
        color: #475467 !important;
        -webkit-text-fill-color: #475467 !important;
    }

    body .ff-table-box table tbody td.ff-mom-cell .mom-pill.mom-up,
    body .ff-table-box table tbody tr:hover td.ff-mom-cell .mom-pill.mom-up,
    body table[class*="tabela"] .mom-chip-funil.mom-up,
    body table[class*="tabela"] .mom-chip-migracoes.mom-up {
        color: #087443 !important;
        -webkit-text-fill-color: #087443 !important;
    ...
    body .ff-table-box table tbody td.ff-mom-cell .mom-pill.mom-down,
    body .ff-table-box table tbody tr:hover td.ff-mom-cell .mom-pill.mom-down,
    body table[class*="tabela"] .mom-chip-funil.mom-down,
    body table[class*="tabela"] .mom-chip-migracoes.mom-down {
        color: #B42318 !important;
        -webkit-text-fill-color: #B42318 !important;
    ...
    body .ff-table-box table tbody td.ff-mom-cell .mom-pill.mom-flat,
    body .ff-table-box table tbody tr:hover td.ff-mom-cell .mom-pill.mom-flat,
    body table[class*="tabela"] .mom-chip-funil.mom-flat,
    body table[class*="tabela"] .mom-chip-migracoes.mom-flat {
        color: #475467 !important;
        -webkit-text-fill-color: #475467 !important;
    ...
    body table[class*="tabela"] tbody tr[class*="total"] td,
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="positivo"],
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="negativo"],
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="neutro"],
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="status"],
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="percentual"],
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="variacao"],
    body table[class*="tabela"] tbody tr[class*="total"]:hover td,
    body .conv-table tbody tr.linha-total td,
    body .conv-table tbody tr.linha-total:hover td {
        background: linear-gradient(180deg, #68110C 0%, #420604 100%) !important;
        color: #FFFFFF !important;
        -webkit-text-fill-color: #FFFFFF !important;
    ...
    body table[class*="tabela"] tbody tr[class*="total"] td *,
    body table[class*="tabela"] tbody tr[class*="total"] td::before,
    body table[class*="tabela"] tbody tr[class*="total"] td::after,
    body .conv-table tbody tr.linha-total td *,
    body .conv-table tbody tr.linha-total td::before,
    body .conv-table tbody tr.linha-total td::after {
        color: #FFFFFF !important;
        -webkit-text-fill-color: #FFFFFF !important;
    }
    </style>
...
st.markdown(
    """
    <style>
    /* A9 TABLE GRID STANDARD - override final */
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td,
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total) td,
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .ff-table-box table tbody tr:not(.linha-total) td {
        border-right: 1px solid #EEF0F3 !important;
        border-bottom: 1px solid #EEF0F3 !important;
        border-left: 0 !important;
        border-top: 0 !important;
    ...
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(odd):not([class*="total"]):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(odd):not([class*="total"]):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(odd):not([class*="total"]):not(.linha-total) td[class*="neutro"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(odd):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(odd):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(odd):not(.linha-total) td[class*="neutro"] {
        background: #FFFFFF !important;
        background-color: #FFFFFF !important;
        background-image: none !important;
        border-color: #EEF0F3 !important;
    ...
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(even):not([class*="total"]):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(even):not([class*="total"]):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:nth-child(even):not([class*="total"]):not(.linha-total) td[class*="neutro"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(even):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(even):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:nth-child(even):not(.linha-total) td[class*="neutro"] {
        background: #FAFBFC !important;
        background-color: #FAFBFC !important;
        background-image: none !important;
        border-color: #EEF0F3 !important;
    ...
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total):hover td[class*="positivo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total):hover td[class*="negativo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total):hover td[class*="neutro"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total):hover td[class*="positivo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total):hover td[class*="negativo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total):hover td[class*="neutro"] {
        background: #FFF8F7 !important;
        background-color: #FFF8F7 !important;
        background-image: none !important;
        border-color: #EEF0F3 !important;
    ...
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td[class*="positivo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total) td[class*="positivo"] {
        color: #087443 !important;
        -webkit-text-fill-color: #087443 !important;
        font-weight: 500 !important;
    }

    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr:not([class*="total"]):not(.linha-total) td[class*="negativo"],
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr:not(.linha-total) td[class*="negativo"] {
        color: #B42318 !important;
        -webkit-text-fill-color: #B42318 !important;
        font-weight: 500 !important;
    }

    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) table[class*="tabela"] tbody tr[class*="total"] td,
    html body :is(#a9-table-grid-final, div[id], div[class*="tabela"], div[class*="table"]) .conv-table tbody tr.linha-total td {
        border-right: 1px solid rgba(255, 255, 255, 0.16) !important;
        border-bottom: 1px solid rgba(255, 255, 255, 0.16) !important;
    }
    </style>
...
st.markdown(
    """
    <style>
    /* CONDITIONAL TABLE COLORS FINAL - devolve destaque de MoM/YoY/% sem afetar KPI */
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-positivo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-positivo-pedidos,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-positivo-desativados,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.variacao-positiva,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-positivo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.pct-positivo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-var.status-positivo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao.status-positivo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao.percentual-positivo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao-pedidos.percentual-positivo-pedidos,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao-desativados.percentual-positivo-desativados,
    body .conv-table tbody tr:not(.linha-total) td.percentual-positivo,
    body .conv-table tbody tr:not(.linha-total) td.status-positivo,
    body .conv-table tbody tr:not(.linha-total) td.pct-positivo {
        color: #087443 !important;
        -webkit-text-fill-color: #087443 !important;
        background: #ECFDF3 !important;
        border-color: #ABEFC6 !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-negativo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-negativo-pedidos,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-negativo-desativados,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.variacao-negativa,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-negativo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.pct-negativo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-var.status-negativo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao.status-negativo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao.percentual-negativo,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao-pedidos.percentual-negativo-pedidos,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao-desativados.percentual-negativo-desativados,
    body .conv-table tbody tr:not(.linha-total) td.percentual-negativo,
    body .conv-table tbody tr:not(.linha-total) td.status-negativo,
    body .conv-table tbody tr:not(.linha-total) td.pct-negativo {
        color: #B42318 !important;
        -webkit-text-fill-color: #B42318 !important;
        background: #FEF3F2 !important;
        border-color: #FECDCA !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-neutro,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.variacao-neutra,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-neutro,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.pct-neutro,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-var.status-neutro,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao.status-neutro,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-variacao.percentual-neutro,
    body .conv-table tbody tr:not(.linha-total) td.percentual-neutro,
    body .conv-table tbody tr:not(.linha-total) td.status-neutro,
    body .conv-table tbody tr:not(.linha-total) td.pct-neutro {
        color: #475467 !important;
        -webkit-text-fill-color: #475467 !important;
        background: #F2F4F7 !important;
        border-color: #D0D5DD !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-superavit,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-var.status-superavit {
        color: #087443 !important;
        -webkit-text-fill-color: #087443 !important;
        background: #ECFDF3 !important;
        border-color: #ABEFC6 !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-gap,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.col-var.status-gap {
        color: #B42318 !important;
        -webkit-text-fill-color: #B42318 !important;
        background: #FEF3F2 !important;
        border-color: #FECDCA !important;
    ...
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-positivo::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-positivo-pedidos::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-positivo-desativados::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.variacao-positiva::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-positivo::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.pct-positivo::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-superavit::before {
        color: #087443 !important;
        -webkit-text-fill-color: #087443 !important;
    }

    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-negativo::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-negativo-pedidos::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.percentual-negativo-desativados::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.variacao-negativa::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-negativo::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.pct-negativo::before,
    body table[class*="tabela"] tbody tr:not([class*="total"]) td.status-gap::before {
        color: #B42318 !important;
        -webkit-text-fill-color: #B42318 !important;
    }
    ...
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="variacao"],
    body .conv-table tbody tr.linha-total td,
    body .conv-table tbody tr.linha-total td[class*="status"],
    body .conv-table tbody tr.linha-total td[class*="percentual"] {
        background: linear-gradient(180deg, #68110C 0%, #420604 100%) !important;
        color: #FFFFFF !important;
        -webkit-text-fill-color: #FFFFFF !important;
        border-color: rgba(255,255,255,0.16) !important;
...
st.markdown(
    """
    <style>
    /* FINAL VISUAL REFINEMENT - total rows readable */

    body [class^="tabela-container-"],
    body [class*=" tabela-container-"],
    body .conv-table-wrap {
        border-radius: 12px !important;
        border: 1px solid rgba(121, 14, 9, 0.13) !important;
        box-shadow: 0 12px 26px rgba(16, 24, 40, 0.075) !important;
        background: #FFFFFF !important;
    }

    body table[class*="tabela"],
    body .conv-table {
        border-collapse: separate !important;
        border-spacing: 0 !important;
        font-family: "Manrope", "Segoe UI", sans-serif !important;
        color: #241211 !important;
    }

    body table[class*="tabela"] thead th,
    body .conv-table thead th {
        background: linear-gradient(180deg, #7F100B 0%, #5F0A07 100%) !important;
        color: #FFFFFF !important;
        -webkit-text-fill-color: #FFFFFF !important;
    ...
    body table[class*="tabela"] tbody td,
    body .conv-table tbody td {
        border-bottom: 1px solid rgba(121, 14, 9, 0.07) !important;
        color: #2B1715 !important;
        -webkit-text-fill-color: #2B1715 !important;
    }

    body table[class*="tabela"] tbody tr:not([class*="total"]):nth-child(even) td,
    body .conv-table tbody tr:not(.linha-total):nth-child(even) td {
        background: #FFFDFC !important;
    }

    body table[class*="tabela"] tbody tr:not([class*="total"]):hover td,
    body .conv-table tbody tr:not(.linha-total):hover td {
    ...
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="percentual"],
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="variacao"],
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="col-var"],
    body table[class*="tabela"] tbody tr[class*="total"] td[class*="pct"],
    body .conv-table tbody tr.linha-total td,
    body .conv-table tbody tr.linha-total td[class*="status"],
    body .conv-table tbody tr.linha-total td[class*="percentual"],
    body .conv-table tbody tr.linha-total td[class*="variacao"],
    body .conv-table tbody tr.linha-total td[class*="pct"] {
        background: linear-gradient(180deg, #68110C 0%, #3F0604 100%) !important;
        color: #FFFFFF !important;
        -webkit-text-fill-color: #FFFFFF !important;
        border-color: rgba(255,255,255,0.16) !important;
        box-shadow: none !important;
    ...
    body table[class*="tabela"] tbody tr[class*="total"] td *,
    body table[class*="tabela"] tbody tr[class*="total"] td span,
    body table[class*="tabela"] tbody tr[class*="total"] td strong,
    body .conv-table tbody tr.linha-total td *,
    body .conv-table tbody tr.linha-total td span,
    body .conv-table tbody tr.linha-total td strong {
        color: #FFFFFF !important;
        -webkit-text-fill-color: #FFFFFF !important;
        text-shadow: none !important;
    }
...
def load_data(path: str, file_mtime: float | None = None) -> pd.DataFrame:
    ...
    try:
        ...
        elif DASHBOARD_PARQUET_ONLY:
            raise RuntimeError("Modo Parquet-only ativo: gere `dados_preprocessados/base_principal.parquet` com preprocess_all.py.")
        else:
...
def load_excel_cached(
    ...
    _ = file_mtime
    if DASHBOARD_PARQUET_ONLY:
        raise RuntimeError(f"Modo Parquet-only ativo: leitura Excel bloqueada para {Path(path).name}.")
...
def load_tabular_cached(
    ...
    if DASHBOARD_PARQUET_ONLY:
        raise RuntimeError(f"Modo Parquet-only ativo: leitura tabular bruta bloqueada para {path_obj.name}.")
...
NECESSIDADE_DIARIA_FILE_PATH = resolver_arquivo_preprocessado("necessidade_diaria.parquet", ANALITICA_DIARIA_FILE_PATH)
...
def montar_tabela_convergencia_dimensao_html(
    ...
    largura_primeira_coluna = "136px" if coluna_dimensao == 'CANAL_PLAN' else "82px"
    colgroup = f'<colgroup><col style="width:{largura_primeira_coluna};">' + ''.join(
...
def criar_grafico_evolucao_convergencia(df_ref: pd.DataFrame, meses_ref: list[str]) -> go.Figure:
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    if df_ref is None or df_ref.empty or not meses_ref:
        fig.update_layout(
            height=380,
    ...
    df_chart = df_ref[df_ref['mes_ano'].astype(str).str.strip().str.lower().isin(meses_ref)].copy()
    if df_chart.empty:
        return criar_grafico_evolucao_convergencia(pd.DataFrame(), meses_ref)

    df_agg = agregar_convergencia_metricas(df_chart, ['mes_ano'])
    serie = pd.DataFrame({'mes_ano': [str(m).strip().lower() for m in meses_ref]})
    serie = serie.merge(df_agg, on='mes_ano', how='left')
    for coluna in ['TOTAL', 'CONV']:
        serie[coluna] = pd.to_numeric(serie.get(coluna, 0), errors='coerce').fillna(0.0)
    serie['PCT_CONV'] = np.where(
        serie['TOTAL'].gt(0),
        serie['CONV'] / serie['TOTAL'] * 100.0,
        0.0
    )
    serie['ROTULO'] = serie['mes_ano'].astype(str).str.replace('/', '-', regex=False).str.upper()

    texto_total = [
        formatar_numero_brasileiro(valor, 0) if float(valor or 0) > 0 else ""
        for valor in serie['TOTAL']
    ]
    texto_conv = [
        f"{formatar_numero_brasileiro(conv, 0)}<br>{fmt_pct_convergencia(pct)}"
        if float(conv or 0) > 0 else ""
        for conv, pct in zip(serie['CONV'], serie['PCT_CONV'])
    ]
    custom_total = np.stack([serie['CONV'], serie['PCT_CONV']], axis=-1)
    custom_conv = np.stack([serie['TOTAL'], serie['PCT_CONV']], axis=-1)
    ...
    fig.add_trace(
        go.Bar(
            ...
            marker=dict(
                ...
                line=dict(color="rgba(31, 41, 55, 0.20)", width=0.7),
                cornerradius=8
            ),
            textfont=dict(size=11, color="#4B5563", family="Manrope, Segoe UI, sans-serif"),
            width=0.58,
            customdata=custom_total,
            hovertemplate=(
                "<b>%{x}</b><br>"
                "Clientes totais: %{y:,.0f}<br>"
                "Convergentes: %{customdata[0]:,.0f}<br>"
                "% Convergência: %{customdata[1]:.1f}%"
                "<extra></extra>"
            ),
        ),
        secondary_y=False
    ...
    fig.add_trace(
        go.Scatter(
            ...
            textfont=dict(size=11, color="#790E09", family="Manrope, Segoe UI, sans-serif"),
            line=dict(color="#790E09", width=3.4, shape="spline"),
            marker=dict(
                size=8,
                color="#790E09",
            ...
            hovertemplate=(
                "<b>%{x}</b><br>"
                "Clientes convergentes: %{y:,.0f}<br>"
                "Clientes totais: %{customdata[0]:,.0f}<br>"
                "% Convergência: %{customdata[1]:.1f}%"
                "<extra></extra>"
            ),
        ),
    ...
    max_total = float(serie['TOTAL'].max() or 0)
    max_conv = float(serie['CONV'].max() or 0)
    fig.update_yaxes(
        title_text=None,
        secondary_y=False,
        range=[0, max_total * 1.22 if max_total > 0 else 1],
        showgrid=False,
        zeroline=False,
    ...
    fig.update_yaxes(
        title_text=None,
        secondary_y=True,
        range=[0, max_conv * 1.42 if max_conv > 0 else 1],
        showgrid=False,
        zeroline=False,
        showticklabels=False,
        ticks="",
    ...
    fig.update_xaxes(
        type="category",
        categoryorder="array",
        categoryarray=serie['ROTULO'].tolist(),
        tickfont=dict(size=11, color="#4B5563", family="Manrope, Segoe UI, sans-serif"),
        showgrid=False,
        zeroline=False,
        showline=False,
        ticks=""
    ...
    fig.update_layout(
        ...
        paper_bgcolor="#FFFFFF",
        plot_bgcolor="#FFFFFF",
        margin=dict(l=16, r=18, t=34, b=26),
        hovermode="x unified",
        bargap=0.30,
        legend=dict(
            orientation="h",
            ...
            font=dict(size=12, color="#374151", family="Manrope, Segoe UI, sans-serif")
        ),
        font=dict(family="Manrope, Segoe UI, sans-serif", color="#1F2937"),
        hoverlabel=dict(
            bgcolor="#FFFFFF",
            bordercolor="rgba(121,14,9,0.18)",
            font_size=12,
...
def _read_excel_with_copy_fallback(
    ...
    *,
    ...
    path_obj = Path(path)
    if DASHBOARD_PARQUET_ONLY:
        raise RuntimeError(f"Modo Parquet-only ativo: leitura Excel bloqueada para {path_obj.name}.")
    try:
        return pd.read_excel(path_obj, usecols=usecols, nrows=nrows)
...
def load_cotacoes_data(
    ...
    if _bloquear_fallback_pesado(path_obj):
        return pd.DataFrame()
...
def montar_tabela_backlog_canais(
    ...
    for col in df_fmt.columns:
        if col == "CANAL":
            continue
        if col in {"MoM", "YoY", "YTD26 vs YTD25"}:
...
def criar_tabela_html_funil_cotacoes(
    ...
    return criar_tabela_html_resumo_mensal_canal(df_formatado, df_numerico, table_id)
...
def criar_tabela_html_resumo_mensal_canal(
    ...
    """Tabela limpa para o resumo mensal, no mesmo conceito visual do resumo semanal."""
    if df_formatado is None or df_formatado.empty:
        return ""

    colunas = list(df_formatado.columns)
    col_canal = colunas[0] if colunas else "CANAL"
    col_indicador = colunas[1] if len(colunas) > 1 and str(colunas[1]).strip().upper() in {"INDICADOR", "INDICE", "ÍNDICE"} else None
    qtd_colunas_texto = 2 if col_indicador else 1
    colunas_variacao = {"MoM", "YoY", "TEND vs ORC", "YTD26 vs YTD25", "YTD26 vs YTD_ORÇ"}
    colunas_ytd = {"YTD25", "YTD26"}
    colunas_meta = {"ORC", "YTD_ORÇ"}
    linhas_grupo = {"DEMANDA", "VENDA BRUTA", "ATIVADOS", "INSTALADOS"}
    largura_canal_pct = 9.2 if col_indicador else 10.3
    largura_indicador_pct = 7.6 if col_indicador else 0.0
    colunas_texto = {col_canal}
    if col_indicador:
        colunas_texto.add(col_indicador)
    colunas_ytd_meta = colunas_ytd | colunas_meta
    qtd_variacoes = sum(1 for coluna in colunas if coluna in colunas_variacao)
    qtd_ytd_meta = sum(1 for coluna in colunas if coluna in colunas_ytd_meta)
    qtd_meses = max(len(colunas) - len(colunas_texto) - qtd_variacoes - qtd_ytd_meta, 1)
    largura_variacao_pct = 5.05
    largura_ytd_meta_pct = 4.05
    largura_base_texto_pct = largura_canal_pct + largura_indicador_pct
    largura_meses_total = 100.0 - largura_base_texto_pct - (qtd_variacoes * largura_variacao_pct) - (qtd_ytd_meta * largura_ytd_meta_pct)
    if largura_meses_total < qtd_meses * 3.25:
        largura_variacao_pct = 4.75
        largura_ytd_meta_pct = 3.85
        largura_meses_total = 100.0 - largura_base_texto_pct - (qtd_variacoes * largura_variacao_pct) - (qtd_ytd_meta * largura_ytd_meta_pct)
    largura_mes_pct = max(largura_meses_total / qtd_meses, 3.05)
    larguras_colunas = []
    for coluna in colunas:
        if coluna == col_canal:
            larguras_colunas.append(largura_canal_pct)
        elif col_indicador and coluna == col_indicador:
            larguras_colunas.append(largura_indicador_pct)
        elif coluna in colunas_variacao:
            larguras_colunas.append(largura_variacao_pct)
        elif coluna in colunas_ytd_meta:
            larguras_colunas.append(largura_ytd_meta_pct)
        else:
            larguras_colunas.append(largura_mes_pct)
    colgroup_html = "<colgroup>" + "".join(
        [f'<col style="width:{largura:.4f}%;">' for largura in larguras_colunas]
    ) + "</colgroup>"
    ...
    def _cabecalho(coluna: str) -> str:
        ...
        if coluna_str == "TEND vs ORC":
            return "TEND<br>vs<br>ORC"
    ...
    <style>
        ...
        table.{table_id} {{
            ...
            min-width: 100%;
            table-layout: fixed;
            font-size: clamp(8.8px, 0.66vw, 10.3px);
            font-family: 'Manrope', 'Segoe UI', sans-serif;
            font-variant-numeric: tabular-nums;
        }}
        .{table_id} thead th {{
            ...
            overflow-wrap: anywhere;
            line-height: 1.0;
            font-size: clamp(7.8px, 0.60vw, 9.4px);
            text-transform: uppercase;
        }}
        .{table_id} thead th.col-canal {{
            position: sticky;
        ...
        .{table_id} thead th.col-indicador {{
            text-align: left;
            padding-left: 6px;
            background: linear-gradient(135deg, #6C0C08 0%, #4A0704 100%) !important;
        }}
        ...
        .{table_id} tbody td {{
            ...
            line-height: 1.08;
            letter-spacing: -0.02em;
            font-size: clamp(9.1px, 0.70vw, 10.8px);
        }}
        ...
        .{table_id} tbody td.col-indicador {{
            text-align: left;
            padding-left: 5px;
            font-weight: 600;
            color: #4A5568;
        ...
        .{table_id} tbody tr:nth-child(odd) td.col-indicador {{ background: #FFF9F8 !important; }}
        .{table_id} tbody tr:nth-child(even) td.col-indicador {{ background: #FDF3F2 !important; }}
        ...
        .{table_id} tbody tr:nth-child(odd) td.col-variacao {{ background: #FFF9F8 !important; }}
        .{table_id} tbody tr:nth-child(even) td.col-variacao {{ background: #FDF3F2 !important; }}
        ...
        .{table_id} tbody tr:not(.linha-total):not(.linha-grupo-resumo) td.col-canal {{
            padding-left: 14px;
            font-weight: 500;
        }}
        .{table_id} tbody tr.linha-total td.col-canal,
        .{table_id} tbody tr.linha-total td.col-indicador,
        ...
        .{table_id} tbody tr.linha-grupo-resumo td::before {{
            content: "" !important;
        }}
    </style>
    <div class="{table_id}-container">
    ...
    for idx_col, coluna in enumerate(colunas):
        ...
        if idx_col == 0:
            classes.append("col-canal")
        elif col_indicador and idx_col == 1:
            classes.append("col-indicador")
    ...
    for idx_linha, row in df_formatado.iterrows():
        ...
        canal_ref_upper = canal_ref.strip().upper()
        is_grupo = canal_ref_upper in linhas_grupo
        is_total = canal_ref_upper == "TOTAL" or is_grupo
        if is_grupo:
            grupo_classe = "grupo-demanda"
            if canal_ref_upper == "VENDA BRUTA":
                grupo_classe = "grupo-vb"
            elif canal_ref_upper in {"ATIVADOS", "INSTALADOS"}:
                grupo_classe = "grupo-ativ"
            html += f'<tr class="linha-total linha-grupo-resumo {grupo_classe}">'
        else:
...
def load_migracoes_pme_data(path: str, file_mtime: float | None = None) -> pd.DataFrame:
    ...
    _ = file_mtime
    path_obj = Path(path)
    colunas_saida = ["REGIONAL", "CANAL_PLAN", "MES_ANO", "DSC_PLANO_ATU", "QTDE", "VAL_PRODUTO_FINAL"]
    if not path_obj.exists():
        return pd.DataFrame(columns=colunas_saida)
    ...
    if path_obj.suffix.lower() == ".parquet":
        df_opt = _carregar_dataframe_preprocessado(
            str(path_obj),
            file_mtime,
            required_cols={"REGIONAL", "MES_ANO", "QTDE"},
            text_cols=["REGIONAL", "CANAL_PLAN", "MES_ANO", "DSC_PLANO_ATU"],
            numeric_cols=["QTDE", "VAL_PRODUTO_FINAL"],
            category_cols=["REGIONAL", "CANAL_PLAN", "MES_ANO", "DSC_PLANO_ATU"],
            default_values={"CANAL_PLAN": "Todos", "DSC_PLANO_ATU": "SEM PLANO", "VAL_PRODUTO_FINAL": 0.0}
        )
        if df_opt.empty:
            return pd.DataFrame(columns=colunas_saida)
        df_opt["REGIONAL"] = df_opt["REGIONAL"].astype(str).str.strip().str.upper().str[:3]
        if "CANAL_PLAN" not in df_opt.columns:
            df_opt["CANAL_PLAN"] = "Todos"
        df_opt["CANAL_PLAN"] = (
            df_opt["CANAL_PLAN"]
            .astype("string")
            .str.strip()
            .fillna("Não informado")
            .replace({"": "Não informado", "nan": "Não informado", "NaN": "Não informado"})
            .apply(normalizar_canal_plan)
        )
        df_opt["DSC_PLANO_ATU"] = (
            df_opt["DSC_PLANO_ATU"]
            .astype("string")
            .str.strip()
            .fillna("SEM PLANO")
            .replace({"": "SEM PLANO", "nan": "SEM PLANO", "NaN": "SEM PLANO"})
        )
    ...
    coluna_plano = encontrar_coluna_por_alias(
        header_df.columns,
        "DSC_PLANO_ATU",
        "DSC PLANO ATU",
        "PLANO",
    ...
    coluna_valor = encontrar_coluna_por_alias(
        header_df.columns,
        "VAL_PRODUTO_FINAL",
        "VAL PRODUTO FINAL",
        "VALOR_PRODUTO_FINAL",
    ...
    colunas_leitura = [coluna_data, coluna_regional, coluna_qtde]
    if coluna_canal:
        colunas_leitura.append(coluna_canal)
    if coluna_plano:
        colunas_leitura.append(coluna_plano)
    if coluna_valor:
        colunas_leitura.append(coluna_valor)
    ...
    df = df.rename(
        columns={
            ...
            coluna_regional: "REGIONAL",
            coluna_qtde: "QTDE",
            **({coluna_canal: "CANAL_PLAN"} if coluna_canal else {}),
            **({coluna_plano: "DSC_PLANO_ATU"} if coluna_plano else {}),
            **({coluna_valor: "VAL_PRODUTO_FINAL"} if coluna_valor else {}),
        }
    ).copy()
    ...
    df["QTDE"] = normalizar_numerico_serie(df["QTDE"]).fillna(0.0)
    if "CANAL_PLAN" not in df.columns:
        df["CANAL_PLAN"] = "Não informado"
    df["CANAL_PLAN"] = (
        df["CANAL_PLAN"]
        .astype("string")
        .str.strip()
    ...
    if "DSC_PLANO_ATU" not in df.columns:
        df["DSC_PLANO_ATU"] = "SEM PLANO"
    df["DSC_PLANO_ATU"] = (
        df["DSC_PLANO_ATU"]
        .astype("string")
        .str.strip()
    ...
    if "VAL_PRODUTO_FINAL" not in df.columns:
        df["VAL_PRODUTO_FINAL"] = 0.0
    df["VAL_PRODUTO_FINAL"] = normalizar_numerico_serie(df["VAL_PRODUTO_FINAL"]).fillna(0.0)
    ...
    df = (
        df.groupby(["REGIONAL", "CANAL_PLAN", "MES_ANO", "DSC_PLANO_ATU"], as_index=False, observed=True, dropna=False)[["QTDE", "VAL_PRODUTO_FINAL"]]
        .sum()
    )
    compactar_colunas_categoricas(df, ["REGIONAL", "CANAL_PLAN", "MES_ANO", "DSC_PLANO_ATU"])
...
def _mes_atual_migracoes() -> str:
    """Mes de referencia para tendencia de migracoes: nunca projeta mes futuro."""
    try:
        return str(get_mes_atual_formatado()).strip().lower()
    except Exception:
        return ""

def _filtrar_meses_migracoes_ate_atual(meses_ref: list[str]) -> list[str]:
    if not meses_ref:
        return []
    meses_ordenados = sorted(
        [str(mes).strip().lower() for mes in meses_ref if str(mes).strip()],
        key=mes_ano_para_data
    )
    mes_atual = _mes_atual_migracoes()
    if not mes_atual:
    ...
    try:
        ...
        meses_filtrados = [
            ...
            if pd.Timestamp(mes_ano_para_data(str(mes))).normalize() <= data_atual
        ]
        return meses_filtrados or meses_ordenados
    except Exception:
        return meses_ordenados

def _resolver_mes_tendencia_migracoes(meses_reais: list[str]) -> str:
    if not meses_reais:
        return ""
    mes_atual = _mes_atual_migracoes()
    if not mes_atual:
        return str(meses_reais[-1]).strip().lower()
    try:
        data_atual = pd.Timestamp(mes_ano_para_data(mes_atual)).normalize()
        data_ultimo_real = pd.Timestamp(mes_ano_para_data(str(meses_reais[-1]))).normalize()
        return mes_atual if data_atual >= data_ultimo_real else str(meses_reais[-1]).strip().lower()
    except Exception:
        return str(meses_reais[-1]).strip().lower()
...
def _resolver_coluna_mes_migracoes(colunas_valor: list[str], mes_ref: str | None) -> str:
    ...
    if not colunas_valor:
        return ""
    mes_ref_raw = str(mes_ref or "").strip()
    for coluna in colunas_valor:
        if str(coluna).strip().lower() == mes_ref_raw.lower():
            return str(coluna)
    mes_ref_norm = mes_ref_raw.replace("TEND.", "").strip().lower()
    for coluna in colunas_valor:
        coluna_norm = str(coluna).replace("TEND.", "").strip().lower()
...
def obter_meses_mom_migracoes(df_migracoes: pd.DataFrame) -> tuple[list[str], str]:
    """Retorna meses reais + mes atual de tendencia para o filtro de MoM."""
    if df_migracoes is None or df_migracoes.empty or "MES_ANO" not in df_migracoes.columns:
        return [], ""

    meses_reais = _filtrar_meses_migracoes_ate_atual(
        df_migracoes["MES_ANO"].dropna().astype(str).unique().tolist()
    )
    if not meses_reais:
        return [], ""

    mes_tendencia = _resolver_mes_tendencia_migracoes(meses_reais)
...
def montar_tabela_migracoes_pme_regionais(
    ...
    dimensao_col: str = "REGIONAL",
    rotulo_coluna: str = "REGIONAL"
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Monta a tabela regional x mes para migracoes PME somando QTDE."""
    colunas_vazias = [rotulo_coluna]
    if df_migracoes is None or df_migracoes.empty or dimensao_col not in df_migracoes.columns:
        return pd.DataFrame(columns=colunas_vazias), pd.DataFrame(columns=colunas_vazias)
    ...
    df_work = df_migracoes.copy()
    df_work[dimensao_col] = (
        df_work[dimensao_col]
        .astype("string")
        .str.strip()
        .fillna("Não informado")
    ...
    if tabela.empty:
        return pd.DataFrame(columns=[rotulo_coluna, *meses_ordem]), pd.DataFrame(columns=[rotulo_coluna, *meses_ordem])

    mes_tendencia = _resolver_mes_tendencia_migracoes(meses_ordem)
    coluna_tendencia = f"TEND. {mes_tendencia}" if mes_tendencia else "TEND."
    try:
        data_ref_tend = pd.Timestamp(mes_ano_para_data(mes_tendencia)).normalize()
        meses_hist_tend = [
            mes for mes in meses_ordem
            if pd.Timestamp(mes_ano_para_data(str(mes))).normalize() < data_ref_tend
        ]
        if not meses_hist_tend:
            meses_hist_tend = meses_ordem
    except Exception:
        meses_hist_tend = meses_ordem

    tabela[coluna_tendencia] = [
        _prever_tendencia_mensal_migracoes(
            pd.to_numeric(tabela.loc[regional, meses_hist_tend], errors="coerce").fillna(0.0)
        )
    ...
    totais_mes = (
        df_work.groupby("MES_ANO", observed=True)["QTDE"]
        .sum()
        .reindex(meses_ordem, fill_value=0)
    )

    colunas_saida_numericas = [*colunas_exibicao_valor, "MoM"]
    df_num = tabela[colunas_saida_numericas].reset_index().rename(columns={dimensao_col: rotulo_coluna})
    linha_total = {rotulo_coluna: "TOTAL"}
...
def _formatar_ticket_migracoes(valor: float | int | None) -> str:
    try:
        valor_float = float(valor or 0.0)
    except Exception:
        valor_float = 0.0
    return f"R$ {formatar_numero_brasileiro(valor_float, 2)}"

def _calcular_ticket_medio_migracoes(valor_total: float, qtde_total: float) -> float:
    qtde = float(qtde_total or 0.0)
    if qtde == 0:
        return 0.0
    return float(valor_total or 0.0) / qtde

def montar_tabela_migracoes_pme_dimensao(
    df_migracoes: pd.DataFrame,
    dimensao_col: str,
    rotulo_coluna: str,
    metrica: str = "TICKET",
    mes_mom_ref: str | None = None,
    excluir_mes_atual: bool = False,
    mes_limite: str | None = None
) -> tuple[pd.DataFrame, pd.DataFrame]:
    """Monta tabelas mês a mês de Migrações PME por regional/plano."""
    colunas_vazias = [rotulo_coluna]
    if df_migracoes is None or df_migracoes.empty or "MES_ANO" not in df_migracoes.columns:
        return pd.DataFrame(columns=colunas_vazias), pd.DataFrame(columns=colunas_vazias)
    if dimensao_col not in df_migracoes.columns:
        return pd.DataFrame(columns=colunas_vazias), pd.DataFrame(columns=colunas_vazias)

    meses_ordem = _filtrar_meses_migracoes_ate_atual(
        df_migracoes["MES_ANO"].dropna().astype(str).unique().tolist()
    )
    if not meses_ordem:
        return pd.DataFrame(columns=colunas_vazias), pd.DataFrame(columns=colunas_vazias)
    if excluir_mes_atual:
        mes_atual = _mes_atual_migracoes()
        meses_fechados = [
            mes for mes in meses_ordem
            if str(mes).strip().lower() != str(mes_atual).strip().lower()
        ]
        if meses_fechados:
            meses_ordem = meses_fechados
    mes_limite_norm = str(mes_limite or "").replace("TEND.", "").strip().lower()
    if mes_limite_norm:
        try:
            data_limite = pd.Timestamp(mes_ano_para_data(mes_limite_norm)).normalize()
            meses_filtrados = [
                mes for mes in meses_ordem
                if pd.Timestamp(mes_ano_para_data(str(mes))).normalize() <= data_limite
            ]
            if meses_filtrados:
                meses_ordem = meses_filtrados
        except Exception:
            pass
    ...
    if "VAL_PRODUTO_FINAL" not in df_work.columns:
        df_work["VAL_PRODUTO_FINAL"] = 0.0
    df_work["VAL_PRODUTO_FINAL"] = normalizar_numerico_serie(df_work["VAL_PRODUTO_FINAL"]).fillna(0.0)

    agg = (
        df_work.groupby([dimensao_col, "MES_ANO"], as_index=False, observed=True, dropna=False)[["QTDE", "VAL_PRODUTO_FINAL"]]
        .sum()
    )
    metrica_norm = str(metrica or "TICKET").strip().upper()
    if metrica_norm == "TICKET":
        agg["VALOR_METRICA"] = np.where(
            agg["QTDE"].astype(float).ne(0),
            agg["VAL_PRODUTO_FINAL"].astype(float) / agg["QTDE"].astype(float),
            0.0
        )
    else:
        agg["VALOR_METRICA"] = agg["QTDE"].astype(float)
    ...
    colunas_valor = meses_ordem.copy()
    coluna_mom_ref = _resolver_coluna_mes_migracoes(colunas_valor, mes_mom_ref)
    mes_mom_base = str(coluna_mom_ref).replace("TEND.", "").strip()
    mes_mom_anterior = get_mes_anterior(mes_mom_base)
    coluna_mom_anterior = _resolver_coluna_mes_migracoes(colunas_valor, mes_mom_anterior)
    if str(coluna_mom_anterior).strip().upper().startswith("TEND."):
        coluna_mom_anterior = str(coluna_mom_anterior).replace("TEND.", "").strip()
    if str(coluna_mom_anterior).replace("TEND.", "").strip().lower() != str(mes_mom_anterior).strip().lower():
        coluna_mom_anterior = ""
    ...
    totais_raw = (
        df_work.groupby("MES_ANO", as_index=False, observed=True, dropna=False)[["QTDE", "VAL_PRODUTO_FINAL"]]
        .sum()
    )
    total_lookup = {}
    for _, row in totais_raw.iterrows():
        mes_ref = str(row["MES_ANO"]).strip()
        if metrica_norm == "TICKET":
            total_lookup[mes_ref] = _calcular_ticket_medio_migracoes(row["VAL_PRODUTO_FINAL"], row["QTDE"])
        else:
            total_lookup[mes_ref] = float(row["QTDE"] or 0.0)

    df_num = tabela[[*colunas_valor, "MoM"]].reset_index().rename(columns={dimensao_col: rotulo_coluna})
    linha_total = {rotulo_coluna: "TOTAL"}
    for mes in colunas_valor:
        linha_total[mes] = float(total_lookup.get(mes, 0.0))
    valor_total_atual = float(linha_total.get(coluna_mom_ref, 0.0))
    valor_total_anterior = float(linha_total.get(coluna_mom_anterior, 0.0)) if coluna_mom_anterior else 0.0
    linha_total["MoM"] = ((valor_total_atual / valor_total_anterior) - 1.0) * 100.0 if valor_total_anterior > 0 else 0.0
    df_num = pd.concat([pd.DataFrame([linha_total]), df_num], ignore_index=True)
    ...
    for col in df_fmt.columns:
        ...
        if col == "MoM":
            df_fmt[col] = serie_col.apply(_formatar_mom_migracoes)
        elif metrica_norm == "TICKET":
            df_fmt[col] = serie_col.apply(_formatar_ticket_migracoes)
        else:
            df_fmt[col] = serie_col.apply(lambda valor: formatar_numero_brasileiro(valor, 0))
    return df_fmt, df_num

def calcular_ticket_medio_migracoes_por_mes(
    df_migracoes: pd.DataFrame,
    regional_ref: str = "Todos"
) -> dict[str, float]:
    """Calcula ticket médio mensal: soma VAL_PRODUTO_FINAL / soma QTDE_FINAL."""
    if (
        df_migracoes is None or df_migracoes.empty or
        "MES_ANO" not in df_migracoes.columns or
        "VAL_PRODUTO_FINAL" not in df_migracoes.columns
    ):
        return {}

    df_work = df_migracoes.copy()
    regional_norm = str(regional_ref or "Todos").strip()
    if regional_norm.lower() not in {"", "todos"} and "REGIONAL" in df_work.columns:
        df_work = df_work[df_work["REGIONAL"].astype(str).str.strip().eq(regional_norm)].copy()
    if df_work.empty:
        return {}
    ...
    return {
        str(row["MES_ANO"]).strip(): _calcular_ticket_medio_migracoes(row["VAL_PRODUTO_FINAL"], row["QTDE"])
...
def montar_serie_grafico_migracoes_pme(
    ...
    regional_ref: str = "Todos",
    df_migracoes: pd.DataFrame | None = None
) -> pd.DataFrame:
    """Converte a tabela numerica de migracoes PME em serie mensal para o grafico."""
    colunas_saida = ["MES_ANO", "MES_LABEL", "QTDE", "TIPO", "TICKET_MEDIO", "PARCIAL"]
    ...
    row = linha.iloc[0]
    ticket_lookup = calcular_ticket_medio_migracoes_por_mes(df_migracoes, regional_ref=regional_ref)
    registros: list[dict[str, object]] = []
    meses_com_tendencia = {
        str(coluna).replace("TEND.", "").strip().lower()
        for coluna in df_tabela_numerica.columns
        if str(coluna).strip().upper().startswith("TEND.")
    }
    for coluna in df_tabela_numerica.columns:
        ...
        if not eh_tendencia and mes_ref_norm in meses_com_tendencia:
            continue
        valor = float(pd.to_numeric(pd.Series([row[coluna]]), errors="coerce").fillna(0.0).iloc[0])
        valor_parcial = np.nan
        if eh_tendencia and mes_ref in row.index:
            valor_parcial = float(pd.to_numeric(pd.Series([row.get(mes_ref, 0.0)]), errors="coerce").fillna(0.0).iloc[0])
        registros.append(
            {
                "MES_ANO": mes_ref,
                "MES_LABEL": f"{mes_ref.upper()} TEND." if eh_tendencia else mes_ref.upper(),
                "QTDE": valor,
                "TIPO": "TENDENCIA" if eh_tendencia else "REALIZADO",
                "TICKET_MEDIO": float(ticket_lookup.get(mes_ref, np.nan)),
                "PARCIAL": valor_parcial,
            }
        )
...
def criar_grafico_migracoes_pme_mensal(
    ...
    if idx_tend is not None and idx_tend > 0:
        mes_tend_ref = str(df_serie.iloc[idx_tend].get("MES_ANO", "")).strip()
        mes_tend_anterior = get_mes_anterior(mes_tend_ref)
        idx_real_anterior = next(
            (
                idx for idx, row in df_serie.iterrows()
                if str(row.get("TIPO", "")).upper() != "TENDENCIA"
                and str(row.get("MES_ANO", "")).strip().lower() == str(mes_tend_anterior).strip().lower()
            ),
            None
        )
        comparacoes.append({"origem": idx_real_anterior if idx_real_anterior is not None else idx_tend - 1, "destino": idx_tend})
    ...
    parciais = (
        pd.to_numeric(df_serie.get("PARCIAL", pd.Series([np.nan] * len(df_serie))), errors="coerce")
        .reindex(df_serie.index)
        .astype(float)
        .tolist()
    )
    customdata = []
    for idx in range(len(categorias)):
        tipo_label = "Tendência" if str(tipos[idx]).upper() == "TENDENCIA" else "Realizado"
        if str(tipos[idx]).upper() == "TENDENCIA":
            parcial = parciais[idx] if idx < len(parciais) else np.nan
            parcial_txt = formatar_numero_brasileiro(parcial, 0) if pd.notna(parcial) else "0"
            detalhe_hover = (
                f"<b>Parcial:</b> {parcial_txt}<br>"
                f"<b>Tend:</b> {formatar_numero_brasileiro(valores[idx], 0)}"
            )
        else:
            detalhe_hover = f"<b>Realizado:</b> {formatar_numero_brasileiro(valores[idx], 0)}"
        customdata.append([categorias[idx], tipo_label, detalhe_hover])
...
def criar_tabela_html_necessidade_diaria_produto(
    ...
    produto_norm = normalizar_rotulo_produto(produto_ref)
    df_work = df_work[df_work['COD_PLATAFORMA'] == produto_norm].copy(deep=False)
    if regional_ref and str(regional_ref).strip() != "Todas":
        df_work = df_work[df_work['REGIONAL'] == regional_ref].copy(deep=False)
    if canal_ref and str(canal_ref).strip() != "Todos":
        df_work = df_work[df_work['CANAL_PLAN'] == canal_ref].copy(deep=False)
    if df_work.empty:
        return "", (ctx_default if incluir_ctx else {})
    ...
    aliases_necessidade = set(alias_vb) | set(alias_ativ) | set(alias_meta_ativ)
    df_work = df_work[df_work['IND_NORM'].isin(aliases_necessidade)].copy(deep=False)
    if df_work.empty:
        return "", (ctx_default if incluir_ctx else {})
    ...
    .{table_id} tbody td.col-pct::before,
    .{table_id} tbody td.pct-positivo::before,
    .{table_id} tbody td.pct-negativo::before,
    .{table_id} tbody td.pct-neutro::before {{
        content: none !important;
        display: none !important;
    }}
    ...
    .{table_id} tbody tr.linha-projecao td.col-pct,
    .{table_id} tbody tr.linha-realizado td.col-pct,
    .{table_id} tbody tr.linha-ating td.col-pct {{
        background: transparent !important;
        background-color: transparent !important;
        background-image: none !important;
        box-shadow: none !important;
...
def _normalizar_texto_analise_ativados(valor) -> str:
    if pd.isna(valor):
        return ""
    texto = unicodedata.normalize("NFKD", str(valor))
    texto = texto.encode("ASCII", "ignore").decode("ASCII")
...
def _complementar_demanda_ligacoes_analise_ativados(base: pd.DataFrame) -> pd.DataFrame:
    if base is None or base.empty or "IND_NORM" not in base.columns:
        return base

    mask_ligacoes = (
        base.get("METRICA", pd.Series("", index=base.index)).astype(str).eq("DEMANDA")
        & base["IND_NORM"].astype(str).str.contains("LIGAC", regex=True, na=False)
    )
    if not bool(mask_ligacoes.any()):
        return base

    try:
        path_lig = Path(LIGACOES_PERFORMANCE_FILE_PATH)
        lig_mtime = path_lig.stat().st_mtime if path_lig.exists() else None
        df_lig = load_ligacoes_performance_data(str(LIGACOES_PERFORMANCE_FILE_PATH), lig_mtime)
    except Exception:
        return base
    ...
    if df_lig is None or df_lig.empty or not colunas_lig.issubset(set(df_lig.columns)):
        return base

    lig = df_lig.loc[:, list(colunas_lig)].copy()
    lig["IND_NORM"] = lig["DSC_INDICADOR"].apply(_normalizar_texto_analise_ativados)
    lig = lig.loc[lig["IND_NORM"].astype(str).str.contains("LIGAC", regex=True, na=False)].copy()
    if lig.empty:
        return base

    lig["PRODUTO"] = lig["COD_PLATAFORMA"].apply(normalizar_rotulo_produto)
    lig["CANAL_PLAN"] = lig["CANAL_PLAN"].astype(str).str.strip()
    lig["REGIONAL"] = lig["REGIONAL"].astype(str).str[:3].str.upper()
    lig["MES_ANO"] = lig["dat_tratada"].astype(str).str.strip().str.lower()
    for coluna_num in ["QTDE", "TEND_QTD"]:
        lig[coluna_num] = pd.to_numeric(lig[coluna_num], errors="coerce").fillna(0.0)

    chaves = ["PRODUTO", "CANAL_PLAN", "REGIONAL", "MES_ANO"]
    lig_agg = (
        lig.groupby(chaves, as_index=False, observed=True)[["QTDE", "TEND_QTD"]]
        .sum()
        .rename(columns={"QTDE": "QTDE_LIGACOES_REAL", "TEND_QTD": "TEND_LIGACOES_REAL"})
    )
    if lig_agg.empty:
        return base

    base_sem_lig = base.loc[~mask_ligacoes].copy()
    base_lig = base.loc[mask_ligacoes].copy()

    agregacoes = {}
    for coluna in base_lig.columns:
        if coluna in chaves:
            continue
        if coluna in {"QTDE", "TEND_QTD", "DESAFIO_QTD"}:
            agregacoes[coluna] = "sum"
        else:
            agregacoes[coluna] = "first"

    base_lig = base_lig.groupby(chaves, as_index=False, observed=True).agg(agregacoes)
    base_lig = base_lig.merge(lig_agg, on=chaves, how="left")

    real_lig = pd.to_numeric(base_lig.get("QTDE_LIGACOES_REAL"), errors="coerce")
    tend_lig = pd.to_numeric(base_lig.get("TEND_LIGACOES_REAL"), errors="coerce")
    tend_base = pd.to_numeric(base_lig.get("TEND_QTD"), errors="coerce").fillna(0.0)

    mask_real_valido = real_lig.notna()
    base_lig.loc[mask_real_valido, "QTDE"] = real_lig.loc[mask_real_valido].fillna(0.0)

    # Mantem a tendencia projetada da base principal quando ela existir; nos meses fechados,
    # usa o realizado de ligacoes para impedir YTD com real zerado contra orcamento cheio.
    tendencia_fallback = tend_lig.where(tend_lig.notna(), real_lig)
    mask_tend_valido = tendencia_fallback.notna() & (tend_base <= 0)
    base_lig.loc[mask_tend_valido, "TEND_QTD"] = tendencia_fallback.loc[mask_tend_valido].fillna(0.0)

    base_lig = base_lig.drop(columns=["QTDE_LIGACOES_REAL", "TEND_LIGACOES_REAL"], errors="ignore")
    return pd.concat([base_sem_lig, base_lig.loc[:, base.columns]], ignore_index=True)


def _preparar_base_analise_ativados(df_ref: pd.DataFrame) -> pd.DataFrame:
    colunas_base = {
        "CANAL_PLAN",
        "REGIONAL",
        "COD_PLATAFORMA",
    ...
    if df_ref is None or df_ref.empty or not colunas_base.issubset(set(df_ref.columns)):
        return pd.DataFrame()
    ...
    base = df_ref.loc[:, colunas].copy()
    base["PRODUTO"] = base["COD_PLATAFORMA"].apply(normalizar_rotulo_produto)
    base["CANAL_PLAN"] = base["CANAL_PLAN"].astype(str).str.strip()
    base["REGIONAL"] = base["REGIONAL"].astype(str).str[:3].str.upper()
    base["MES_ANO"] = base["dat_tratada"].astype(str).str.strip().str.lower()
    base["IND_NORM"] = base["DSC_INDICADOR"].apply(_normalizar_texto_analise_ativados)

    for coluna_num in ["QTDE", "TEND_QTD", "DESAFIO_QTD"]:
        if coluna_num not in base.columns:
            base[coluna_num] = 0.0
        base[coluna_num] = pd.to_numeric(base[coluna_num], errors="coerce").fillna(0.0)

    coluna_data = "DAT_MOVIMENTO2" if "DAT_MOVIMENTO2" in base.columns else "DAT_MOVIMENTO"
    if coluna_data in base.columns:
        base["DATA_DIA"] = pd.to_datetime(base[coluna_data], errors="coerce", dayfirst=True).dt.normalize()
    else:
        base["DATA_DIA"] = pd.NaT
    if base["DATA_DIA"].isna().all():
        base["DATA_DIA"] = base["MES_ANO"].apply(lambda mes: mes_ano_para_data(mes) if str(mes).strip() else pd.NaT)

    ind_norm = base["IND_NORM"].astype(str)
    mask_demanda = ind_norm.str.contains("PEDID|LIGAC", regex=True, na=False)
    mask_venda_bruta = (
        ind_norm.str.contains("VENDA BRUTA|VENDAS BRUTAS|GROSS BRUTO|GROSS BRUT", regex=True, na=False)
        | ind_norm.eq("VB")
    )
    mask_ativados = (
        (base["PRODUTO"].eq("CONTA") & ind_norm.str.contains("GROSS LIQ", regex=True, na=False))
        | (base["PRODUTO"].eq("FIXA") & ind_norm.str.contains("INSTAL", regex=True, na=False))
    )
    mask_meta_fixa = base["PRODUTO"].eq("FIXA") & ind_norm.str.contains("GROSS LIQ", regex=True, na=False)

    base["METRICA"] = np.select(
        [mask_demanda, mask_venda_bruta, mask_ativados | mask_meta_fixa],
        ["DEMANDA", "VENDA BRUTA", "ATIVADOS"],
        default="",
    )
    # Na FIXA, o orcamento do indicador final vem em GROSS LIQUIDO, enquanto o realizado/tend vem em Instalacao.
    # Mantemos a linha de GROSS apenas como meta para evitar somar valor operacional indevido.
    base.loc[mask_meta_fixa, ["QTDE", "TEND_QTD"]] = 0.0
    base = base.loc[
        base["PRODUTO"].isin(["CONTA", "FIXA"]) & base["METRICA"].ne("")
    ].copy()
    base = _complementar_demanda_ligacoes_analise_ativados(base)
    base = _adicionar_orc_venda_bruta_analise_ativados(base)
    return base


def _adicionar_orc_venda_bruta_analise_ativados(base: pd.DataFrame) -> pd.DataFrame:
    if base is None or base.empty:
        return base
    colunas_chave = ["PRODUTO", "CANAL_PLAN", "REGIONAL", "MES_ANO"]
    if not set(colunas_chave + ["METRICA", "QTDE", "TEND_QTD", "DESAFIO_QTD"]).issubset(base.columns):
        return base

    base_calc = base.copy()
    for coluna in ["QTDE", "TEND_QTD", "DESAFIO_QTD"]:
        base_calc[coluna] = pd.to_numeric(base_calc[coluna], errors="coerce").fillna(0.0)
    base_calc["VALOR_BASE_CONVERSAO"] = np.where(
        base_calc["TEND_QTD"] > 0,
        base_calc["TEND_QTD"],
        base_calc["QTDE"],
    )

    resumo = (
        base_calc.groupby(colunas_chave + ["METRICA"], as_index=False, observed=True)[
            ["VALOR_BASE_CONVERSAO", "DESAFIO_QTD"]
        ]
        .sum()
    )
    pivot_valor = resumo.pivot_table(
        index=colunas_chave,
        columns="METRICA",
        values="VALOR_BASE_CONVERSAO",
        aggfunc="sum",
        fill_value=0.0,
        observed=True,
    )
    pivot_orc = resumo.pivot_table(
        index=colunas_chave,
        columns="METRICA",
        values="DESAFIO_QTD",
        aggfunc="sum",
    ...
    if "VENDA BRUTA" not in pivot_valor.columns or "ATIVADOS" not in pivot_valor.columns:
        return base

    for coluna in ["VENDA BRUTA", "ATIVADOS"]:
        if coluna not in pivot_orc.columns:
            pivot_orc[coluna] = 0.0
    conversao = np.where(
        pivot_valor["VENDA BRUTA"] > 0,
        pivot_valor["ATIVADOS"] / pivot_valor["VENDA BRUTA"],
        np.nan,
    )
    orc_vb_estimado = np.where(
        (pivot_orc["VENDA BRUTA"] <= 0) & (pivot_orc["ATIVADOS"] > 0) & pd.notna(conversao) & (conversao > 0),
        pivot_orc["ATIVADOS"] / conversao,
        0.0,
    )
    serie_orc_vb = pd.Series(orc_vb_estimado, index=pivot_valor.index, name="DESAFIO_QTD")
    serie_orc_vb = serie_orc_vb[pd.to_numeric(serie_orc_vb, errors="coerce").fillna(0.0) > 0]
    if serie_orc_vb.empty:
        return base

    linhas_meta = serie_orc_vb.reset_index()
    linhas_meta["METRICA"] = "VENDA BRUTA"
    linhas_meta["DSC_INDICADOR"] = "META VENDA BRUTA ESTIMADA"
    linhas_meta["IND_NORM"] = "META VENDA BRUTA ESTIMADA"
    linhas_meta["COD_PLATAFORMA"] = linhas_meta["PRODUTO"]
    linhas_meta["dat_tratada"] = linhas_meta["MES_ANO"]
    linhas_meta["QTDE"] = 0.0
    linhas_meta["TEND_QTD"] = 0.0
    linhas_meta["DATA_DIA"] = linhas_meta["MES_ANO"].apply(lambda mes: mes_ano_para_data(mes) if str(mes).strip() else pd.NaT)
    for coluna in base.columns:
        if coluna not in linhas_meta.columns:
            if coluna in {"QTDE", "TEND_QTD", "DESAFIO_QTD"}:
                linhas_meta[coluna] = 0.0
            elif coluna in {"DAT_MOVIMENTO2", "DAT_MOVIMENTO"}:
                linhas_meta[coluna] = linhas_meta["DATA_DIA"]
            else:
                linhas_meta[coluna] = ""
    linhas_meta = linhas_meta.loc[:, base.columns]
    return pd.concat([base, linhas_meta], ignore_index=True)


def _obter_meses_periodo_analise_ativados(
    base: pd.DataFrame,
    mes_ref: str,
    modo_periodo: str = "MES",
) -> list[str]:
    if base is None or base.empty or "MES_ANO" not in base.columns:
        return []
    mes_norm = str(mes_ref or "").strip().lower()
    if not mes_norm:
        return []
    modo_norm = str(modo_periodo or "MES").strip().upper()
    if modo_norm != "YTD":
        return [mes_norm]

    try:
        data_ref = mes_ano_para_data(mes_norm)
    except Exception:
        return [mes_norm]

    pares = []
    for mes in base["MES_ANO"].dropna().astype(str).str.strip().str.lower().unique():
        try:
            data_mes = mes_ano_para_data(mes)
        except Exception:
            continue
        if data_mes.year == data_ref.year and data_mes <= data_ref:
            pares.append((mes, data_mes))
    pares.sort(key=lambda item: item[1])
    return [mes for mes, _ in pares]


def _descrever_periodo_analise_ativados(meses_periodo: list[str], modo_periodo: str) -> str:
    if not meses_periodo:
        return "-"
    if str(modo_periodo or "MES").strip().upper() == "YTD":
        return f"YTD {meses_periodo[0].upper()} até {meses_periodo[-1].upper()}"
    return meses_periodo[-1].upper()


def _agregar_analise_ativados(
    base: pd.DataFrame,
    mes_ref: str,
    chaves: list[str],
    modo_periodo: str = "MES",
    ...
    modo_norm = str(modo_periodo or "MES").strip().upper()
    meses_periodo = _obter_meses_periodo_analise_ativados(base, mes_ref, modo_norm)
    if not meses_periodo:
        return pd.DataFrame()

    base_mes = base.loc[base["MES_ANO"].isin(meses_periodo)].copy()
    if base_mes.empty:
        return pd.DataFrame()

    mes_corrente = get_mes_atual_formatado().strip().lower()
    usar_tend_mes_ref = eh_mes_atual_dashboard(str(mes_ref or "").strip().lower(), mes_corrente)

    # Primeiro agrega por mes para nunca somar real+tend no mesmo indicador.
    chaves_mes = chaves + ["MES_ANO"]
    agg_mes = (
        base_mes.groupby(chaves_mes, as_index=False, observed=True)[["QTDE", "TEND_QTD", "DESAFIO_QTD"]]
        .sum()
    )
    mask_tend_mes = (
        usar_tend_mes_ref
        & agg_mes["MES_ANO"].astype(str).str.strip().str.lower().eq(mes_corrente)
        & (agg_mes["TEND_QTD"] > 0)
    )
    agg_mes["VALOR_MES"] = np.where(mask_tend_mes, agg_mes["TEND_QTD"], agg_mes["QTDE"])
    agg_mes["USOU_TEND"] = mask_tend_mes

    agg = (
        agg_mes.groupby(chaves, as_index=False, observed=True)[["QTDE", "TEND_QTD", "DESAFIO_QTD", "VALOR_MES", "USOU_TEND"]]
        .sum()
    )
    agg["VALOR_ANALISE"] = agg["VALOR_MES"]
    if modo_norm == "YTD":
        agg["FONTE"] = np.where(agg["USOU_TEND"] > 0, "YTD+TEND", "YTD REAL")
    else:
        agg["FONTE"] = np.where(agg["USOU_TEND"] > 0, "TEND", "REAL")
    agg["ORC"] = agg["DESAFIO_QTD"]
    agg["GAP"] = agg["VALOR_ANALISE"] - agg["ORC"]
    agg["ATING_PCT"] = np.where(agg["ORC"] > 0, (agg["VALOR_ANALISE"] / agg["ORC"]) - 1.0, np.nan)
    agg = agg.drop(columns=["VALOR_MES", "USOU_TEND"], errors="ignore")
    return agg


def _formatar_pct_delta_ativados(valor) -> str:
    if pd.isna(valor):
        return "-"
    return f"{float(valor) * 100:+.1f}%".replace(".", ",")


def _formatar_pct_simples_ativados(valor) -> str:
    if pd.isna(valor):
        return "-"
    return f"{float(valor) * 100:.1f}%".replace(".", ",")


def _map_dataframe_compat(df_ref: pd.DataFrame, func) -> pd.DataFrame:
    mapper = getattr(df_ref, "map", None)
    if callable(mapper):
        return mapper(func)
    return df_ref.apply(lambda coluna: coluna.map(func))


def _formatar_card_gap_ativados(titulo: str, valor: str, detalhe: str, tom: str = "neutro") -> str:
    cor_barra = {
        "negativo": "#C62828",
        "positivo": "#1B5E20",
        "alerta": "#A23B36",
    ...
    return f"""
        <div style="
            background:#FFFFFF;
            border:1px solid rgba(121,14,9,.12);
            border-left:5px solid {cor_barra};
            border-radius:14px;
            box-shadow:0 8px 24px rgba(31,41,55,.08);
            padding:14px 16px;
            min-height:96px;">
            <div style="font-size:11px;font-weight:800;color:#6B7280;letter-spacing:.08em;text-transform:uppercase;">
                {escape(titulo)}
            </div>
            <div style="font-size:26px;font-weight:850;color:#1F2937;line-height:1.08;margin-top:7px;">
                {escape(valor)}
            </div>
            <div style="font-size:12px;font-weight:650;color:#4B5563;margin-top:6px;line-height:1.25;">
                {escape(detalhe)}
            </div>
        </div>
    """
...
def _formatar_insights_ativados(titulo: str, itens: list[str]) -> str:
    itens_validos = [str(item).strip() for item in itens if str(item).strip()]
    if not itens_validos:
        return ""

    cards = []
    for idx, item in enumerate(itens_validos[:5], start=1):
        if "|" in item:
            rotulo, texto = [parte.strip() for parte in item.split("|", 1)]
        else:
            rotulo, texto = f"Ponto {idx}", item
        rotulo_norm = _normalizar_texto_analise_ativados(rotulo)
        cor = "#790E09"
        fundo = "#FFFFFF"
        if "CRIT" in rotulo_norm or "GAP" in rotulo_norm:
            cor = "#C62828"
            fundo = "#FFF7F7"
        elif "PERIODO" in rotulo_norm:
            cor = "#374151"
            fundo = "#F8FAFC"
        elif "CONVERS" in rotulo_norm:
            cor = "#5A6268"
            fundo = "#F8FAFC"
        cards.append(
            f"""
            <div style="
                ...
                <div style="font-size:10px;font-weight:850;color:{cor};letter-spacing:.08em;text-transform:uppercase;margin-bottom:5px;">
                    {escape(rotulo)}
                </div>
                <div style="font-size:13px;font-weight:750;color:#1F2937;line-height:1.25;">
                    {escape(texto)}
                </div>
            </div>
            """
    ...
    return f"""
        <div style="
            ...
            border:1px solid rgba(121,14,9,.10);
            border-radius:18px;
            box-shadow:0 10px 30px rgba(31,41,55,.075);
            padding:15px;
            margin:12px 0 16px 0;">
            <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:12px;">
                <div style="font-size:12px;font-weight:900;color:#790E09;letter-spacing:.10em;text-transform:uppercase;">
                {escape(titulo)}
                </div>
                <div style="height:1px;background:linear-gradient(90deg,rgba(121,14,9,.22),rgba(121,14,9,0));flex:1;"></div>
            </div>
...
def _card_ponto_atencao_ativados(rotulo: str, texto: str) -> str:
    rotulo_norm = _normalizar_texto_analise_ativados(rotulo)
    cor = "#790E09"
    fundo = "#FFFFFF"
    if "CRIT" in rotulo_norm or "GAP" in rotulo_norm:
...
def _render_pontos_atencao_ativados(itens: list[str]) -> None:
    itens_validos = [str(item).strip() for item in itens if str(item).strip()]
    if not itens_validos:
        return
    ...
    st.markdown(
        ...
        <div style="
            ...
            padding:0 2px;">
            <div style="font-size:12px;font-weight:900;color:#790E09;letter-spacing:.10em;text-transform:uppercase;white-space:nowrap;">
                PONTOS DE ATENÇÃO
            </div>
            <div style="height:1px;background:linear-gradient(90deg,rgba(121,14,9,.22),rgba(121,14,9,0));flex:1;"></div>
        </div>
    ...
    cols = st.columns(min(len(itens_validos), 5), gap="small")
    for idx, item in enumerate(itens_validos[:5]):
        if "|" in item:
            rotulo, texto = [parte.strip() for parte in item.split("|", 1)]
        else:
            rotulo, texto = f"Ponto {idx + 1}", item
        with cols[idx]:
            st.markdown(_card_ponto_atencao_ativados(rotulo, texto), unsafe_allow_html=True)


def _criar_tabela_html_ofensores_regionais_ativados(
    tabela: pd.DataFrame,
    table_id: str = "tbl_ofensores_regionais_ativados",
) -> str:
    if tabela is None or tabela.empty:
        return ""

    df = tabela.copy()
    for coluna in ["PRODUTO", "CANAL_PLAN", "REGIONAL", "METRICA", "FONTE"]:
        if coluna not in df.columns:
            df[coluna] = "-"

    valor_num = pd.to_numeric(df.get("VALOR_ANALISE", 0), errors="coerce").fillna(0.0)
    orc_num = pd.to_numeric(df.get("ORC", 0), errors="coerce").fillna(0.0)
    gap_num = pd.to_numeric(df.get("GAP", 0), errors="coerce").fillna(0.0)
    ating_num = pd.to_numeric(df.get("ATING_PCT", np.nan), errors="coerce")

    gap_total = float(gap_num.sum())
    qtd_abaixo = int((gap_num < 0).sum())
    qtd_acima = int((gap_num > 0).sum())

    def _fmt_gap(valor: float) -> str:
        texto = formatar_numero_brasileiro(valor, 0)
        return f"+{texto}" if valor > 0 else texto

    def _classe_gap(valor: float) -> str:
        if valor < 0:
            return "neg"
        if valor > 0:
            return "pos"
    ...
    def _prioridade(gap: float, ating: float) -> str:
        if gap < 0 and pd.notna(ating) and ating <= -0.20:
            return "Critico"
        if gap < 0:
            return "Atencao"
        if gap > 0:
    ...
    for posicao, (_, row) in enumerate(df.iterrows()):
        gap = float(gap_num.iloc[posicao])
        ating = ating_num.iloc[posicao] if posicao < len(ating_num) else np.nan
        classe = _classe_gap(gap)
        prioridade = _prioridade(gap, ating)
        classe_linha = f"row-{classe}"
        if posicao == 0 and gap < 0:
            classe_linha += " row-critical"

        linhas.append(
            f"""
            <tr class="{classe_linha}">
                <td><span class="priority priority-{classe}">{escape(prioridade)}</span></td>
                <td><span class="pill pill-produto">{escape(str(row.get("PRODUTO", "-")))}</span></td>
                <td class="text-strong">{escape(str(row.get("REGIONAL", "-")))}</td>
                <td>{escape(str(row.get("CANAL_PLAN", "-")))}</td>
                <td><span class="pill pill-metrica">{escape(str(row.get("METRICA", "-")))}</span></td>
                <td><span class="pill pill-fonte">{escape(str(row.get("FONTE", "-")))}</span></td>
                <td class="num">{formatar_numero_brasileiro(valor_num.iloc[posicao], 0)}</td>
                <td class="num">{formatar_numero_brasileiro(orc_num.iloc[posicao], 0)}</td>
                <td class="num gap-{classe}">{_fmt_gap(gap)}</td>
                <td class="num gap-{classe}">{_formatar_pct_delta_ativados(ating)}</td>
            </tr>
            """
        )

    table_id_safe = re.sub(r"[^a-zA-Z0-9_-]", "", str(table_id)) or "tbl_ofensores_regionais_ativados"
    classe_gap_total = "neg" if gap_total < 0 else "pos" if gap_total > 0 else ""

    return f"""
    <style>
        #{table_id_safe}.ofensores-card {{
            background:#FFFFFF;
            border:1px solid rgba(31,41,55,.10);
            border-radius:16px;
        ...
        #{table_id_safe} .ofensores-summary {{
            display:flex;
            flex-wrap:wrap;
            gap:8px;
            margin-bottom:10px;
        }}
        #{table_id_safe} .summary-chip {{
            background:#F8FAFC;
        ...
        #{table_id_safe} .summary-chip.neg {{ color:#C62828; background:#FFF7F7; border-color:#F3D1D1; }}
        #{table_id_safe} .summary-chip.pos {{ color:#1B5E20; background:#F3FBF5; border-color:#D8EBDD; }}
        #{table_id_safe} .ofensores-table-wrap {{
            max-height:430px;
            overflow:auto;
        ...
        #{table_id_safe} tbody td {{
            padding:9px 10px;
            border-bottom:1px solid #EEF2F7;
            vertical-align:middle;
        }}
        #{table_id_safe} tbody tr:hover {{ background:#FAFAFB; }}
        #{table_id_safe} tbody tr.row-critical td:first-child {{ border-left:4px solid #C62828; }}
        #{table_id_safe} tbody tr.row-pos td:first-child {{ border-left:4px solid #1B5E20; }}
        #{table_id_safe} tbody tr.row-neu td:first-child {{ border-left:4px solid #CBD5E1; }}
        #{table_id_safe} .num {{ text-align:right; font-variant-numeric:tabular-nums; white-space:nowrap; }}
        #{table_id_safe} .text-strong {{ font-weight:850; color:#111827; }}
        #{table_id_safe} .gap-neg {{ color:#C62828; font-weight:900; }}
        #{table_id_safe} .gap-pos {{ color:#1B5E20; font-weight:900; }}
        #{table_id_safe} .gap-neu {{ color:#374151; font-weight:850; }}
        #{table_id_safe} .pill,
        #{table_id_safe} .priority {{
            display:inline-flex;
            align-items:center;
            border-radius:999px;
            font-size:10.5px;
        ...
        #{table_id_safe} .pill-produto {{ background:#F8FAFC; color:#374151; border:1px solid #E5E7EB; }}
        #{table_id_safe} .pill-metrica {{ background:#FFF7F2; color:#790E09; border:1px solid #F2D5CB; }}
        #{table_id_safe} .pill-fonte {{ background:#F3F4F6; color:#4B5563; border:1px solid #E5E7EB; }}
        #{table_id_safe} .priority-neg {{ background:#FFF7F7; color:#C62828; border:1px solid #F3D1D1; }}
        #{table_id_safe} .priority-pos {{ background:#F3FBF5; color:#1B5E20; border:1px solid #D8EBDD; }}
        #{table_id_safe} .priority-neu {{ background:#F8FAFC; color:#374151; border:1px solid #E5E7EB; }}
    </style>
    <div id="{table_id_safe}" class="ofensores-card">
        <div class="ofensores-summary">
            <span class="summary-chip {classe_gap_total}">Gap total: {_fmt_gap(gap_total)}</span>
            <span class="summary-chip neg">Abaixo do ORC: {qtd_abaixo}</span>
            <span class="summary-chip pos">Acima do ORC: {qtd_acima}</span>
            <span class="summary-chip">Linhas: {len(df)}</span>
        </div>
        <div class="ofensores-table-wrap">
            <table>
                <thead>
                    <tr>
                        <th>Status</th>
...
def _criar_tabela_html_resumo_semanal_gap_ativados(
    resumo_semana: pd.DataFrame,
    table_id: str = "tbl_resumo_semanal_gap_ativados",
) -> str:
    if resumo_semana is None or resumo_semana.empty:
        return ""

    df = resumo_semana.copy()
    for coluna in ["SEMANA_LABEL", "REAL", "ORC", "GAP", "ATING"]:
        if coluna not in df.columns:
            df[coluna] = np.nan

    real_num = pd.to_numeric(df["REAL"], errors="coerce").fillna(0.0)
    orc_num = pd.to_numeric(df["ORC"], errors="coerce").fillna(0.0)
    gap_num = pd.to_numeric(df["GAP"], errors="coerce").fillna(0.0)
    ating_num = pd.to_numeric(df["ATING"], errors="coerce")
    gap_total = float(gap_num.sum())
    ...
    for idx, row in df.reset_index(drop=True).iterrows():
        gap = float(gap_num.iloc[idx])
        classe = _classe(gap)
        status = "Abaixo" if gap < 0 else "Acima" if gap > 0 else "No ORC"
        linhas.append(
            f"""
            <tr class="row-{classe}">
                <td class="semana">{escape(str(row.get("SEMANA_LABEL", "-")))}</td>
                <td><span class="status status-{classe}">{status}</span></td>
                <td class="num">{formatar_numero_brasileiro(real_num.iloc[idx], 0)}</td>
                <td class="num">{formatar_numero_brasileiro(orc_num.iloc[idx], 0)}</td>
                <td class="num gap-{classe}">{_fmt_gap(gap)}</td>
                <td class="num gap-{classe}">{_formatar_pct_delta_ativados(ating_num.iloc[idx])}</td>
            </tr>
            """
        )

    table_id_safe = re.sub(r"[^a-zA-Z0-9_-]", "", str(table_id)) or "tbl_resumo_semanal_gap_ativados"
    classe_total = _classe(gap_total)

    return f"""
    <style>
        #{table_id_safe}.weekly-card {{
            background:#FFFFFF;
            border:1px solid rgba(31,41,55,.10);
            border-radius:15px;
            box-shadow:0 8px 22px rgba(31,41,55,.065);
            padding:11px;
        }}
        #{table_id_safe} .weekly-head {{
            display:flex;
            align-items:center;
            justify-content:space-between;
            gap:10px;
        ...
        #{table_id_safe} .weekly-chip.neg {{ background:#FFF7F7; color:#C62828; border-color:#F3D1D1; }}
        #{table_id_safe} .weekly-chip.pos {{ background:#F3FBF5; color:#1B5E20; border-color:#D8EBDD; }}
        #{table_id_safe} table {{
            width:100%;
            border-collapse:separate;
            border-spacing:0;
        ...
        #{table_id_safe} tbody tr:last-child td {{ border-bottom:0; }}
        #{table_id_safe} tbody tr.row-neg td:first-child {{ border-left:4px solid #C62828; }}
        #{table_id_safe} tbody tr.row-pos td:first-child {{ border-left:4px solid #1B5E20; }}
        #{table_id_safe} tbody tr.row-neu td:first-child {{ border-left:4px solid #CBD5E1; }}
        #{table_id_safe} .semana {{ font-weight:900; color:#111827; }}
        #{table_id_safe} .num {{ text-align:right; font-variant-numeric:tabular-nums; white-space:nowrap; }}
        #{table_id_safe} .gap-neg {{ color:#C62828; font-weight:900; }}
        #{table_id_safe} .gap-pos {{ color:#1B5E20; font-weight:900; }}
        #{table_id_safe} .gap-neu {{ color:#374151; font-weight:850; }}
        #{table_id_safe} .status {{
            display:inline-flex;
            border-radius:999px;
            font-size:10.3px;
            font-weight:850;
        ...
        #{table_id_safe} .status-neg {{ background:#FFF7F7; color:#C62828; border:1px solid #F3D1D1; }}
        #{table_id_safe} .status-pos {{ background:#F3FBF5; color:#1B5E20; border:1px solid #D8EBDD; }}
        #{table_id_safe} .status-neu {{ background:#F8FAFC; color:#374151; border:1px solid #E5E7EB; }}
    </style>
    <div id="{table_id_safe}" class="weekly-card">
        <div class="weekly-head">
            <div class="weekly-title">Resumo semanal</div>
            <div class="weekly-chip {classe_total}">Gap total: {_fmt_gap(gap_total)}</div>
        </div>
        <table>
            <thead>
                <tr>
...
def _criar_tabela_html_ofensores_canais_ativados(
    tabela: pd.DataFrame,
    table_id: str = "tbl_ofensores_canais_ativados",
) -> str:
    if tabela is None or tabela.empty:
        return ""

    df = tabela.copy()
    for coluna in ["PRODUTO", "CANAL_PLAN", "METRICA", "FONTE"]:
        if coluna not in df.columns:
            df[coluna] = "-"
    ...
    for posicao, (_, row) in enumerate(df.iterrows()):
        ...
        linhas.append(
            ...
            <tr class="row-{classe}">
                <td><span class="priority priority-{classe}">{escape(prioridade)}</span></td>
                <td><span class="pill pill-produto">{escape(str(row.get("PRODUTO", "-")))}</span></td>
                <td class="text-strong">{escape(str(row.get("CANAL_PLAN", "-")))}</td>
                <td><span class="pill pill-metrica">{escape(str(row.get("METRICA", "-")))}</span></td>
                <td><span class="pill pill-fonte">{escape(str(row.get("FONTE", "-")))}</span></td>
                <td class="num">{formatar_numero_brasileiro(valor_num.iloc[posicao], 0)}</td>
                <td class="num">{formatar_numero_brasileiro(orc_num.iloc[posicao], 0)}</td>
    ...
    table_id_safe = re.sub(r"[^a-zA-Z0-9_-]", "", str(table_id)) or "tbl_ofensores_canais_ativados"
    classe_total = _classe(gap_total)
    qtd_abaixo = int((gap_num < 0).sum())
    qtd_acima = int((gap_num > 0).sum())
    ...
    <div id="{table_id_safe}" class="channel-card">
        <div class="channel-summary">
            <span class="summary-chip {classe_total}">Gap total: {_fmt_gap(gap_total)}</span>
            <span class="summary-chip neg">Abaixo do ORC: {qtd_abaixo}</span>
            <span class="summary-chip pos">Acima do ORC: {qtd_acima}</span>
        </div>
        <table>
...
def _criar_tabela_html_conversao_ativados(
    tabela: pd.DataFrame,
    table_id: str = "tbl_conversao_regionais_ativados",
) -> str:
    if tabela is None or tabela.empty:
        return ""

    df = tabela.copy()
    for coluna in ["PRODUTO", "CANAL_PLAN", "REGIONAL"]:
        if coluna not in df.columns:
            df[coluna] = "-"

    vb_num = pd.to_numeric(df.get("VENDA BRUTA", 0), errors="coerce").fillna(0.0)
    ativ_num = pd.to_numeric(df.get("ATIVADOS", 0), errors="coerce").fillna(0.0)
    conv_num = pd.to_numeric(df.get("CONVERSAO", np.nan), errors="coerce")
    pp_num = pd.to_numeric(df.get("PP_MOM", np.nan), errors="coerce")

    conv_media = conv_num.replace([np.inf, -np.inf], np.nan).mean()
    pp_media = pp_num.replace([np.inf, -np.inf], np.nan).mean()
    ...
    def _fmt_pp(valor: float) -> str:
        if pd.isna(valor):
            return "-"
        return f"{float(valor) * 100:+.1f} p.p.".replace(".", ",")

    linhas = []
    for posicao, (_, row) in enumerate(df.iterrows()):
        conv = conv_num.iloc[posicao]
        pp = pp_num.iloc[posicao] if posicao < len(pp_num) else np.nan
        classe_pp = _classe(pp)
        classe_conv = "pos" if pd.notna(conv) and conv >= conv_media else "neg" if pd.notna(conv) else "neu"
        linhas.append(
            f"""
            <tr class="row-{classe_conv}">
                <td><span class="pill pill-produto">{escape(str(row.get("PRODUTO", "-")))}</span></td>
                <td>{escape(str(row.get("CANAL_PLAN", "-")))}</td>
                <td class="text-strong">{escape(str(row.get("REGIONAL", "-")))}</td>
                <td class="num">{formatar_numero_brasileiro(vb_num.iloc[posicao], 0)}</td>
                <td class="num">{formatar_numero_brasileiro(ativ_num.iloc[posicao], 0)}</td>
                <td class="num conv-{classe_conv}">{_formatar_pct_simples_ativados(conv)}</td>
                <td class="num conv-{classe_pp}">{_fmt_pp(pp)}</td>
            </tr>
            """
        )

    table_id_safe = re.sub(r"[^a-zA-Z0-9_-]", "", str(table_id)) or "tbl_conversao_regionais_ativados"

    return f"""
    <style>
        #{table_id_safe}.conversion-card {{
            background:#FFFFFF;
            border:1px solid rgba(31,41,55,.10);
            border-radius:15px;
            box-shadow:0 8px 22px rgba(31,41,55,.065);
        ...
        #{table_id_safe} .conversion-summary {{
            display:flex;
            flex-wrap:wrap;
            gap:8px;
            margin-bottom:9px;
        ...
        #{table_id_safe} .num {{ text-align:right; font-variant-numeric:tabular-nums; white-space:nowrap; }}
        #{table_id_safe} .text-strong {{ font-weight:850; color:#111827; }}
        #{table_id_safe} .conv-neg {{ color:#C62828; font-weight:900; }}
        #{table_id_safe} .conv-pos {{ color:#1B5E20; font-weight:900; }}
        #{table_id_safe} .conv-neu {{ color:#374151; font-weight:850; }}
        #{table_id_safe} .pill {{
            display:inline-flex;
            border-radius:999px;
            font-size:10.3px;
    ...
    <div id="{table_id_safe}" class="conversion-card">
        <div class="conversion-summary">
            <span class="summary-chip">Conversão média: {_formatar_pct_simples_ativados(conv_media)}</span>
            <span class="summary-chip">p.p. MoM médio: {_fmt_pp(pp_media)}</span>
            <span class="summary-chip">Linhas: {len(df)}</span>
        </div>
        <table>
            <thead>
                <tr>
                    ...
                    <th>Canal</th>
                    <th>Regional</th>
                    <th class="num">Venda Bruta</th>
                    <th class="num">Ativados</th>
                    <th class="num">Conversão</th>
                    <th class="num">p.p. MoM</th>
                </tr>
            </thead>
            <tbody>{"".join(linhas)}</tbody>
        </table>
...
def _montar_resumo_semanal_gap_ativados(serie_dia: pd.DataFrame) -> pd.DataFrame:
    if serie_dia is None or serie_dia.empty or "DATA_DIA" not in serie_dia.columns:
        return pd.DataFrame()
    df_sem = serie_dia.copy()
    df_sem["DATA_DIA"] = pd.to_datetime(df_sem["DATA_DIA"], errors="coerce")
    df_sem = df_sem.dropna(subset=["DATA_DIA"])
    if df_sem.empty:
        return pd.DataFrame()
    df_sem["SEMANA"] = ((df_sem["DATA_DIA"].dt.day - 1) // 7 + 1).clip(upper=5)
    df_sem["ORC_DIA"] = pd.to_numeric(df_sem["ORC_ACUM"], errors="coerce").fillna(0.0).diff().fillna(
        pd.to_numeric(df_sem["ORC_ACUM"], errors="coerce").fillna(0.0)
    )
    resumo = (
        df_sem.groupby("SEMANA", as_index=False, observed=True)[["QTDE", "ORC_DIA"]]
        .sum()
        .rename(columns={"QTDE": "REAL", "ORC_DIA": "ORC"})
    )
    resumo["GAP"] = resumo["REAL"] - resumo["ORC"]
    resumo["ATING"] = np.where(resumo["ORC"] > 0, resumo["REAL"] / resumo["ORC"] - 1.0, np.nan)
    resumo["SEMANA_LABEL"] = resumo["SEMANA"].apply(lambda v: f"S{int(v)}")
    return resumo


def _normalizar_canais_filtro_analise_ativados(canal_ref) -> list[str]:
    if canal_ref is None:
        return []
    if isinstance(canal_ref, (list, tuple, set)):
        return [str(canal).strip() for canal in canal_ref if str(canal).strip() and str(canal).strip() != "Todos"]
    canal_txt = str(canal_ref).strip()
    if not canal_txt or canal_txt == "Todos":
        return []
    return [canal_txt]


def _montar_dados_analise_ativados(
    df_ref: pd.DataFrame,
    mes_ref: str,
    modo_periodo: str = "MES",
    base_preparada: pd.DataFrame | None = None,
) -> dict[str, pd.DataFrame]:
    base = base_preparada if base_preparada is not None else _preparar_base_analise_ativados(df_ref)
    if base.empty:
        return {
            "base": base,
            "canal": pd.DataFrame(),
    ...
    modo_norm = str(modo_periodo or "MES").strip().upper()
    df_canal = _agregar_analise_ativados(base, mes_ref, ["PRODUTO", "CANAL_PLAN", "METRICA"], modo_norm)
    df_regional = _agregar_analise_ativados(base, mes_ref, ["PRODUTO", "CANAL_PLAN", "REGIONAL", "METRICA"], modo_norm)

    if df_regional.empty:
        df_conv = pd.DataFrame()
    else:
        conv = df_regional.pivot_table(
            index=["PRODUTO", "CANAL_PLAN", "REGIONAL"],
            columns="METRICA",
        ...
        for coluna in ["VENDA BRUTA", "ATIVADOS"]:
            if coluna not in conv.columns:
                conv[coluna] = 0.0
        conv["CONVERSAO"] = np.where(
            conv["VENDA BRUTA"] > 0,
            conv["ATIVADOS"] / conv["VENDA BRUTA"],
            np.nan,
        )

        mes_anterior = get_mes_anterior(mes_ref)
        reg_m1 = _agregar_analise_ativados(
            base,
            mes_anterior,
            ["PRODUTO", "CANAL_PLAN", "REGIONAL", "METRICA"],
            modo_norm,
        )
        if not reg_m1.empty:
            conv_m1 = reg_m1.pivot_table(
                index=["PRODUTO", "CANAL_PLAN", "REGIONAL"],
                columns="METRICA",
                values="VALOR_ANALISE",
                aggfunc="sum",
            ...
            for coluna in ["VENDA BRUTA", "ATIVADOS"]:
                if coluna not in conv_m1.columns:
                    conv_m1[coluna] = 0.0
            conv_m1["CONVERSAO_M1"] = np.where(
                conv_m1["VENDA BRUTA"] > 0,
                conv_m1["ATIVADOS"] / conv_m1["VENDA BRUTA"],
                np.nan,
            )
            conv = conv.merge(
                conv_m1[["PRODUTO", "CANAL_PLAN", "REGIONAL", "CONVERSAO_M1"]],
                on=["PRODUTO", "CANAL_PLAN", "REGIONAL"],
                how="left",
            )
        else:
            conv["CONVERSAO_M1"] = np.nan
        conv["PP_MOM"] = conv["CONVERSAO"] - conv["CONVERSAO_M1"]
        df_conv = conv
...
def _montar_serie_diaria_gap_ativados(
    base: pd.DataFrame,
    mes_ref: str,
    produto: str,
    canal: str,
    ...
    mes_norm = str(mes_ref or "").strip().lower()
    df_dia = base.loc[base["MES_ANO"].eq(mes_norm)].copy()
    if produto != "Todos":
        df_dia = df_dia.loc[df_dia["PRODUTO"].eq(produto)]
    canais_filtro = _normalizar_canais_filtro_analise_ativados(canal)
    if canais_filtro:
        df_dia = df_dia.loc[df_dia["CANAL_PLAN"].isin(canais_filtro)]
    if metrica != "Todos":
        df_dia = df_dia.loc[df_dia["METRICA"].eq(metrica)]
    if regional != "Todos":
        df_dia = df_dia.loc[df_dia["REGIONAL"].eq(regional)]
    if df_dia.empty:
        return pd.DataFrame()

    try:
        data_mes = mes_ano_para_data(mes_norm)
    except Exception:
        data_mes = pd.to_datetime(df_dia["DATA_DIA"], errors="coerce").dropna().min()
    if pd.isna(data_mes):
        return pd.DataFrame()

    inicio_mes = pd.Timestamp(data_mes.year, data_mes.month, 1)
    fim_mes = inicio_mes + pd.offsets.MonthEnd(0)
    dias_mes = pd.date_range(inicio_mes, fim_mes, freq="D")

    real_dia = (
        df_dia.dropna(subset=["DATA_DIA"])
        .groupby("DATA_DIA", as_index=False, observed=True)["QTDE"]
        .sum()
    )
    real_dia = real_dia.set_index("DATA_DIA").reindex(dias_mes, fill_value=0.0).rename_axis("DATA_DIA").reset_index()
    orc_mes = float(pd.to_numeric(df_dia["DESAFIO_QTD"], errors="coerce").fillna(0.0).sum())
    total_dias = max(len(dias_mes), 1)
    real_dia["REAL_ACUM"] = real_dia["QTDE"].cumsum()
    real_dia["ORC_ACUM"] = [(orc_mes / total_dias) * (idx + 1) for idx in range(total_dias)]
    real_dia["GAP_ACUM"] = real_dia["REAL_ACUM"] - real_dia["ORC_ACUM"]
    return real_dia


def _montar_serie_ytd_gap_ativados(
    base: pd.DataFrame,
    mes_ref: str,
    produto: str,
    canal: str,
    ...
    meses_periodo = _obter_meses_periodo_analise_ativados(base, mes_ref, "YTD")
    if not meses_periodo:
        return pd.DataFrame()

    df_mes = base.loc[base["MES_ANO"].isin(meses_periodo)].copy()
    if produto != "Todos":
        df_mes = df_mes.loc[df_mes["PRODUTO"].eq(produto)]
    canais_filtro = _normalizar_canais_filtro_analise_ativados(canal)
    if canais_filtro:
        df_mes = df_mes.loc[df_mes["CANAL_PLAN"].isin(canais_filtro)]
    if metrica != "Todos":
        df_mes = df_mes.loc[df_mes["METRICA"].eq(metrica)]
    if regional != "Todos":
        df_mes = df_mes.loc[df_mes["REGIONAL"].eq(regional)]
    if df_mes.empty:
        return pd.DataFrame()

    agg_mes = (
        df_mes.groupby("MES_ANO", as_index=False, observed=True)[["QTDE", "TEND_QTD", "DESAFIO_QTD"]]
        .sum()
    )
    mes_corrente = get_mes_atual_formatado().strip().lower()
    usar_tend_mes_ref = eh_mes_atual_dashboard(str(mes_ref or "").strip().lower(), mes_corrente)
    ...
    agg_mes["REAL"] = np.where(mask_tend, agg_mes["TEND_QTD"], agg_mes["QTDE"])
    agg_mes["ORC"] = agg_mes["DESAFIO_QTD"]
    agg_mes["DATA_MES"] = agg_mes["MES_ANO"].apply(lambda mes: mes_ano_para_data(mes))
    agg_mes = agg_mes.sort_values("DATA_MES")
    agg_mes["REAL_ACUM"] = agg_mes["REAL"].cumsum()
    agg_mes["ORC_ACUM"] = agg_mes["ORC"].cumsum()
    agg_mes["GAP_ACUM"] = agg_mes["REAL_ACUM"] - agg_mes["ORC_ACUM"]
    agg_mes["ATING"] = np.where(agg_mes["ORC"] > 0, agg_mes["REAL"] / agg_mes["ORC"] - 1.0, np.nan)
    return agg_mes


def render_bloco_analises_detalhadas_ativados(
    df_ref: pd.DataFrame,
    mes_ref: str,
    mes_anterior_ref: str | None = None,
) -> None:
    ...
    st.markdown(
        build_visual_title_html(
            "ANÁLISES DETALHADAS",
            "search",
            subtitle="DIAGNÓSTICO DE GAPS QUE IMPACTAM ORÇAMENTO, CONVERSÃO E RITMO DIÁRIO",
            extra_style="margin-top:18px;"
        ),
        unsafe_allow_html=True
    )

    chave_estado = "ativados_analises_detalhadas_aberta"
    col_botao, col_hint = st.columns([1.1, 3.9], gap="medium")
    with col_botao:
        if st.button("🔎 Ver análises detalhadas", key="btn_ativados_analises_detalhadas"):
            st.session_state[chave_estado] = not bool(st.session_state.get(chave_estado, False))
    with col_hint:
        st.markdown(
            """
            <div style="margin-top:3px;padding:9px 12px;border-radius:12px;background:#F8FAFC;border:1px solid #E5E7EB;color:#4B5563;font-size:12px;font-weight:650;">
                Abra somente quando quiser investigar os ofensores de orçamento. O bloco cruza tendência/real, orçamento, regional, canal, produto e conversão.
            </div>
            """,
            unsafe_allow_html=True
        )

    if not st.session_state.get(chave_estado, False):
        return

    meses_analise = []
    if "dat_tratada" in df_ref.columns:
        for mes_item in df_ref["dat_tratada"].dropna().astype(str).str.strip().str.lower().unique():
            try:
                meses_analise.append((mes_item, mes_ano_para_data(mes_item)))
            except Exception:
                continue
    meses_analise = [mes for mes, _ in sorted(meses_analise, key=lambda item: item[1])]
    if not meses_analise:
        meses_analise = [str(mes_ref or "").strip().lower()]

    mes_ref_norm = str(mes_ref or "").strip().lower()
    idx_mes_analise = meses_analise.index(mes_ref_norm) if mes_ref_norm in meses_analise else max(len(meses_analise) - 1, 0)

    col_periodo1, col_periodo2, col_periodo3 = st.columns([1.05, 1.05, 2.9], gap="medium")
    with col_periodo1:
        render_filter_label("MÊS DA ANÁLISE")
        mes_analise_sel = st.selectbox(
            "Mês da análise detalhada",
            options=meses_analise,
            index=idx_mes_analise,
            key="ativados_analise_mes_ref",
            label_visibility="collapsed"
        )
    with col_periodo2:
        render_filter_label("VISÃO")
        visao_periodo_sel = st.selectbox(
            "Tipo de visão da análise detalhada",
            options=["Mês", "YTD"],
            index=0,
            key="ativados_analise_visao_periodo",
            label_visibility="collapsed"
        )
    modo_periodo = "YTD" if visao_periodo_sel == "YTD" else "MES"

    base_previa = _preparar_base_analise_ativados(df_ref)
    meses_periodo = _obter_meses_periodo_analise_ativados(base_previa, mes_analise_sel, modo_periodo)
    periodo_label = _descrever_periodo_analise_ativados(meses_periodo, modo_periodo)
    with col_periodo3:
        st.markdown(
            f"""
            <div style="margin-top:22px;padding:9px 13px;border-radius:12px;background:#FFFFFF;border:1px solid #E5E7EB;color:#374151;font-size:12px;font-weight:750;">
                Período analisado: <span style="color:#790E09;font-weight:850;">{escape(periodo_label)}</span>
                <span style="color:#6B7280;font-weight:650;"> | Gap principal sempre = Ativados/Instalação vs ORC</span>
            </div>
            """,
            unsafe_allow_html=True
        )

    dados = _montar_dados_analise_ativados(
        df_ref,
        mes_analise_sel,
        modo_periodo,
        base_preparada=base_previa,
    )
    base = dados.get("base", pd.DataFrame())
    df_canal = dados.get("canal", pd.DataFrame())
    df_regional = dados.get("regional", pd.DataFrame())
    df_conv = dados.get("conversao", pd.DataFrame())

    if base.empty or df_canal.empty:
        st.info("Sem dados suficientes para montar as análises detalhadas com os filtros atuais.")
        return

    produtos = ["Todos"] + [p for p in ["CONTA", "FIXA"] if p in set(base["PRODUTO"].dropna().unique())]
    canais_base = sorted([c for c in base["CANAL_PLAN"].dropna().astype(str).unique() if c.strip()])
    metricas = ["Todos", "DEMANDA", "VENDA BRUTA", "ATIVADOS"]
    regionais = ["Todos"] + sorted([r for r in base["REGIONAL"].dropna().astype(str).unique() if r.strip()])

    col_f1, col_f2, col_f3, col_f4 = st.columns([0.9, 1.7, 0.9, 0.9], gap="medium")
    with col_f1:
        render_filter_label("PRODUTO")
        produto_sel = st.selectbox(
            "Produto da análise detalhada",
            produtos,
            index=0,
            key="ativados_analise_produto",
            label_visibility="collapsed"
        )
    with col_f2:
        render_filter_label("CANAIS")
        if "ativados_analise_canal" in st.session_state and not isinstance(st.session_state.get("ativados_analise_canal"), list):
            del st.session_state["ativados_analise_canal"]
        canais_sel_raw = st.multiselect(
            "Canais da análise detalhada",
            options=canais_base,
            default=[],
            placeholder="Todos os canais",
            key="ativados_analise_canal",
            label_visibility="collapsed"
        )
        canais_sel = [str(canal).strip() for canal in canais_sel_raw if str(canal).strip()]
        canal_sel = "Todos" if not canais_sel or set(canais_sel) == set(canais_base) else tuple(canais_sel)
    with col_f3:
        render_filter_label("MÉTRICA")
        metrica_sel = st.selectbox(
            "Métrica da análise detalhada",
            metricas,
            index=0,
            key="ativados_analise_metrica",
            label_visibility="collapsed"
        )
    with col_f4:
        render_filter_label("REGIONAL")
        regional_sel = st.selectbox(
            "Regional da análise detalhada",
            regionais,
            index=0,
            key="ativados_analise_regional",
            label_visibility="collapsed"
        )

    def _filtrar(df_filtro: pd.DataFrame) -> pd.DataFrame:
        df_tmp = df_filtro.copy()
        if produto_sel != "Todos" and "PRODUTO" in df_tmp.columns:
            df_tmp = df_tmp.loc[df_tmp["PRODUTO"].eq(produto_sel)]
        canais_filtro = _normalizar_canais_filtro_analise_ativados(canal_sel)
        if canais_filtro and "CANAL_PLAN" in df_tmp.columns:
            df_tmp = df_tmp.loc[df_tmp["CANAL_PLAN"].isin(canais_filtro)]
        if metrica_sel != "Todos" and "METRICA" in df_tmp.columns:
            df_tmp = df_tmp.loc[df_tmp["METRICA"].eq(metrica_sel)]
        if regional_sel != "Todos" and "REGIONAL" in df_tmp.columns:
            df_tmp = df_tmp.loc[df_tmp["REGIONAL"].eq(regional_sel)]
        return df_tmp

    canal_filtrado = _filtrar(df_canal)
    regional_filtrado = _filtrar(df_regional)
    conv_filtrado = _filtrar(df_conv)

    def _filtrar_dimensoes_sem_metrica(df_filtro: pd.DataFrame) -> pd.DataFrame:
        df_tmp = df_filtro.copy()
        if produto_sel != "Todos" and "PRODUTO" in df_tmp.columns:
            df_tmp = df_tmp.loc[df_tmp["PRODUTO"].eq(produto_sel)]
        canais_filtro = _normalizar_canais_filtro_analise_ativados(canal_sel)
    ...
    # Gap executivo sempre mede apenas o indicador final: Gross Liquido (Conta) ou Instalacao (Fixa).
    # Demanda e Venda Bruta entram como diagnostico de causa, nunca na soma do gap principal.
    canal_dimensoes = _filtrar_dimensoes_sem_metrica(df_canal)
    regional_dimensoes = _filtrar_dimensoes_sem_metrica(df_regional)
    canal_gap_principal = canal_dimensoes.loc[canal_dimensoes["METRICA"].eq("ATIVADOS")].copy()
    regional_gap_principal = regional_dimensoes.loc[regional_dimensoes["METRICA"].eq("ATIVADOS")].copy()
    canal_gap_principal = canal_gap_principal.loc[pd.to_numeric(canal_gap_principal["ORC"], errors="coerce").fillna(0.0) > 0].copy()
    regional_gap_principal = regional_gap_principal.loc[pd.to_numeric(regional_gap_principal["ORC"], errors="coerce").fillna(0.0) > 0].copy()

    canal_ofensores = canal_filtrado.loc[
        pd.to_numeric(canal_filtrado["ORC"], errors="coerce").fillna(0.0) > 0
    ].copy() if not canal_filtrado.empty else pd.DataFrame()
    regional_ofensores = regional_filtrado.loc[
        pd.to_numeric(regional_filtrado["ORC"], errors="coerce").fillna(0.0) > 0
    ].copy() if not regional_filtrado.empty else pd.DataFrame()
    titulo_metrica_ofensores = "MÉTRICAS" if metrica_sel == "Todos" else str(metrica_sel)

    gap_total = float(pd.to_numeric(canal_gap_principal["GAP"], errors="coerce").fillna(0.0).sum()) if not canal_gap_principal.empty else 0.0
    orc_total = float(pd.to_numeric(canal_gap_principal["ORC"], errors="coerce").fillna(0.0).sum()) if not canal_gap_principal.empty else 0.0
    valor_total = float(pd.to_numeric(canal_gap_principal["VALOR_ANALISE"], errors="coerce").fillna(0.0).sum()) if not canal_gap_principal.empty else 0.0
    ating_total = (valor_total / orc_total - 1.0) if orc_total > 0 else np.nan
    fonte_principal = (
        "YTD+TEND"
        if (not canal_gap_principal.empty and canal_gap_principal["FONTE"].astype(str).str.upper().str.contains("TEND", na=False).any() and modo_periodo == "YTD")
        else "TEND"
        if (not canal_gap_principal.empty and canal_gap_principal["FONTE"].astype(str).str.upper().eq("TEND").any())
        else "YTD REAL"
        if modo_periodo == "YTD"
        else "REAL"
    )
    ...
    pior_canal = "-"
    pior_regional = "-"
    pior_canal_ofensores = "-"
    pior_regional_ofensores = "-"
    conv_media = np.nan
    if not canal_gap_principal.empty:
        row_pior = canal_gap_principal.sort_values("GAP", ascending=True).iloc[0]
        pior_canal = f"{row_pior.get('CANAL_PLAN', '-')}/{row_pior.get('METRICA', '-')}: {formatar_numero_brasileiro(row_pior.get('GAP', 0), 0)}"
    if not regional_gap_principal.empty:
        row_pior_reg = regional_gap_principal.sort_values("GAP", ascending=True).iloc[0]
        pior_regional = f"{row_pior_reg.get('REGIONAL', '-')}/{row_pior_reg.get('CANAL_PLAN', '-')}: {formatar_numero_brasileiro(row_pior_reg.get('GAP', 0), 0)}"
    if not canal_ofensores.empty:
        row_pior_ofensor = canal_ofensores.sort_values("GAP", ascending=True).iloc[0]
        pior_canal_ofensores = f"{row_pior_ofensor.get('CANAL_PLAN', '-')}/{row_pior_ofensor.get('METRICA', '-')}: {formatar_numero_brasileiro(row_pior_ofensor.get('GAP', 0), 0)}"
    if not regional_ofensores.empty:
        row_pior_reg_ofensor = regional_ofensores.sort_values("GAP", ascending=True).iloc[0]
        pior_regional_ofensores = f"{row_pior_reg_ofensor.get('REGIONAL', '-')}/{row_pior_reg_ofensor.get('CANAL_PLAN', '-')}: {formatar_numero_brasileiro(row_pior_reg_ofensor.get('GAP', 0), 0)}"
    if not conv_filtrado.empty:
        conv_media = pd.to_numeric(conv_filtrado["CONVERSAO"], errors="coerce").replace([np.inf, -np.inf], np.nan).mean()

    insights = [f"Período | {periodo_label}"]
    if gap_total < 0:
        insights.append(
            f"Gap principal | {formatar_numero_brasileiro(gap_total, 0)} vs ORC ({_formatar_pct_delta_ativados(ating_total)})"
        )
    elif gap_total > 0:
        insights.append(
            f"Gap principal | +{formatar_numero_brasileiro(gap_total, 0)} vs ORC ({_formatar_pct_delta_ativados(ating_total)})"
        ...
        insights.append("Gap principal | Sem diferença relevante vs ORC")
    if pior_canal != "-":
        insights.append(f"Canal crítico | {pior_canal}")
    if pior_regional != "-":
        insights.append(f"Regional crítica | {pior_regional}")
    if pd.notna(conv_media):
        insights.append(f"Conversão | Média {_formatar_pct_simples_ativados(conv_media)}")

    _render_pontos_atencao_ativados(insights)

    st.markdown(
        """
        <div style="margin:-8px 0 12px 0;color:#6B7280;font-size:11.5px;font-weight:650;">
            Gap principal considera somente Ativados/Instalação vs ORC. Demanda e Venda Bruta aparecem como pistas de causa, não entram na soma.
        </div>
        """,
        unsafe_allow_html=True
    )

    aba_canal, aba_regional, aba_conversao, aba_ritmo = st.tabs([
        "1. Canal",
        "2. Regional",
        "3. Conversão",
        "4. Ritmo diário",
    ...
    with aba_canal:
        ...
        with col_k1:
            st.markdown(
                _formatar_card_gap_ativados(
                    "Gap vs ORC",
                    formatar_numero_brasileiro(gap_total, 0),
                    f"Atingimento: {_formatar_pct_delta_ativados(ating_total)}",
                    "negativo" if gap_total < 0 else "positivo"
                ),
                unsafe_allow_html=True
            )
        with col_k2:
            st.markdown(
                _formatar_card_gap_ativados(
                    fonte_principal,
                    formatar_numero_brasileiro(valor_total, 0),
                    "Valor usado no gap principal",
                    "neutro"
                ),
                unsafe_allow_html=True
            )
        with col_k3:
            st.markdown(
                _formatar_card_gap_ativados(
                    "ORC",
                    formatar_numero_brasileiro(orc_total, 0),
                    "Meta mensal do recorte selecionado",
                    "neutro"
                ),
                unsafe_allow_html=True
            )
        with col_k4:
            st.markdown(
                _formatar_card_gap_ativados(
                    "Principal ofensor",
                    pior_canal_ofensores,
                    "Maior gap dentro dos filtros selecionados",
                    "alerta"
                ),
                unsafe_allow_html=True
            )
        ...
        st.markdown(
            build_visual_title_html(f"GAP DE {titulo_metrica_ofensores} POR CANAL", "bar-chart", "subsection-title"),
            unsafe_allow_html=True
        )
        top_gap = canal_ofensores.copy()
        if not top_gap.empty:
            top_gap["LABEL"] = top_gap["PRODUTO"].astype(str) + " | " + top_gap["CANAL_PLAN"].astype(str) + " | " + top_gap["METRICA"].astype(str)
            top_gap = top_gap.sort_values("GAP", ascending=True).head(15)
            cores = ["#C62828" if valor < 0 else "#1B5E20" for valor in top_gap["GAP"]]
            fig_gap = go.Figure()
            fig_gap.add_trace(go.Bar(
                ...
                text=[formatar_numero_brasileiro(v, 0) for v in top_gap["GAP"]],
                textposition="outside",
                hovertemplate="<b>%{y}</b><br>Gap: %{x:,.0f}<extra></extra>",
            ))
            fig_gap.update_layout(
                height=430,
                margin=dict(l=8, r=26, t=10, b=16),
                paper_bgcolor="#FFFFFF",
                plot_bgcolor="#FFFFFF",
                xaxis=dict(title="", zeroline=True, zerolinecolor="#790E09", gridcolor="#EEF2F7"),
                yaxis=dict(title="", automargin=True),
                showlegend=False,
                font=dict(family="Manrope, Segoe UI, sans-serif", size=11, color="#1F2937"),
            )
            st.plotly_chart(fig_gap, width="stretch", config={"displayModeBar": False, "displaylogo": False})

            tabela_canal = top_gap.sort_values("GAP", ascending=True).copy()
            renderizar_html_tabela_dashboard(
                _criar_tabela_html_ofensores_canais_ativados(
                    tabela_canal,
                    table_id="tbl_ofensores_canais_ativados",
                ),
                height=320,
            )

            heat_canal = canal_dimensoes.pivot_table(
                index="CANAL_PLAN",
                columns="METRICA",
                values="ATING_PCT",
                aggfunc="mean",
            ...
            if not heat_canal.empty:
                heat_canal = heat_canal.reindex(columns=[c for c in ["DEMANDA", "VENDA BRUTA", "ATIVADOS"] if c in heat_canal.columns])
                z_heat = (heat_canal * 100.0).round(1)
                texto_heat = _map_dataframe_compat(z_heat, lambda v: "-" if pd.isna(v) else f"{v:+.1f}%".replace(".", ","))
                fig_heat_canal = go.Figure(data=go.Heatmap(
                    z=z_heat.values,
                    x=list(z_heat.columns),
                    y=list(z_heat.index),
                    colorscale=[
                        [0.0, "#C62828"],
                        [0.5, "#F8FAFC"],
                    ...
                    zmin=-100,
                    zmax=100,
                    text=texto_heat.values,
                    texttemplate="%{text}",
                    hovertemplate="<b>%{y}</b><br>%{x}: %{text}<extra></extra>",
                    colorbar=dict(title="vs ORC"),
                ))
                fig_heat_canal.update_layout(
                    title=dict(text="Mapa de calor: atingimento vs ORC (sem ORC fica em branco)", font=dict(size=13, color="#374151")),
                    height=300,
                ...
                st.plotly_chart(fig_heat_canal, width="stretch", config={"displayModeBar": False, "displaylogo": False})
        else:
            st.info("Sem gaps no recorte selecionado.")

    with aba_regional:
        gap_regional_total = (
            float(pd.to_numeric(regional_ofensores["GAP"], errors="coerce").fillna(0.0).sum())
            if not regional_ofensores.empty else 0.0
        )
        regionais_negativas = 0
        maior_recuperacao = "-"
        if not regional_ofensores.empty:
            regionais_negativas = int(regional_ofensores.loc[regional_ofensores["GAP"] < 0, "REGIONAL"].nunique())
            row_melhor_reg = regional_ofensores.sort_values("GAP", ascending=False).iloc[0]
            maior_recuperacao = f"{row_melhor_reg.get('REGIONAL', '-')}/{row_melhor_reg.get('CANAL_PLAN', '-')}: {formatar_numero_brasileiro(row_melhor_reg.get('GAP', 0), 0)}"
        ...
        with col_r1:
            st.markdown(
                _formatar_card_gap_ativados(
                    "Gap regional",
                    formatar_numero_brasileiro(gap_regional_total, 0),
                    "Soma dos gaps regionais no recorte",
                    "negativo" if gap_regional_total < 0 else "positivo"
                ),
                unsafe_allow_html=True
            )
        with col_r2:
        ...
        with col_r3:
            st.markdown(
                _formatar_card_gap_ativados(
                    "Regionais abaixo",
                    str(regionais_negativas),
                    f"Melhor compensação: {maior_recuperacao}",
                    "neutro"
                ),
                unsafe_allow_html=True
            )
        ...
        st.markdown(
            build_visual_title_html(f"OFENSORES DE {titulo_metrica_ofensores} POR REGIONAL", "table", "subsection-title"),
            unsafe_allow_html=True
        )
        if not regional_ofensores.empty:
            top_reg = regional_ofensores.sort_values("GAP", ascending=True).copy()
            top_reg["LABEL"] = (
                top_reg["PRODUTO"].astype(str)
                + " | "
                + top_reg["REGIONAL"].astype(str)
                + " | "
                + top_reg["CANAL_PLAN"].astype(str)
                + " | "
                + top_reg["METRICA"].astype(str)
            )
            fig_reg = go.Figure()
            fig_reg.add_trace(go.Bar(
                ...
                y=top_reg["LABEL"],
                orientation="h",
                marker=dict(color=["#C62828" if valor < 0 else "#1B5E20" for valor in top_reg["GAP"]]),
                text=[formatar_numero_brasileiro(v, 0) for v in top_reg["GAP"]],
                textposition="outside",
                hovertemplate="<b>%{y}</b><br>Gap: %{x:,.0f}<extra></extra>",
            ))
            altura_ofensores_reg = max(440, min(2600, 28 * len(top_reg) + 90))
            fig_reg.update_layout(
                height=altura_ofensores_reg,
                margin=dict(l=8, r=26, t=10, b=16),
                paper_bgcolor="#FFFFFF",
                plot_bgcolor="#FFFFFF",
                xaxis=dict(title="", zeroline=True, zerolinecolor="#790E09", gridcolor="#EEF2F7"),
            ...
            if len(top_reg) > 18:
                with st.container(height=560):
                    st.plotly_chart(fig_reg, width="stretch", config={"displayModeBar": False, "displaylogo": False})
            else:
                st.plotly_chart(fig_reg, width="stretch", config={"displayModeBar": False, "displaylogo": False})

            tabela = regional_ofensores.sort_values("GAP", ascending=True).copy()
            renderizar_html_tabela_dashboard(
                _criar_tabela_html_ofensores_regionais_ativados(
                    tabela,
                    table_id="tbl_ofensores_regionais_ativados",
                ),
                height=520,
            )

            heat_base_regional = regional_dimensoes.copy()
            heat_ordem_base = regional_ofensores.copy()
            canais_heat_options = sorted([
                canal for canal in heat_base_regional.get("CANAL_PLAN", pd.Series(dtype=object)).dropna().astype(str).str.strip().unique()
                if canal
            ])
            canais_heat_sel = []
            if len(canais_heat_options) > 1:
                col_heat_canal, col_heat_hint = st.columns([1.35, 3.65], gap="medium")
                with col_heat_canal:
                    render_filter_label("CANAIS DO MAPA")
                    if "ativados_analise_heatmap_canais" in st.session_state and not isinstance(st.session_state.get("ativados_analise_heatmap_canais"), list):
                        del st.session_state["ativados_analise_heatmap_canais"]
                    canais_heat_sel = st.multiselect(
                        "Canais do mapa de calor",
                        options=canais_heat_options,
                        default=[],
                        placeholder="Todos do recorte",
                        key="ativados_analise_heatmap_canais",
                        label_visibility="collapsed"
                    )
                with col_heat_hint:
                    st.markdown(
                        """
                        <div style="margin-top:22px;padding:8px 12px;border-radius:11px;background:#F8FAFC;border:1px solid #E5E7EB;color:#6B7280;font-size:11.5px;font-weight:650;">
                            Este filtro altera somente o mapa de calor abaixo. Os gráficos e tabelas de ofensores continuam seguindo os filtros principais.
                        </div>
                        """,
                        unsafe_allow_html=True
                    )

            canais_heat_sel = [str(canal).strip() for canal in canais_heat_sel if str(canal).strip()]
            if canais_heat_sel:
                heat_base_regional = heat_base_regional.loc[heat_base_regional["CANAL_PLAN"].isin(canais_heat_sel)].copy()
                heat_ordem_base = heat_ordem_base.loc[heat_ordem_base["CANAL_PLAN"].isin(canais_heat_sel)].copy()

            regional_dimensoes_com_orc = heat_base_regional.loc[
                pd.to_numeric(heat_base_regional["ORC"], errors="coerce").fillna(0.0) > 0
            ].copy()
            heat_reg = regional_dimensoes_com_orc.pivot_table(
                index="REGIONAL",
                columns="METRICA",
                values="ATING_PCT",
                aggfunc="mean",
            ...
            if not heat_reg.empty:
                if heat_ordem_base.empty:
                    heat_ordem_base = regional_dimensoes_com_orc.copy()
                ordem_reg = (
                    heat_ordem_base.groupby("REGIONAL", observed=True)["GAP"]
                    .sum()
                    .sort_values(ascending=True)
                    .head(14)
                    .index
                )
                heat_reg = heat_reg.loc[[r for r in ordem_reg if r in heat_reg.index]]
                heat_reg = heat_reg.reindex(columns=[c for c in ["DEMANDA", "VENDA BRUTA", "ATIVADOS"] if c in heat_reg.columns])
                z_reg = (heat_reg * 100.0).round(1)
                texto_reg = _map_dataframe_compat(z_reg, lambda v: "-" if pd.isna(v) else f"{v:+.1f}%".replace(".", ","))
                fig_heat_reg = go.Figure(data=go.Heatmap(
                    z=z_reg.values,
                    x=list(z_reg.columns),
                    y=list(z_reg.index),
                    colorscale=[
                        [0.0, "#C62828"],
                        [0.5, "#F8FAFC"],
                    ...
                    zmin=-100,
                    zmax=100,
                    text=texto_reg.values,
                    texttemplate="%{text}",
                    hovertemplate="<b>%{y}</b><br>%{x}: %{text}<extra></extra>",
                    colorbar=dict(title="vs ORC"),
                ))
                fig_heat_reg.update_layout(
                    title=dict(text="Mapa de calor: atingimento vs ORC por regional", font=dict(size=13, color="#374151")),
                    height=330,
                    margin=dict(l=8, r=8, t=38, b=10),
                    paper_bgcolor="#FFFFFF",
                    plot_bgcolor="#FFFFFF",
                ...
                st.plotly_chart(fig_heat_reg, width="stretch", config={"displayModeBar": False, "displaylogo": False})
        else:
            st.info("Sem dados regionais no recorte selecionado.")

    with aba_conversao:
        col_c1, col_c2 = st.columns([1, 3], gap="medium")
        with col_c1:
            st.markdown(
                _formatar_card_gap_ativados(
                    "Conversão média",
                    _formatar_pct_simples_ativados(conv_media),
                    "Ativados / Venda Bruta",
                    "neutro"
                ),
                unsafe_allow_html=True
            )
        with col_c2:
            st.markdown(
                """
                <div style="margin-top:3px;padding:12px 14px;border-radius:12px;background:#FFFFFF;border:1px solid #E5E7EB;color:#4B5563;font-size:12px;font-weight:650;">
                    Use esta visão quando o volume de venda bruta parece saudável, mas a ativação/instalação não acompanha. As regionais no topo do ranking são as que mais merecem investigação operacional.
                </div>
                """,
                unsafe_allow_html=True
            )
        ...
        st.markdown(
            build_visual_title_html("CONVERSÃO POR REGIONAL", "target", "subsection-title"),
            unsafe_allow_html=True
        )
        conv_plot = conv_filtrado.dropna(subset=["CONVERSAO"]).copy()
        if not conv_plot.empty:
            conv_plot["LABEL"] = conv_plot["PRODUTO"].astype(str) + " | " + conv_plot["CANAL_PLAN"].astype(str) + " | " + conv_plot["REGIONAL"].astype(str)
            conv_plot = conv_plot.sort_values("CONVERSAO", ascending=True).head(15)
            fig_conv = go.Figure()
            fig_conv.add_trace(go.Bar(
                x=conv_plot["CONVERSAO"] * 100.0,
                y=conv_plot["LABEL"],
                orientation="h",
                marker=dict(color="#790E09"),
                text=[f"{v * 100:.1f}%".replace(".", ",") for v in conv_plot["CONVERSAO"]],
                textposition="outside",
                hovertemplate="<b>%{y}</b><br>Conversão: %{x:.1f}%<extra></extra>",
            ))
            fig_conv.update_layout(
                ...
                paper_bgcolor="#FFFFFF",
                plot_bgcolor="#FFFFFF",
                xaxis=dict(title="", ticksuffix="%", gridcolor="#EEF2F7"),
                yaxis=dict(title="", automargin=True),
                showlegend=False,
                font=dict(family="Manrope, Segoe UI, sans-serif", size=11, color="#1F2937"),
            )
            st.plotly_chart(fig_conv, width="stretch", config={"displayModeBar": False, "displaylogo": False})

            conv_scatter = conv_filtrado.dropna(subset=["CONVERSAO"]).copy()
            conv_scatter = conv_scatter.loc[pd.to_numeric(conv_scatter["VENDA BRUTA"], errors="coerce").fillna(0.0) > 0]
            if not conv_scatter.empty:
                fig_scatter = go.Figure()
                paleta_conversao = ["#790E09", "#D12405", "#A23B36", "#5A6268", "#AEAFAF", "#1B5E20"]
                for idx_prod, (produto_nome, df_prod) in enumerate(conv_scatter.groupby("PRODUTO", observed=True)):
                    fig_scatter.add_trace(go.Scatter(
                        x=df_prod["VENDA BRUTA"],
                        y=df_prod["CONVERSAO"] * 100.0,
                        mode="markers",
                        name=str(produto_nome),
                        marker=dict(
                            size=np.clip(pd.to_numeric(df_prod["ATIVADOS"], errors="coerce").fillna(0.0) / 80.0, 8, 28),
                            color=paleta_conversao[idx_prod % len(paleta_conversao)],
                            opacity=0.78,
                            line=dict(color="#FFFFFF", width=1),
                        ),
                        text=df_prod["REGIONAL"].astype(str) + " | " + df_prod["CANAL_PLAN"].astype(str),
                        hovertemplate="<b>%{text}</b><br>Venda Bruta: %{x:,.0f}<br>Conversão: %{y:.1f}%<extra></extra>",
                    ))
                fig_scatter.update_layout(
                    title=dict(text="Volume vendido x eficiência de ativação", font=dict(size=13, color="#374151")),
                    height=340,
                    margin=dict(l=8, r=18, t=42, b=24),
                    paper_bgcolor="#FFFFFF",
                    plot_bgcolor="#FFFFFF",
                    xaxis=dict(title="Venda Bruta", gridcolor="#EEF2F7"),
                    yaxis=dict(title="Conversão", ticksuffix="%", gridcolor="#EEF2F7"),
                    legend=dict(orientation="h", y=1.12, x=0, bgcolor="rgba(255,255,255,0)"),
                    font=dict(family="Manrope, Segoe UI, sans-serif", size=11, color="#1F2937"),
                )
                st.plotly_chart(fig_scatter, width="stretch", config={"displayModeBar": False, "displaylogo": False})

            tabela_conv = conv_plot.copy()
            renderizar_html_tabela_dashboard(
                _criar_tabela_html_conversao_ativados(
                    tabela_conv,
                    table_id="tbl_conversao_regionais_ativados",
                ),
                height=340,
            )
        else:
            st.info("Sem dados de conversão para o recorte selecionado.")

    with aba_ritmo:
        titulo_ritmo = (
            "RITMO MENSAL YTD ACUMULADO VS ORÇAMENTO"
            if modo_periodo == "YTD"
            else "RITMO DIÁRIO ACUMULADO VS ORÇAMENTO"
        )
        st.markdown(
            build_visual_title_html(titulo_ritmo, "calendar", "subsection-title"),
            unsafe_allow_html=True
        )
        metrica_diaria = "ATIVADOS" if metrica_sel == "Todos" else metrica_sel
        if modo_periodo == "YTD":
            serie_ytd = _montar_serie_ytd_gap_ativados(
                base=base,
                mes_ref=mes_analise_sel,
                produto=produto_sel,
                canal=canal_sel,
                metrica=metrica_diaria,
                regional=regional_sel,
            )
            if not serie_ytd.empty:
                fig_ytd = go.Figure()
                fig_ytd.add_trace(go.Scatter(
                    x=serie_ytd["DATA_MES"],
                    y=serie_ytd["REAL_ACUM"],
                    mode="lines+markers",
                    name="Real/Tend acumulado",
                    line=dict(color="#790E09", width=3),
                ))
                fig_ytd.add_trace(go.Scatter(
                    x=serie_ytd["DATA_MES"],
                    y=serie_ytd["ORC_ACUM"],
                    mode="lines+markers",
                    name="ORC acumulado",
                ...
                fig_ytd.update_layout(
                    ...
                    paper_bgcolor="#FFFFFF",
                    plot_bgcolor="#FFFFFF",
                    xaxis=dict(title="", tickformat="%b/%y", gridcolor="#EEF2F7"),
                    yaxis=dict(title="", gridcolor="#EEF2F7"),
                    legend=dict(orientation="h", y=1.08, x=0, bgcolor="rgba(255,255,255,0)"),
                    font=dict(family="Manrope, Segoe UI, sans-serif", size=11, color="#1F2937"),
                )
                st.plotly_chart(fig_ytd, width="stretch", config={"displayModeBar": False, "displaylogo": False})

                ultimo_mes_ytd = serie_ytd.tail(1)
                if not ultimo_mes_ytd.empty:
                    gap_acum = float(ultimo_mes_ytd["GAP_ACUM"].iloc[0])
                    st.markdown(
                        _formatar_card_gap_ativados(
                            "Leitura YTD",
                            f"{formatar_numero_brasileiro(gap_acum, 0)}",
                            f"Gap acumulado de {metrica_diaria.lower()} no {periodo_label}.",
                            "negativo" if gap_acum < 0 else "positivo"
                        ),
                        unsafe_allow_html=True
                    )

                tabela_ytd = pd.DataFrame({
                    "Mês": serie_ytd["MES_ANO"].astype(str).str.upper(),
                    "Real/Tend": serie_ytd["REAL"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                    "ORC": serie_ytd["ORC"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                    "Gap": (serie_ytd["REAL"] - serie_ytd["ORC"]).apply(lambda v: formatar_numero_brasileiro(v, 0)),
                    "Ating.": serie_ytd["ATING"].apply(_formatar_pct_delta_ativados),
                    "Gap Acum.": serie_ytd["GAP_ACUM"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                })
                st.dataframe(tabela_ytd, use_container_width=True, hide_index=True, height=260)
            else:
                st.info("Sem série YTD disponível para o recorte selecionado.")
        else:
            serie_dia = _montar_serie_diaria_gap_ativados(
                base=base,
                mes_ref=mes_analise_sel,
                produto=produto_sel,
                canal=canal_sel,
            ...
            if not serie_dia.empty:
                ...
                fig_dia.add_trace(go.Scatter(
                    x=serie_dia["DATA_DIA"],
                    y=serie_dia["REAL_ACUM"],
                    mode="lines+markers",
                    name="Real acumulado",
                    line=dict(color="#790E09", width=3),
                ))
                ...
                st.plotly_chart(fig_dia, width="stretch", config={"displayModeBar": False, "displaylogo": False})

                ultimo_dia = serie_dia.dropna(subset=["DATA_DIA"]).tail(1)
                if not ultimo_dia.empty:
                    gap_acum = float(ultimo_dia["GAP_ACUM"].iloc[0])
                    st.markdown(
                        _formatar_card_gap_ativados(
                            "Leitura do ritmo",
                            f"{formatar_numero_brasileiro(gap_acum, 0)}",
                            f"Gap acumulado de {metrica_diaria.lower()} até o último dia com base no orçamento linear do mês.",
                            "negativo" if gap_acum < 0 else "positivo"
                        ),
                        unsafe_allow_html=True
                    )

                resumo_semana = _montar_resumo_semanal_gap_ativados(serie_dia)
                if not resumo_semana.empty:
                    col_sem1, col_sem2 = st.columns([1.15, 1], gap="large")
                    with col_sem1:
                        renderizar_html_tabela_dashboard(
                            _criar_tabela_html_resumo_semanal_gap_ativados(
                                resumo_semana,
                                table_id="tbl_resumo_semanal_gap_ativados",
                            ),
                            height=260,
                        )
                    with col_sem2:
                        pior_semana = resumo_semana.sort_values("GAP", ascending=True).iloc[0]
                        melhor_semana = resumo_semana.sort_values("GAP", ascending=False).iloc[0]
                        gap_pior = float(pd.to_numeric(pd.Series([pior_semana["GAP"]]), errors="coerce").fillna(0.0).iloc[0])
                        gap_melhor = float(pd.to_numeric(pd.Series([melhor_semana["GAP"]]), errors="coerce").fillna(0.0).iloc[0])
                        semanas_negativas = int((pd.to_numeric(resumo_semana["GAP"], errors="coerce").fillna(0.0) < 0).sum())
                        renderizar_html_tabela_dashboard(
                            _formatar_insights_ativados(
                                "Leitura semanal",
                                [
                                    f"Pior semana | {pior_semana['SEMANA_LABEL']} com gap de {formatar_numero_brasileiro(gap_pior, 0)}",
                                    f"Melhor semana | {melhor_semana['SEMANA_LABEL']} com gap de {formatar_numero_brasileiro(gap_melhor, 0)}",
                                    f"Semanas abaixo | {semanas_negativas} de {len(resumo_semana)} semanas no recorte",
                                ],
                            ),
                            height=300,
                        )
            else:
                st.info("Sem série diária disponível para o recorte selecionado.")
...
def criar_tabela_html_analitica(df_formatado: pd.DataFrame, df_numerico: pd.DataFrame, table_id: str) -> str:
    ...
    for idx, row in df_formatado.iterrows():
        ...
        for col_idx, col in enumerate(colunas):
            valor_bruto = str(row[col])
            if col == col_var:
                valor_bruto = valor_bruto.replace("▲", "").replace("▼", "").strip()
            valor_fmt = escape(valor_bruto)
            classes = []
            col_upper = str(col).upper()
...
def montar_tabela_resultado_canais_exibicao_numerica(
    ...
    df_num = pd.DataFrame({
        ...
        colunas_saida[6]: normalizar_numerico_serie(df_tabela['MOM']).fillna(0.0),
        colunas_saida[7]: normalizar_numerico_serie(df_tabela['VAR_META']).fillna(0.0),
        colunas_saida[8]: normalizar_numerico_serie(df_tabela['YOY']).fillna(0.0),
        colunas_saida[9]: normalizar_numerico_serie(df_tabela['YTD25']).fillna(0.0),
        colunas_saida[10]: normalizar_numerico_serie(df_tabela['YTD26']).fillna(0.0),
        colunas_saida[11]: normalizar_numerico_serie(df_tabela['YTD_ORC']).fillna(0.0),
        colunas_saida[12]: normalizar_numerico_serie(df_tabela['VAR_YTD']).fillna(0.0),
        colunas_saida[13]: normalizar_numerico_serie(df_tabela['VAR_YTD_ORC']).fillna(0.0)
    })
    ...
    if incluir_total and not df_num.empty:
        ...
        total_ytd25 = float(pd.to_numeric(df_num[colunas_saida[9]], errors='coerce').fillna(0.0).sum())
        total_ytd26 = float(pd.to_numeric(df_num[colunas_saida[10]], errors='coerce').fillna(0.0).sum())
        total_ytd_orc = float(pd.to_numeric(df_num[colunas_saida[11]], errors='coerce').fillna(0.0).sum())
        ...
        linha_total = {
            ...
            colunas_saida[6]: mom_total,
            colunas_saida[7]: var_meta_total,
            colunas_saida[8]: yoy_total,
            colunas_saida[9]: total_ytd25,
            colunas_saida[10]: total_ytd26,
            colunas_saida[11]: total_ytd_orc,
            colunas_saida[12]: var_ytd_total,
            colunas_saida[13]: var_ytd_orc_total
        }
        df_num = pd.concat([pd.DataFrame([linha_total]), df_num], ignore_index=True)
...
def cached_tabela_html_funil_cotacoes(df_fmt_json: str, df_num_json: str, table_id: str) -> str:
    _cache_version_visual = "funil_cotacoes_clean_v2"
...
# ==============================
FUNIL_FIXA_FILE_PATH = resolver_arquivo_preprocessado("funil_fixa_ecommerce.parquet", RAW_FUNIL_FIXA_FILE_PATH)
...
def _mes_atual_funil_fixa() -> tuple[pd.Timestamp, int, str]:
    hoje = pd.Timestamp.today()
    periodo_ref = pd.Timestamp(year=int(hoje.year), month=int(hoje.month), day=1)
    mes_ordem = int(periodo_ref.year * 100 + periodo_ref.month)
    return periodo_ref, mes_ordem, _formatar_mes_ano_funil_fixa(periodo_ref)


def _aplicar_qtde_tend_funil_fixa(df_funil: pd.DataFrame) -> pd.DataFrame:
    if df_funil is None or df_funil.empty:
        return pd.DataFrame()

    base = df_funil.copy()
    if 'QTDE_TEND' not in base.columns:
        base['QTDE_TEND'] = 0.0
        return _normalizar_indicadores_funil_fixa_df(base)

    for coluna_txt in ['SEGMENTO', 'ORIGEM_AGG', 'CANAL_ENTRADA', 'INDICADOR', 'MES_ANO', 'INDICADOR_CHAVE']:
        if coluna_txt in base.columns:
            base[coluna_txt] = base[coluna_txt].astype(str).str.strip()

    base['QTDE'] = normalizar_numerico_serie(base.get('QTDE', 0)).fillna(0.0)
    base['QTDE_TEND'] = normalizar_numerico_serie(base['QTDE_TEND']).fillna(0.0)
    base['MES_ANO_ORDEM'] = normalizar_numerico_serie(base.get('MES_ANO_ORDEM', 0)).fillna(0).astype(int)
    base['INDICADOR_ORDEM'] = normalizar_numerico_serie(base.get('INDICADOR_ORDEM', 999.0)).fillna(999.0)
    base['EH_TEND'] = pd.to_numeric(base.get('EH_TEND', 0), errors='coerce').fillna(0).astype(int)
    if 'FONTE_TEND' not in base.columns:
        base['FONTE_TEND'] = ''

    linhas_tend = base[base['QTDE_TEND'].gt(0)].copy()
    if linhas_tend.empty:
        return _normalizar_indicadores_funil_fixa_df(base)

    periodo_ref, mes_ordem, mes_rotulo = _mes_atual_funil_fixa()
    linhas_tend['QTDE'] = linhas_tend['QTDE_TEND']
    linhas_tend['PERIODO_MES'] = periodo_ref
    linhas_tend['MES_ANO'] = mes_rotulo
    linhas_tend['MES_ANO_ORDEM'] = mes_ordem
    linhas_tend['EH_TEND'] = 1
    linhas_tend['FONTE_TEND'] = 'QTDE_TEND'

    # A tendencia da base principal substitui o realizado parcial do mes atual
    # no nivel do indicador/segmento. Isso evita misturar QTDE_TEND com linhas
    # realizadas de origens/canais que nao tiveram tendencia preenchida.
    chaves_dim = [col for col in ['SEGMENTO', 'INDICADOR'] if col in base.columns]
    base_sem_tend = base.copy()
    if chaves_dim:
        chaves_tend = linhas_tend[chaves_dim].drop_duplicates().assign(_remover_tend=1)
        base_reset = base.reset_index(drop=False).rename(columns={'index': '_idx_base'})
        base_reset = base_reset.merge(chaves_tend, on=chaves_dim, how='left')
        mask_mes_atual = pd.to_numeric(base_reset['MES_ANO_ORDEM'], errors='coerce').fillna(0).astype(int).eq(mes_ordem)
        mask_remover = mask_mes_atual & base_reset['_remover_tend'].eq(1)
        base_sem_tend = base_reset.loc[~mask_remover, base.columns.tolist()].copy()

    todas_colunas = list(dict.fromkeys(list(base_sem_tend.columns) + list(linhas_tend.columns)))
    for coluna in todas_colunas:
        if coluna not in base_sem_tend.columns:
            base_sem_tend[coluna] = np.nan
        if coluna not in linhas_tend.columns:
            linhas_tend[coluna] = np.nan

    df_out = pd.concat([base_sem_tend[todas_colunas], linhas_tend[todas_colunas]], ignore_index=True, sort=False)
    df_out['QTDE'] = normalizar_numerico_serie(df_out['QTDE']).fillna(0.0)
    df_out['QTDE_TEND'] = normalizar_numerico_serie(df_out['QTDE_TEND']).fillna(0.0)
    ...
    return _normalizar_indicadores_funil_fixa_df(df_out)
...
def load_funil_fixa_ecommerce_data(
    ...
    if path_obj.suffix.lower() == ".parquet":
        df_opt = _carregar_dataframe_preprocessado(
            ...
            text_cols=['SEGMENTO', 'ORIGEM_AGG', 'CANAL_ENTRADA', 'INDICADOR', 'MES_ANO', 'INDICADOR_CHAVE', 'FONTE_TEND'],
            numeric_cols=['QTDE', 'QTDE_TEND', 'INDICADOR_ORDEM', 'MES_ANO_ORDEM', 'EH_TEND'],
            date_cols=['PERIODO_MES'],
            category_cols=['SEGMENTO', 'ORIGEM_AGG', 'CANAL_ENTRADA', 'INDICADOR', 'MES_ANO', 'INDICADOR_CHAVE'],
            default_values={'EH_TEND': 0, 'QTDE_TEND': 0.0, 'CANAL_ENTRADA': 'Não Informado', 'FONTE_TEND': ''}
        )
        if not df_opt.empty:
            ...
            df_opt = _aplicar_qtde_tend_funil_fixa(df_opt)
            return df_opt
    ...
    col_qtde_tend = _encontrar_coluna_funil_fixa(df_raw.columns, 'QTDE_TEND', 'QTDE TEND', 'QTDE_TENDENCIA', 'QTDE TENDENCIA')
    ...
    if col_qtde_tend:
        rename_map[col_qtde_tend] = 'QTDE_TEND'
    ...
    df['QTDE'] = normalizar_numerico_serie(df['QTDE']).fillna(0.0)
    if 'QTDE_TEND' not in df.columns:
        df['QTDE_TEND'] = 0.0
    df['QTDE_TEND'] = normalizar_numerico_serie(df['QTDE_TEND']).fillna(0.0)
    ...
    df['EH_TEND'] = 0
    df = _aplicar_qtde_tend_funil_fixa(df)
    return df
...
def montar_estrutura_funil_fixa_ecommerce(
    ...
    if mes_tend_ordem is not None and int(mes_atual_ordem) == int(mes_tend_ordem):
        ...
        observacao_mm3 = (
            f"O mes {mapa_ordem_rotulo.get(mes_atual_ordem, str(mes_atual_ordem))} foi carregado pela coluna QTDE_TEND"
            f"{complemento}"
        )
...
def criar_grafico_funil_segmentado_fixa(
    ...
    if mes_tend_ordem is not None and int(mes_ref_ordem) == int(mes_tend_ordem):
        ...
        observacao_mm3 = (
            f"O mes {mapa_ordem_rotulo.get(int(mes_ref_ordem), str(mes_ref_ordem))} foi carregado pela coluna QTDE_TEND"
            f"{complemento}"
        )
...
def render_visual_funil_fixa_ecommerce() -> None:
    funil_path = (
        Path(FUNIL_FIXA_FILE_PATH)
        if DASHBOARD_PARQUET_ONLY
        else resolver_arquivo_dashboard(
...
@fragmento_dashboard
def render_bloco_convergencia_tabelas() -> None:
    st.markdown(
        build_visual_title_html(
            "CONVERGÊNCIA",
            "target",
    ...
    convergencia_mtime = (
        convergencia_path_obj.stat().st_mtime
        if convergencia_path_obj.exists()
        else None
    )
    df_convergencia = load_convergencia_data(
        str(CONVERGENCIA_FILE_PATH),
        convergencia_mtime
    )
    ...
    produtos_base_convergencia = [
        produto for produto in df_convergencia['COD_PLATAFORMA'].dropna().astype(str).str.strip().str.upper().unique().tolist()
        if produto and produto not in {'NAN', 'NONE', '<NA>'}
    ]
    produtos_convergencia = [
        produto for produto in ['FIXA', 'CONTA']
        if produto in set(produtos_base_convergencia)
    ]
    produtos_extras_convergencia = sorted([
        produto for produto in produtos_base_convergencia
        if produto and produto not in set(produtos_convergencia)
    ])
    produtos_convergencia.extend(produtos_extras_convergencia)
    if not produtos_convergencia:
        st.info("Base de convergência sem produtos válidos para montar os visuais.")
        return

    opcoes_produto_convergencia = ["Todos"] + produtos_convergencia
    produto_default_convergencia = "FIXA" if "FIXA" in produtos_convergencia else produtos_convergencia[0]

    st.markdown(
        build_visual_title_html("CONVERGÊNCIA POR REGIONAL", "grid", "subsection-title", extra_style="margin-top:10px;"),
        unsafe_allow_html=True
    )
    col_conv_produto, col_conv_tab_mes, col_conv_tab_canal, col_conv_tab_spacer = st.columns([1.05, 1.05, 1.35, 1.55], gap="medium")
    with col_conv_produto:
        render_filter_label("PRODUTO")
        produto_convergencia_sel = st.selectbox(
            "Produto da convergência",
            options=opcoes_produto_convergencia,
            index=opcoes_produto_convergencia.index(produto_default_convergencia),
            key="produto_convergencia_ativados",
            label_visibility="collapsed"
        )

    df_convergencia_produto = df_convergencia
    if produto_convergencia_sel != "Todos":
        df_convergencia_produto = df_convergencia_produto.loc[
            df_convergencia_produto['COD_PLATAFORMA'].astype(str).str.strip().str.upper().eq(str(produto_convergencia_sel).strip().upper())
        ]
    if df_convergencia_produto.empty:
        st.info("Base de convergência sem dados válidos para o produto selecionado.")
        return

    meses_convergencia = ordenar_meses_convergencia(
        df_convergencia_produto['mes_ano'].dropna().unique().tolist()
    )
    ...
    mes_default_conv = (
        mes_atual_conv
        if mes_atual_conv in meses_convergencia
        else meses_convergencia[-1]
    )
    ...
    if 'QTDE_CNPJ8' in df_convergencia_produto.columns:
        canais_base_conv = (
            df_convergencia_produto.groupby('CANAL_PLAN', observed=True)['QTDE_CNPJ8']
            .sum()
            .sort_values(ascending=False)
            .index.astype(str)
            .tolist()
        ...
        canais_base_conv = sorted(
            df_convergencia_produto['CANAL_PLAN'].dropna().astype(str).str.strip().unique().tolist()
        )
    ...
    df_conv_tabela = df_convergencia_produto
    ...
    html_conv_regional = obter_cache_session_dashboard(
        "html_convergencia_regional_ativados",
        (
            convergencia_mtime,
            str(produto_convergencia_sel),
            tuple(meses_tabela_conv),
            str(canal_tabela_convergencia_sel),
        ),
        lambda: montar_tabela_convergencia_regional_html(df_conv_tabela, meses_tabela_conv),
        max_variacoes=2
    )
    if html_conv_regional:
        renderizar_html_otimizado(html_conv_regional)
    else:
        st.info("Sem dados para montar a tabela regional de convergência.")

    st.markdown(
        build_visual_title_html(
            "EVOLUÇÃO MENSAL DA CONVERGÊNCIA",
            "trend",
            "subsection-title",
            subtitle="CLIENTES TOTAIS E CLIENTES CONVERGENTES NO RECORTE SELECIONADO",
            extra_style="margin-top:16px;"
        ),
        unsafe_allow_html=True
    )
    regionais_grafico_conv = sorted(
        df_convergencia_produto['REGIONAL'].dropna().astype(str).str.strip().unique().tolist()
    )
    col_conv_graf_reg, col_conv_graf_canal, col_conv_graf_mes, col_conv_graf_spacer = st.columns([1.05, 1.35, 1.05, 1.55], gap="medium")
    with col_conv_graf_reg:
        render_filter_label("SELECIONE A REGIONAL")
        regional_grafico_convergencia_sel = st.selectbox(
            "Regional do gráfico de convergência",
            options=["Todas"] + regionais_grafico_conv,
            index=0,
            key="regional_grafico_convergencia_ativados",
            label_visibility="collapsed"
        )
    with col_conv_graf_canal:
        render_filter_label("SELECIONE O CANAL")
        canal_grafico_convergencia_sel = st.selectbox(
            "Canal do gráfico de convergência",
            options=["Todos"] + canais_base_ordenados_conv,
            index=0,
            key="canal_grafico_convergencia_ativados",
            label_visibility="collapsed"
        )
    with col_conv_graf_mes:
        render_filter_label("MÊS FINAL")
        mes_final_grafico_convergencia_sel = st.selectbox(
            "Mês final do gráfico de convergência",
            options=meses_convergencia,
            index=meses_convergencia.index(mes_default_conv),
            key="mes_final_grafico_convergencia_ativados",
            label_visibility="collapsed"
        )
    with col_conv_graf_spacer:
        st.empty()

    idx_mes_conv_grafico = (
        meses_convergencia.index(mes_final_grafico_convergencia_sel)
        if mes_final_grafico_convergencia_sel in meses_convergencia
        else len(meses_convergencia) - 1
    )
    meses_grafico_conv = meses_convergencia[max(0, idx_mes_conv_grafico - 12):idx_mes_conv_grafico + 1]
    df_conv_grafico = df_convergencia_produto
    if regional_grafico_convergencia_sel != "Todas":
        df_conv_grafico = df_conv_grafico[
            df_conv_grafico['REGIONAL'].astype(str).str.strip().eq(str(regional_grafico_convergencia_sel).strip())
        ]
    if canal_grafico_convergencia_sel != "Todos":
        df_conv_grafico = df_conv_grafico[
            df_conv_grafico['CANAL_PLAN'].astype(str).str.strip().eq(str(canal_grafico_convergencia_sel).strip())
        ]
    fig_conv_evolucao = criar_grafico_evolucao_convergencia(df_conv_grafico, meses_grafico_conv)
    st.plotly_chart(
        fig_conv_evolucao,
        width="stretch",
        config={"displayModeBar": False, "displaylogo": False}
    ...
    with col_conv_canal_mes:
        ...
        mes_tabela_canal_convergencia_sel = st.selectbox(
            ...
            options=meses_convergencia,
            index=meses_convergencia.index(mes_default_conv),
            key="mes_convergencia_canal_ativados",
            label_visibility="collapsed"
        )
    with col_conv_canal_reg:
        ...
        regional_tabela_convergencia_sel = st.selectbox(
            ...
            index=0,
            key="regional_convergencia_canal_ativados",
            label_visibility="collapsed"
        )
    with col_conv_canal_spacer:
        st.empty()
    ...
    df_conv_canal = df_convergencia_produto
    ...
    html_conv_canal = obter_cache_session_dashboard(
        "html_convergencia_canal_ativados",
        (
            convergencia_mtime,
            str(produto_convergencia_sel),
            tuple(meses_tabela_conv_canal),
            str(regional_tabela_convergencia_sel),
        ),
        lambda: montar_tabela_convergencia_canal_html(
            df_conv_canal,
    ...
    if html_conv_canal:
        renderizar_html_otimizado(html_conv_canal)
    else:
...
def render_bloco_migracoes_pme() -> None:
    migracoes_path = (
        Path(ANALITICO_MIGRACOES_FILE_PATH)
        if DASHBOARD_PARQUET_ONLY
        else resolver_arquivo_dashboard(
    ...
    if "CANAL_PLAN" not in df_migracoes_pme.columns:
        df_migracoes_pme["CANAL_PLAN"] = "Todos"
    df_migracoes_pme["CANAL_PLAN"] = (
        df_migracoes_pme["CANAL_PLAN"]
        .astype("string")
        .str.strip()
        .fillna("Não informado")
    ...
    canais_migracoes_disp = (
        df_migracoes_pme.groupby("CANAL_PLAN", observed=True)["QTDE"]
        .sum()
        .sort_values(ascending=False)
        .index.astype(str)
        .tolist()
    ...
    canais_migracoes_disp = [
        canal for canal in canais_migracoes_disp
        if canal and canal.strip().upper() not in {"TODOS", "NAN", "NONE", "<NA>"}
    ]
    col_mig_canal_global, col_mig_mes_mom, col_mig_regional_grafico, col_mig_filtros_spacer = st.columns(
        [1.12, 0.95, 1.05, 1.38],
        gap="medium"
    )
    with col_mig_canal_global:
        render_filter_label("Canal")
        canal_migracoes_ref = st.selectbox(
            "Selecione o canal de Migrações PME",
            options=["Todos"] + canais_migracoes_disp,
            index=0,
            key="funil_movel_migracoes_pme_canal_global",
            label_visibility="collapsed"
        )

    if str(canal_migracoes_ref).strip().lower() != "todos":
        df_migracoes_pme = df_migracoes_pme[
            df_migracoes_pme["CANAL_PLAN"].astype(str).str.strip().eq(str(canal_migracoes_ref).strip())
        ].copy()
    if df_migracoes_pme.empty:
        st.info("Sem dados disponíveis de Migrações PME para o canal selecionado.")
        return
    ...
    regionais_migracoes_disp = (
        df_migracoes_pme.groupby("REGIONAL", observed=True)["QTDE"]
        .sum()
        .sort_values(ascending=False)
        .index.astype(str)
        .tolist()
    )
    opcoes_regional_migracoes = ["Todos"] + [
        regional for regional in regionais_migracoes_disp
        if str(regional).strip().upper() not in {"", "TOTAL", "NAN", "NONE", "<NA>"}
    ]
    with col_mig_regional_grafico:
        render_filter_label("Regional")
    ...
    with col_mig_filtros_spacer:
        st.empty()

    mes_migracoes_mom_ref_calc = (
        f"TEND. {mes_migracoes_mom_ref}"
        if str(mes_migracoes_mom_ref).strip().lower() == str(mes_tend_migracoes).strip().lower()
        else mes_migracoes_mom_ref
    )

    tabela_migracoes_fmt, tabela_migracoes_num = montar_tabela_migracoes_pme_regionais(
        df_migracoes_pme,
        mes_mom_ref=mes_migracoes_mom_ref_calc
    )
    ...
    tabela_migracoes_canal_fmt, tabela_migracoes_canal_num = montar_tabela_migracoes_pme_regionais(
        df_migracoes_pme,
        mes_mom_ref=mes_migracoes_mom_ref_calc if "mes_migracoes_mom_ref_calc" in locals() else None,
        dimensao_col="CANAL_PLAN",
        rotulo_coluna="CANAL"
    )
    if tabela_migracoes_canal_fmt.empty:
        st.info("Sem dados disponíveis para montar a tabela de Migrações PME por canal.")
    else:
        st.markdown(
            criar_tabela_html_migracoes_regionais(
                df_formatado=tabela_migracoes_canal_fmt,
                df_numerico=tabela_migracoes_canal_num,
                table_id="tabela-migracoes-canal-pme"
            ),
            unsafe_allow_html=True
        )

    if {"DSC_PLANO_ATU", "VAL_PRODUTO_FINAL"}.issubset(set(df_migracoes_pme.columns)):
        mes_mom_base_migracoes = mes_migracoes_mom_ref_calc if "mes_migracoes_mom_ref_calc" in locals() else None
        st.markdown(
            build_visual_title_html(
                "MIGRAÇÕES PME - TICKET MÉDIO POR REGIONAL E MÊS",
                "grid",
                "subsection-title",
                extra_style="margin-top:18px;"
            ),
        ...
        opcoes_mes_ticket_regional = _filtrar_meses_migracoes_ate_atual(
            df_migracoes_pme["MES_ANO"].dropna().astype(str).unique().tolist()
        )
        mes_ticket_regional_limite = None
        if opcoes_mes_ticket_regional:
            mes_atual_ticket_regional = _mes_atual_migracoes()
            indice_mes_ticket_regional = max(len(opcoes_mes_ticket_regional) - 1, 0)
            for idx_mes, mes_ref in enumerate(opcoes_mes_ticket_regional):
                if str(mes_ref).strip().lower() == str(mes_atual_ticket_regional).strip().lower() and idx_mes > 0:
                    indice_mes_ticket_regional = idx_mes - 1
                    break
            col_ticket_regional_mes, col_ticket_regional_spacer = st.columns([0.85, 2.65], gap="medium")
            with col_ticket_regional_mes:
                render_filter_label("Mês")
                mes_ticket_regional_limite = st.selectbox(
                    "Selecione o mês limite do ticket médio por regional",
                    options=opcoes_mes_ticket_regional,
                    index=indice_mes_ticket_regional,
                    key="migracoes_ticket_regional_mes_limite",
                    label_visibility="collapsed",
                    format_func=lambda mes: str(mes).upper()
                )
            with col_ticket_regional_spacer:
                st.empty()

        tabela_ticket_reg_fmt, tabela_ticket_reg_num = montar_tabela_migracoes_pme_dimensao(
            df_migracoes_pme,
            dimensao_col="REGIONAL",
            rotulo_coluna="REGIONAL",
            metrica="TICKET",
            mes_mom_ref=mes_ticket_regional_limite or mes_mom_base_migracoes,
            mes_limite=mes_ticket_regional_limite
        )
        if tabela_ticket_reg_fmt.empty:
            st.info("Sem dados disponíveis para montar a tabela de ticket médio por regional.")
        else:
            st.markdown(
                criar_tabela_html_migracoes_regionais(
                    df_formatado=tabela_ticket_reg_fmt,
                    df_numerico=tabela_ticket_reg_num,
                    table_id="tabela-migracoes-ticket-regional"
                ),
                unsafe_allow_html=True
            )
        ...
        st.markdown(
            build_visual_title_html(
                "MIGRAÇÕES PME - TICKET MÉDIO POR PLANO E MÊS",
                "grid",
                "subsection-title",
                extra_style="margin-top:18px;"
            ),
        ...
        opcoes_mes_ticket_plano = _filtrar_meses_migracoes_ate_atual(
            df_migracoes_pme["MES_ANO"].dropna().astype(str).unique().tolist()
        )
        mes_ticket_plano_limite = None
        if opcoes_mes_ticket_plano:
            mes_atual_ticket_plano = _mes_atual_migracoes()
            indice_mes_ticket_plano = max(len(opcoes_mes_ticket_plano) - 1, 0)
            for idx_mes, mes_ref in enumerate(opcoes_mes_ticket_plano):
                if str(mes_ref).strip().lower() == str(mes_atual_ticket_plano).strip().lower() and idx_mes > 0:
                    indice_mes_ticket_plano = idx_mes - 1
                    break
            col_ticket_plano_mes, col_ticket_plano_spacer = st.columns([0.85, 2.65], gap="medium")
            with col_ticket_plano_mes:
                render_filter_label("Mês")
                mes_ticket_plano_limite = st.selectbox(
                    "Selecione o mês limite do ticket médio por plano",
                    options=opcoes_mes_ticket_plano,
                    index=indice_mes_ticket_plano,
                    key="migracoes_ticket_plano_mes_limite",
                    label_visibility="collapsed",
                    format_func=lambda mes: str(mes).upper()
                )
            with col_ticket_plano_spacer:
                st.empty()

        tabela_ticket_plano_fmt, tabela_ticket_plano_num = montar_tabela_migracoes_pme_dimensao(
            df_migracoes_pme,
            dimensao_col="DSC_PLANO_ATU",
            rotulo_coluna="PLANO",
            metrica="TICKET",
            mes_mom_ref=mes_ticket_plano_limite or mes_mom_base_migracoes,
            mes_limite=mes_ticket_plano_limite
        )
        if tabela_ticket_plano_fmt.empty:
            st.info("Sem dados disponíveis para montar a tabela de ticket médio por plano.")
        else:
            st.markdown(
                criar_tabela_html_migracoes_regionais(
                    df_formatado=tabela_ticket_plano_fmt,
                    df_numerico=tabela_ticket_plano_num,
                    table_id="tabela-migracoes-ticket-plano"
                ),
                unsafe_allow_html=True
            )

        tabela_qtde_plano_fmt, tabela_qtde_plano_num = montar_tabela_migracoes_pme_dimensao(
            df_migracoes_pme,
            dimensao_col="DSC_PLANO_ATU",
            rotulo_coluna="PLANO",
            metrica="QTDE",
            mes_mom_ref=mes_mom_base_migracoes
        )
        st.markdown(
            build_visual_title_html(
                "MIGRAÇÕES PME - QTDE POR PLANO E MÊS",
                "grid",
                "subsection-title",
                extra_style="margin-top:18px;"
            ),
        ...
        if tabela_qtde_plano_fmt.empty:
            st.info("Sem dados disponíveis para montar a tabela de quantidade por plano.")
        else:
            st.markdown(
                criar_tabela_html_migracoes_regionais(
                    df_formatado=tabela_qtde_plano_fmt,
                    df_numerico=tabela_qtde_plano_num,
                    table_id="tabela-migracoes-qtde-plano"
                ),
                unsafe_allow_html=True
            )
...
def render_bloco_cotacoes_funil_movel() -> None:
    ...
    if (
        (base_funil_cotacoes is None or getattr(base_funil_cotacoes, "empty", True)) and
        not DASHBOARD_PARQUET_ONLY and
        "preparar_base_performance" in globals()
    ):
        try:
    ...
    ligacoes_funil_path_obj = Path(LIGACOES_PERFORMANCE_FILE_PATH)
    if ligacoes_funil_path_obj.exists():
        try:
            ligacoes_funil_mtime = ligacoes_funil_path_obj.stat().st_mtime
            df_ligacoes_funil_conta = load_ligacoes_performance_data(
                str(ligacoes_funil_path_obj),
                ligacoes_funil_mtime
            )
        except Exception:
            df_ligacoes_funil_conta = pd.DataFrame()

        if df_ligacoes_funil_conta is not None and not getattr(df_ligacoes_funil_conta, "empty", True):
            df_ligacoes_funil_conta = df_ligacoes_funil_conta.copy()
            if "TEND_QTD" in df_ligacoes_funil_conta.columns:
                df_ligacoes_funil_conta["TEND_QTD"] = 0.0
            if "DESAFIO_QTD" in df_ligacoes_funil_conta.columns:
                df_ligacoes_funil_conta["DESAFIO_QTD"] = 0.0

            if base_funil_cotacoes is None or getattr(base_funil_cotacoes, "empty", True):
                base_funil_cotacoes = df_ligacoes_funil_conta
            else:
                base_funil_cotacoes = base_funil_cotacoes.copy()
                if {"CANAL_PLAN", "DSC_INDICADOR"}.issubset(base_funil_cotacoes.columns):
                    mask_ligacoes_existentes = (
                        base_funil_cotacoes["CANAL_PLAN"].apply(_mapear_canal_funil_cotacoes).eq("Televendas Receptivo")
                        & base_funil_cotacoes["DSC_INDICADOR"].apply(_normalizar_texto_funil_cotacoes).str.contains("LIGAC", na=False)
                    )
                    if "QTDE" in base_funil_cotacoes.columns:
                        base_funil_cotacoes.loc[mask_ligacoes_existentes, "QTDE"] = 0.0
                base_funil_cotacoes = pd.concat(
                    [base_funil_cotacoes, df_ligacoes_funil_conta],
                    ignore_index=True,
                    sort=False
                )
...
HOSPITALITY_ATIVADOS_NAMES = ("hospitality_ativados.parquet", "base_ativados_hospitality.csv")
HOSPITALITY_RECEITA_NAMES = ("hospitality_receita.parquet", "base_receita_hospitality.csv", "receita_hospitality.csv")
HOSPITALITY_REGION_DEPARA = {
    "GRUPO SP INTERIOR": "RSI",
    "GRUPO SP CAPITAL": "RSC",
    "GRUPO RIO E ES": "RRE",
    "GRUPO SC": "RPS",
    "GRUPO RS": "RRS",
    "GRUPO MINAS GERAIS": "RMG",
    "GRUPO NORDESTE": "RNE",
    "GRUPO PR": "RPS",
    "GRUPO CENTRO OESTE": "RCO",
    "GRUPO CENTRO-OESTE": "RCO",
    "GRUPO BAHIA SERGIPE": "RBS",
    "GRUPO BAHIA/SERGIPE": "RBS",
    "GRUPO NORTE": "RNO",
}
HOSPITALITY_MESES_PT = {
    1: "jan", 2: "fev", 3: "mar", 4: "abr", 5: "mai", 6: "jun",
    7: "jul", 8: "ago", 9: "set", 10: "out", 11: "nov", 12: "dez"
}
HOSP_IND_INSTALACAO = "Instala\u00e7\u00e3o"
HOSP_IND_VENDA_BRUTA = "Venda Bruta"
HOSP_NAO_INFORMADO = "N\u00e3o informado"


def _normalizar_hospitality_chave(valor: object) -> str:
    if valor is None:
        return ""
    try:
        if pd.isna(valor):
...
def _hospitality_data_dirs() -> list[Path]:
    dirs: list[Path] = []
    base = Path(__file__).resolve().parent.parent
    candidatos = [
        Path.cwd(),
        Path.cwd() / "dados_preprocessados",
        Path.cwd() / "Arquivos_Dashboard",
        base,
        base / "dados_preprocessados",
        base / "Arquivos_Dashboard",
        Path.home() / "OneDrive - Claro SA" / "Documentos" / "Extração_VDI" / "FÍSICOS_MOBILIDADE",
        Path(r"C:\Users\thiag\OneDrive - Claro SA\Documentos\Extração_VDI\FÍSICOS_MOBILIDADE"),
        Path(r"C:\Users\F270665\OneDrive - Claro SA\Documentos\Extração_VDI\FÍSICOS_MOBILIDADE"),
    ]
    legado = globals().get("DASHBOARD_LEGACY_MOBILITY_DIR")
    if legado is not None:
        candidatos.insert(0, Path(legado))
...
def _resolver_arquivo_hospitality(nomes: tuple[str, ...], colunas_obrigatorias: tuple[str, ...] = ()) -> Path | None:
    for diretorio in _hospitality_data_dirs():
        for nome in nomes:
            path = diretorio / nome
            if not path.exists():
                continue
            if not colunas_obrigatorias:
                return path
            try:
                if path.suffix.lower() == ".parquet":
                    cabecalho_cols = set(pd.read_parquet(path).columns)
                else:
                    cabecalho_cols = set(pd.read_csv(path, sep=";", encoding="latin1", nrows=0).columns)
                colunas_req = set(colunas_obrigatorias)
                schema_receita_parquet = {"MES_ORDEM", "MES_ANO", "REGIONAL", "NM_PRODUTO", "NM_GRUPO", "RECEITA"}
                if colunas_req.issubset(cabecalho_cols):
                    return path
                if (
                    path.suffix.lower() == ".parquet"
                    and {"SK_DATA", "TV_VL_TT_ASSINANTE"}.issubset(colunas_req)
                    and schema_receita_parquet.issubset(cabecalho_cols)
                ):
                    return path
            except Exception:
                continue
...
def _read_csv_hospitality(path: Path, usecols: set[str]) -> pd.DataFrame:
    for sep in (";", ","):
        for encoding in ("latin1", "utf-8-sig", "cp1252"):
            try:
                df = pd.read_csv(
                    path,
                    sep=sep,
...
def _hospitality_data_dia_ativados(serie: pd.Series) -> pd.Series:
    texto = serie.astype(str).str.strip()
    data = pd.to_datetime(texto, format="%d/%m/%y", errors="coerce")
    mask = data.isna()
    if bool(mask.any()):
        data.loc[mask] = pd.to_datetime(texto.loc[mask], format="%d/%m/%Y", errors="coerce")
    return data


def _hospitality_mes_label(data: pd.Timestamp | datetime | date) -> str:
    data_ts = pd.Timestamp(data)
    return f"{HOSPITALITY_MESES_PT.get(int(data_ts.month), 'jan')}/{str(int(data_ts.year))[-2:]}"


def _hospitality_ordem_para_label(ordem: int | float | str | None) -> str:
    try:
        ordem_int = int(ordem)
        ano = ordem_int // 100
        mes = ordem_int % 100
        return f"{HOSPITALITY_MESES_PT.get(mes, 'jan')}/{str(ano)[-2:]}"
    except Exception:
        return ""


def _hospitality_shift_mes_ordem(ordem: int, meses: int) -> int:
    ano = int(ordem) // 100
    mes = int(ordem) % 100
    periodo = pd.Period(year=ano, month=mes, freq="M") + int(meses)
    return int(periodo.year) * 100 + int(periodo.month)


def _hospitality_fmt_num(valor: object) -> str:
    return formatar_numero_brasileiro(pd.to_numeric(pd.Series([valor]), errors="coerce").fillna(0).iloc[0], 0)


def _hospitality_fmt_moeda(valor: object) -> str:
    numero = float(pd.to_numeric(pd.Series([valor]), errors="coerce").fillna(0).iloc[0])
    return f"R$ {formatar_numero_brasileiro(numero, 0)}"


def _hospitality_fmt_pct(valor: object, casas: int = 1) -> str:
    try:
        numero = float(valor)
    except Exception:
        numero = 0.0
...
def _hospitality_fmt_pp(valor: object) -> str:
    try:
        numero = float(valor)
    except Exception:
        numero = 0.0
    sinal = "+" if numero > 0 else ""
    return f"{sinal}{numero * 100:.1f} p.p.".replace(".", ",")


def _hospitality_variacao(atual: float, base: float) -> float | None:
    if base is None or abs(float(base)) < 1e-9:
        return None
    return (float(atual) / float(base)) - 1.0


def _hospitality_delta_html(valor: float | None, modo: str = "pct", card: bool = False) -> str:
    if valor is None or not np.isfinite(valor):
        classe_neutra = "hosp-delta kpi-variacao-item variacao-neutra" if card else "hosp-delta neutro"
        return f'<span class="{classe_neutra}">-</span>'
    classe = "pos" if valor > 0 else "neg" if valor < 0 else "neutro"
    texto = _hospitality_fmt_pp(valor) if modo == "pp" else _hospitality_fmt_pct(valor)
    if card:
        classe_padrao = "variacao-positiva" if valor > 0 else "variacao-negativa" if valor < 0 else "variacao-neutra"
        return f'<span class="hosp-delta kpi-variacao-item {classe_padrao}">{escape(texto)}</span>'
    icone = "▲" if valor > 0 else "▼" if valor < 0 else "•"
    return f'<span class="hosp-delta {classe}">{icone} {escape(texto)}</span>'


def _hospitality_classe_variacao(valor: float | None) -> str:
    if valor is None or not np.isfinite(valor):
        return "neutro"
    if valor > 0:
        return "pos"
...
def load_hospitality_ativados_data(path_str: str, file_mtime: float | None = None) -> pd.DataFrame:
    path = Path(path_str)
    if path.suffix.lower() == ".parquet":
        try:
            df_parquet = pd.read_parquet(path)
            colunas = ["MES_ORDEM", "MES_ANO", "INDICADOR", "REGIONAL", "PRODUTO_ATUAL", "NM_GRUPO", "QTD"]
            for coluna in colunas:
                if coluna not in df_parquet.columns:
                    df_parquet[coluna] = 0.0 if coluna == "QTD" else ""
            df_parquet["MES_ORDEM"] = pd.to_numeric(df_parquet["MES_ORDEM"], errors="coerce").fillna(0).astype(int)
            df_parquet["QTD"] = pd.to_numeric(df_parquet["QTD"], errors="coerce").fillna(0.0)
            return df_parquet[colunas].copy()
        except Exception:
            return pd.DataFrame(columns=["MES_ORDEM", "MES_ANO", "INDICADOR", "REGIONAL", "PRODUTO_ATUAL", "NM_GRUPO", "QTD"])
    usecols = {
        "DT_CALENDARIO", "NM_INDICADOR_NEGOCIO", "NM_CANAL_AJUSTADO", "CANAL AJUSTADO",
        "PRODUTO_ATUAL", "QTD", "REGIONAL", "NM_GRUPO"
    }
    df = _read_csv_hospitality(path, usecols)
    if df.empty:
        return pd.DataFrame(columns=["MES_ORDEM", "MES_ANO", "INDICADOR", "REGIONAL", "PRODUTO_ATUAL", "NM_GRUPO", "QTD"])

    canal_col = "NM_CANAL_AJUSTADO" if "NM_CANAL_AJUSTADO" in df.columns else "CANAL AJUSTADO" if "CANAL AJUSTADO" in df.columns else None
    if canal_col:
        df = df.loc[df[canal_col].apply(_normalizar_hospitality_chave).eq("HOSPITALITY PME")].copy()
    if df.empty:
        return pd.DataFrame(columns=["MES_ORDEM", "MES_ANO", "INDICADOR", "REGIONAL", "PRODUTO_ATUAL", "NM_GRUPO", "QTD"])

    df["DATA_DIA"] = _hospitality_data_dia_ativados(df.get("DT_CALENDARIO", pd.Series(dtype=object)))
    df = df.loc[df["DATA_DIA"].notna()].copy()
    df["MES_ORDEM"] = df["DATA_DIA"].dt.year.astype(int) * 100 + df["DATA_DIA"].dt.month.astype(int)
    df["MES_ANO"] = df["DATA_DIA"].apply(_hospitality_mes_label)
    df["QTD"] = normalizar_numerico_serie(df.get("QTD", pd.Series(dtype=object))).fillna(0.0)
    df["INDICADOR_CHAVE"] = df.get("NM_INDICADOR_NEGOCIO", "").apply(_normalizar_hospitality_chave)
    df["INDICADOR"] = np.select(
        [
            df["INDICADOR_CHAVE"].str.contains("INSTAL", na=False),
            df["INDICADOR_CHAVE"].str.contains("VENDA BRUTA", na=False),
        ],
        [HOSP_IND_INSTALACAO, HOSP_IND_VENDA_BRUTA],
        default="",
    )
    df = df.loc[df["INDICADOR"].ne("")].copy()
    df["REGIONAL"] = df.get("REGIONAL", "").astype(str).str.strip().str[:3].str.upper()
    df["PRODUTO_ATUAL"] = df.get("PRODUTO_ATUAL", "").astype(str).str.strip().replace({"": HOSP_NAO_INFORMADO, "nan": HOSP_NAO_INFORMADO})
    df["NM_GRUPO"] = df.get("NM_GRUPO", "").astype(str).str.strip().str.upper().replace({"": HOSP_NAO_INFORMADO, "NAN": HOSP_NAO_INFORMADO})
    df_saida = (
        df.groupby(["MES_ORDEM", "MES_ANO", "INDICADOR", "REGIONAL", "PRODUTO_ATUAL", "NM_GRUPO"], as_index=False, observed=True)["QTD"]
        .sum()
        .sort_values(["MES_ORDEM", "INDICADOR", "REGIONAL", "PRODUTO_ATUAL"])
    )
    compactar_colunas_categoricas(df_saida, ["MES_ANO", "INDICADOR", "REGIONAL", "PRODUTO_ATUAL", "NM_GRUPO"])
    del df
    gc.collect()
    return df_saida
...
@st.cache_data(ttl=1800, show_spinner=False, max_entries=CACHE_MAX_ENTRIES_MEDIUM)
def load_hospitality_receita_data(path_str: str, file_mtime: float | None = None) -> pd.DataFrame:
    path = Path(path_str)
    if path.suffix.lower() == ".parquet":
        try:
            df_parquet = pd.read_parquet(path)
            colunas = ["MES_ORDEM", "MES_ANO", "REGIONAL", "NM_PRODUTO", "NM_GRUPO", "RECEITA"]
            for coluna in colunas:
                if coluna not in df_parquet.columns:
                    df_parquet[coluna] = 0.0 if coluna == "RECEITA" else ""
            df_parquet["MES_ORDEM"] = pd.to_numeric(df_parquet["MES_ORDEM"], errors="coerce").fillna(0).astype(int)
            df_parquet["RECEITA"] = pd.to_numeric(df_parquet["RECEITA"], errors="coerce").fillna(0.0)
            return df_parquet[colunas].copy()
        except Exception:
            return pd.DataFrame(columns=["MES_ORDEM", "MES_ANO", "REGIONAL", "NM_PRODUTO", "NM_GRUPO", "RECEITA"])
    usecols = {
        "SK_DATA", "NM_LINHA_NEGOCIO_BI", "FLAG_PME", "NM_GRUPO_REGIONAL",
        "TV_VL_TT_ASSINANTE", "NM_TIPO_PRODUTO_MARCA", "NM_PRODUTO (#1)", "NM_ASSINANTE"
    }
    df = _read_csv_hospitality(path, usecols)
    if df.empty or "SK_DATA" not in df.columns or "TV_VL_TT_ASSINANTE" not in df.columns:
        return pd.DataFrame(columns=["MES_ORDEM", "MES_ANO", "REGIONAL", "NM_PRODUTO", "NM_GRUPO", "RECEITA"])

    if "NM_LINHA_NEGOCIO_BI" in df.columns:
        df = df.loc[df["NM_LINHA_NEGOCIO_BI"].apply(_normalizar_hospitality_chave).str.contains("HOTEL E HOSPITAL", na=False)].copy()
    if df.empty:
        return pd.DataFrame(columns=["MES_ORDEM", "MES_ANO", "REGIONAL", "NM_PRODUTO", "NM_GRUPO", "RECEITA"])

    df["DATA_DIA"] = pd.to_datetime(df["SK_DATA"].astype(str).str.strip(), format="%Y%m%d", errors="coerce")
    df = df.loc[df["DATA_DIA"].notna()].copy()
    df["MES_ORDEM"] = df["DATA_DIA"].dt.year.astype(int) * 100 + df["DATA_DIA"].dt.month.astype(int)
    df["MES_ANO"] = df["DATA_DIA"].apply(_hospitality_mes_label)
    df["RECEITA"] = normalizar_numerico_serie(df["TV_VL_TT_ASSINANTE"]).fillna(0.0)
    df["REGIONAL"] = (
        df.get("NM_GRUPO_REGIONAL", "")
        .apply(_normalizar_hospitality_chave)
        .map(HOSPITALITY_REGION_DEPARA)
        .fillna("N/I")
    )
    flag_pme = df.get("FLAG_PME", "").apply(_normalizar_hospitality_chave)
    df["NM_GRUPO"] = np.where(flag_pme.eq("SIM"), "PME", "GE")
    df["NM_PRODUTO"] = df.get("NM_PRODUTO (#1)", "").astype(str).str.strip().replace({"": HOSP_NAO_INFORMADO, "nan": HOSP_NAO_INFORMADO})
    df_saida = (
        df.groupby(["MES_ORDEM", "MES_ANO", "REGIONAL", "NM_PRODUTO", "NM_GRUPO"], as_index=False, observed=True)["RECEITA"]
        .sum()
        .sort_values(["MES_ORDEM", "REGIONAL", "NM_PRODUTO"])
    )
    compactar_colunas_categoricas(df_saida, ["MES_ANO", "REGIONAL", "NM_PRODUTO", "NM_GRUPO"])
    del df
    gc.collect()
    return df_saida


def _hospitality_meses_referencia(*dfs: pd.DataFrame) -> list[dict[str, object]]:
    partes = []
    for df in dfs:
        if df is not None and not df.empty and {"MES_ORDEM", "MES_ANO"}.issubset(df.columns):
            partes.append(df[["MES_ORDEM", "MES_ANO"]])
    if not partes:
        return []
    meses = (
        pd.concat(partes, ignore_index=True)
        .dropna()
        .drop_duplicates(subset=["MES_ORDEM"])
        .sort_values("MES_ORDEM")
    )
    return meses.to_dict("records")


def _hospitality_valor_qtd(df: pd.DataFrame, indicador: str, mes_ordem: int, grupo: str = "Todos") -> float:
    if df is None or df.empty:
        return 0.0
    base = df.loc[(df["INDICADOR"].astype(str).eq(indicador)) & (df["MES_ORDEM"].astype(int).eq(int(mes_ordem)))]
    if grupo != "Todos":
        base = base.loc[base["NM_GRUPO"].astype(str).eq(str(grupo))]
    return float(pd.to_numeric(base.get("QTD", 0), errors="coerce").fillna(0).sum())


def _hospitality_valor_receita(df: pd.DataFrame, mes_ordem: int, grupo: str = "Todos") -> float:
    if df is None or df.empty:
        return 0.0
    base = df.loc[df["MES_ORDEM"].astype(int).eq(int(mes_ordem))]
    if grupo != "Todos":
        base = base.loc[base["NM_GRUPO"].astype(str).eq(str(grupo))]
    return float(pd.to_numeric(base.get("RECEITA", 0), errors="coerce").fillna(0).sum())


def _hospitality_card_html(titulo: str, valor_principal: str, linhas: list[tuple[str, str, str, str]], destaque: str = "") -> str:
    icon_hint = "money" if "receita" in _normalizar_hospitality_chave(titulo).lower() else "mobile"
    linhas_html = ""
    for rotulo, mes, valor, delta in linhas:
        linhas_html += (
            '<div class="hosp-kpi-row">'
            f'<span class="hosp-kpi-row-label"><b>{escape(rotulo)}</b><small>{escape(mes)}</small></span>'
            f'<span class="hosp-kpi-row-value">{escape(valor)}</span>'
            f'{delta}'
            '</div>'
        )
    return (
        '<div class="hosp-kpi-card kpi-card-dinamico">'
        f'{build_kpi_title_html(titulo, icon_hint)}'
        '<div class="hosp-kpi-value-wrap kpi-value-wrap">'
        f'<div class="hosp-kpi-main kpi-value-dinamico">{valor_principal}</div>'
        '</div>'
        '<div class="hosp-kpi-meta-grid">'
    ) + linhas_html + '</div></div>'


def _hospitality_montar_card_qtd(df: pd.DataFrame, indicador: str, mes_ordem: int, grupo: str) -> str:
    atual = _hospitality_valor_qtd(df, indicador, mes_ordem, grupo)
    m1_ordem = _hospitality_shift_mes_ordem(mes_ordem, -1)
    yoy_ordem = _hospitality_shift_mes_ordem(mes_ordem, -12)
    m1 = _hospitality_valor_qtd(df, indicador, m1_ordem, grupo)
    yoy = _hospitality_valor_qtd(df, indicador, yoy_ordem, grupo)
    return _hospitality_card_html(
        indicador,
        _hospitality_fmt_num(atual),
        [
            ("M-1", _hospitality_ordem_para_label(m1_ordem), _hospitality_fmt_num(m1), _hospitality_delta_html(_hospitality_variacao(atual, m1), card=True)),
            ("YoY", _hospitality_ordem_para_label(yoy_ordem), _hospitality_fmt_num(yoy), _hospitality_delta_html(_hospitality_variacao(atual, yoy), card=True)),
        ],
        "QTD no mês selecionado"
    )


def _hospitality_montar_card_receita(df: pd.DataFrame, mes_ordem: int, grupo: str) -> str:
    atual = _hospitality_valor_receita(df, mes_ordem, grupo)
    m1_ordem = _hospitality_shift_mes_ordem(mes_ordem, -1)
    yoy_ordem = _hospitality_shift_mes_ordem(mes_ordem, -12)
    m1 = _hospitality_valor_receita(df, m1_ordem, grupo)
    yoy = _hospitality_valor_receita(df, yoy_ordem, grupo)
    return _hospitality_card_html(
        "Receita",
        _hospitality_fmt_moeda(atual),
        [
            ("M-1", _hospitality_ordem_para_label(m1_ordem), _hospitality_fmt_moeda(m1), _hospitality_delta_html(_hospitality_variacao(atual, m1), card=True)),
            ("YoY", _hospitality_ordem_para_label(yoy_ordem), _hospitality_fmt_moeda(yoy), _hospitality_delta_html(_hospitality_variacao(atual, yoy), card=True)),
        ],
        "Receita TV no mês"
    )


def _hospitality_fig_produto(df: pd.DataFrame, mes_ordem: int, grupo: str, indicador: str = HOSP_IND_INSTALACAO) -> go.Figure:
    base = df.loc[(df["MES_ORDEM"].astype(int).eq(int(mes_ordem))) & (df["INDICADOR"].astype(str).eq(indicador))].copy()
    if grupo != "Todos":
        base = base.loc[base["NM_GRUPO"].astype(str).eq(str(grupo))]
    agg = (
        base.groupby("PRODUTO_ATUAL", as_index=False, observed=True)["QTD"].sum()
        .sort_values("QTD", ascending=True)
        .tail(10)
    )
    fig = go.Figure()
    fig.add_bar(
        y=agg["PRODUTO_ATUAL"].astype(str),
        x=agg["QTD"],
        orientation="h",
        marker_color="#790E09",
        text=[_hospitality_fmt_num(v) for v in agg["QTD"]],
        textposition="outside",
        hovertemplate="%{y}<br>QTD: %{x:,.0f}<extra></extra>",
    )
    fig.update_layout(
        height=360,
        margin=dict(l=12, r=34, t=8, b=20),
        paper_bgcolor="#FFFFFF",
        plot_bgcolor="#FFFFFF",
        xaxis=dict(showgrid=True, gridcolor="rgba(148,163,184,0.18)", zeroline=False, visible=False),
        yaxis=dict(showgrid=False, title=None, tickfont=dict(size=11, color="#334155")),
        font=dict(family="Manrope, Segoe UI, sans-serif", color="#1F2937"),
        showlegend=False,
    )
    return fig
...
def _hospitality_fig_grupo(df: pd.DataFrame, mes_ordem: int, indicador: str = HOSP_IND_INSTALACAO) -> go.Figure:
    base = df.loc[(df["MES_ORDEM"].astype(int).eq(int(mes_ordem))) & (df["INDICADOR"].astype(str).eq(indicador))].copy()
    agg = base.groupby("NM_GRUPO", as_index=False, observed=True)["QTD"].sum().sort_values("QTD", ascending=False)
    fig = go.Figure()
    fig.add_pie(
        labels=agg["NM_GRUPO"].astype(str),
        values=agg["QTD"],
        hole=0.62,
        marker=dict(colors=["#790E09", "#C84A3F", "#94A3B8"], line=dict(color="#FFFFFF", width=2)),
        textinfo="label+percent",
        textfont=dict(size=11),
        hovertemplate="%{label}<br>QTD: %{value:,.0f}<extra></extra>",
    )
    fig.update_layout(
        height=360,
...
def _hospitality_meses_ultimos(df: pd.DataFrame, qtd: int = 13) -> list[int]:
    if df is None or df.empty or "MES_ORDEM" not in df.columns:
        return []
    return sorted(pd.to_numeric(df["MES_ORDEM"], errors="coerce").dropna().astype(int).unique().tolist())[-qtd:]


def _hospitality_metricas_linha(valores_por_mes: dict[int, float], mes_ref: int) -> tuple[float | None, float | None]:
    atual = float(valores_por_mes.get(mes_ref, 0.0))
    m1 = float(valores_por_mes.get(_hospitality_shift_mes_ordem(mes_ref, -1), 0.0))
    yoy = float(valores_por_mes.get(_hospitality_shift_mes_ordem(mes_ref, -12), 0.0))
    return _hospitality_variacao(atual, m1), _hospitality_variacao(atual, yoy)


def _hospitality_mini_tabela(rows: list[dict[str, object]], meses: list[int], valor_col: str, moeda: bool) -> str:
    if not rows:
        return ""
    mes_ref = meses[-1] if meses else 0
    cab = "".join(f"<th>{escape(_hospitality_ordem_para_label(m))}</th>" for m in meses)
    linhas = ""
    for row in rows:
        valores = {int(k): float(v) for k, v in row.get("valores", {}).items()}
        mom, yoy = _hospitality_metricas_linha(valores, mes_ref)
        fmt = _hospitality_fmt_moeda if moeda else _hospitality_fmt_num
        vals = "".join(f"<td>{escape(fmt(valores.get(m, 0.0)))}</td>" for m in meses)
        linhas += (
            "<tr>"
            f"<td>{escape(str(row.get('nome', '')))}</td>"
            f"{vals}"
            f'<td class="var {_hospitality_classe_variacao(mom)}">{_hospitality_delta_html(mom)}</td>'
            f'<td class="var {_hospitality_classe_variacao(yoy)}">{_hospitality_delta_html(yoy)}</td>'
            "</tr>"
        )
    return (
        '<div class="hosp-detail-wrap"><table class="hosp-detail-table"><thead><tr>'
        f'<th>{escape(valor_col)}</th>{cab}<th>MoM</th><th>YoY</th>'
        '</tr></thead><tbody>'
        f'{linhas}'
        '</tbody></table></div>'
    )
...
def _hospitality_tabela_hierarquica(
    df: pd.DataFrame,
    valor_col: str,
    coluna_pai: str,
    coluna_filho: str,
    ...
    if df is None or df.empty:
        return ""
    base = df.copy()
    if indicador and "INDICADOR" in base.columns:
        base = base.loc[base["INDICADOR"].astype(str).eq(indicador)].copy()
    meses = _hospitality_meses_ultimos(base)
    if not meses:
        return ""
    mes_ref = meses[-1]
    fmt = _hospitality_fmt_moeda if moeda else _hospitality_fmt_num
    meses_header = "".join(f"<th>{escape(_hospitality_ordem_para_label(m))}</th>" for m in meses)

    safe_table_id = re.sub(r"[^A-Za-z0-9_-]+", "-", str(table_id)).strip("-") or "hosp-hier"
    agrupado_pai = base.groupby([coluna_pai, "MES_ORDEM"], as_index=False, observed=True)[valor_col].sum()
    total_por_mes = agrupado_pai.groupby("MES_ORDEM", observed=True)[valor_col].sum().to_dict()

    def _linha(nome: str, valores: dict[int, float], total: bool = False, child_class: str = "", toggle_id: str = "", filho: bool = False) -> str:
        mom, yoy = _hospitality_metricas_linha(valores, mes_ref)
        vals = "".join(f"<td>{escape(fmt(valores.get(m, 0.0)))}</td>" for m in meses)
        classe = "linha-total" if total else child_class
        nome_html = escape(nome)
        if toggle_id:
            nome_html = f'<label class="hosp-expander" for="{escape(toggle_id)}">{escape(nome)}</label>'
        elif filho:
            nome_html = f'<span class="hosp-child-label">{escape(nome)}</span>'
        return (
            f'<tr class="{classe}">'
            f'<td class="col-chave">{nome_html}</td>'
            f"{vals}"
            f'<td class="var {_hospitality_classe_variacao(mom)}">{_hospitality_delta_html(mom)}</td>'
            f'<td class="var {_hospitality_classe_variacao(yoy)}">{_hospitality_delta_html(yoy)}</td>'
            "</tr>"
    ...
    linhas = [_linha("TOTAL", {int(k): float(v) for k, v in total_por_mes.items()}, total=True)]
    controles: list[str] = []
    estilos_toggle: list[str] = []
    pais_ctx = []
    for pai, base_pai in base.groupby(coluna_pai, observed=True):
        valores_pai = {int(k): float(v) for k, v in base_pai.groupby("MES_ORDEM", observed=True)[valor_col].sum().to_dict().items()}
        pais_ctx.append((str(pai), base_pai, valores_pai))
    pais_ctx = sorted(
        pais_ctx,
        key=lambda item: (float(item[2].get(mes_ref, 0.0)), sum(float(v) for v in item[2].values()), item[0]),
        reverse=True,
    )
    for idx_pai, (pai, base_pai, valores_pai) in enumerate(pais_ctx):
        toggle_id = f"{safe_table_id}-toggle-{idx_pai}"
        child_class = f"{safe_table_id}-child-{idx_pai}"
        controles.append(f'<input class="hosp-hier-toggle" type="checkbox" id="{escape(toggle_id)}">')
        estilos_toggle.append(
            f'#{toggle_id}:checked ~ table tr.{child_class} {{ display: table-row; }}'
            f'#{toggle_id}:checked ~ table label[for="{toggle_id}"]::before {{ content: "−"; }}'
        )
        linhas.append(_linha(pai, valores_pai, toggle_id=toggle_id))
        filhos_ctx = []
        for filho, df_filho in base_pai.groupby(coluna_filho, observed=True):
            valores_filho = {
                int(k): float(v)
                for k, v in df_filho.groupby("MES_ORDEM", observed=True)[valor_col].sum().to_dict().items()
            }
            filhos_ctx.append((str(filho), valores_filho))
        filhos_ctx = sorted(
            filhos_ctx,
            key=lambda item: (float(item[1].get(mes_ref, 0.0)), sum(float(v) for v in item[1].values()), item[0]),
            reverse=True,
        )
        for filho, valores_filho in filhos_ctx:
            linhas.append(_linha(filho, valores_filho, child_class=f"hosp-child-row {child_class}", filho=True))

    col_width = 18
    other_width = (100 - col_width) / max(len(meses) + 2, 1)
    colgroup = "<colgroup>" + f'<col style="width:{col_width:.3f}%;">' + "".join(
        f'<col style="width:{other_width:.3f}%;">' for _ in range(len(meses) + 2)
    ) + "</colgroup>"
    return f"""
    <div class="hosp-table-container" id="{escape(table_id)}">
      {''.join(controles)}
      <style>{''.join(estilos_toggle)}</style>
      <table class="hosp-table">
        {colgroup}
        <thead><tr><th class="col-chave">{escape(titulo_coluna)}</th>{meses_header}<th class="col-var">MoM</th><th class="col-var">YoY</th></tr></thead>
        <tbody>{''.join(linhas)}</tbody>
      </table>
    </div>
    """
...
def _hospitality_tabela_simples(
    df: pd.DataFrame,
    valor_col: str,
    coluna_linha: str,
    titulo_coluna: str,
    ...
    fmt = _hospitality_fmt_moeda if moeda else _hospitality_fmt_num
    meses_header = "".join(f"<th>{escape(_hospitality_ordem_para_label(m))}</th>" for m in meses)
    agrupado = base.groupby([coluna_linha, "MES_ORDEM"], as_index=False, observed=True)[valor_col].sum()
    total_por_mes = agrupado.groupby("MES_ORDEM", observed=True)[valor_col].sum().to_dict()

    def _linha(nome: str, valores: dict[int, float], total: bool = False) -> str:
        mom, yoy = _hospitality_metricas_linha(valores, mes_ref)
        vals = "".join(f"<td>{escape(fmt(valores.get(m, 0.0)))}</td>" for m in meses)
        classe = "linha-total" if total else ""
        return (
            f'<tr class="{classe}">'
            f'<td class="col-chave">{escape(nome)}</td>'
            f"{vals}"
            f'<td class="var {_hospitality_classe_variacao(mom)}">{_hospitality_delta_html(mom)}</td>'
            f'<td class="var {_hospitality_classe_variacao(yoy)}">{_hospitality_delta_html(yoy)}</td>'
            "</tr>"
    ...
    linhas = [_linha("TOTAL", {int(k): float(v) for k, v in total_por_mes.items()}, total=True)]
    linhas_ctx = []
    for nome, df_linha in agrupado.groupby(coluna_linha, observed=True):
        valores = {int(k): float(v) for k, v in df_linha.groupby("MES_ORDEM", observed=True)[valor_col].sum().to_dict().items()}
        linhas_ctx.append((str(nome), valores))
    linhas_ctx = sorted(
        linhas_ctx,
        key=lambda item: (float(item[1].get(mes_ref, 0.0)), sum(float(v) for v in item[1].values()), item[0]),
        reverse=True,
    )
    for nome, valores in linhas_ctx:
        linhas.append(_linha(nome, valores))
...
def _hospitality_css() -> str:
    return """
    <style>
    .hosp-shell {
        font-family: 'Manrope', 'Segoe UI', sans-serif;
    }
    .hosp-kpi-card.kpi-card-dinamico {
        min-height: 168px !important;
        padding: 12px 13px 11px !important;
        margin: 0 !important;
        border-radius: 18px !important;
        background:
            radial-gradient(circle at 12% 8%, rgba(255,255,255,0.98) 0%, rgba(255,255,255,0) 30%),
            linear-gradient(180deg, #FFFFFF 0%, #FCFCFD 62%, #F6F7F9 100%) !important;
        border: 1px solid rgba(90, 10, 6, 0.14) !important;
        box-shadow:
            inset 0 1px 0 rgba(255,255,255,0.98),
    ...
    .hosp-kpi-card.kpi-card-dinamico::before {
        left: 0.58rem !important;
        right: 0.58rem !important;
        height: 3px !important;
        border-radius: 999px !important;
        background: linear-gradient(90deg, #5A0A06 0%, #790E09 30%, #FF2800 52%, #790E09 78%, #5A0A06 100%) !important;
    }
    .hosp-kpi-card .kpi-title-dinamico {
        justify-content: center !important;
        gap: 0.62rem !important;
    ...
    .hosp-kpi-card .kpi-title-icon {
        width: 1.62rem !important;
        height: 1.62rem !important;
        border-radius: 0.58rem !important;
    }
    .hosp-kpi-card .kpi-title-icon svg {
        width: 0.84rem !important;
        height: 0.84rem !important;
    }
    .hosp-kpi-card .kpi-title-text {
        letter-spacing: 0.09em !important;
    }
    .hosp-kpi-value-wrap {
        background: rgba(255,255,255,0.74);
        border: 1px solid rgba(121,14,9,0.08);
        border-radius: 13px;
    ...
    .hosp-kpi-row {
        display: grid;
        grid-template-columns: minmax(58px, .95fr) minmax(58px, .8fr) minmax(70px, .9fr);
        align-items: center;
        column-gap: 7px;
        padding: 6px 8px;
    ...
    .hosp-kpi-row-label b {
        color: #790E09;
        margin-right: 4px;
    }
    .hosp-kpi-row-label small {
        color: #94A3B8;
        font-size: 9.6px;
        margin-left: 2px;
    }
    ...
    .hosp-kpi-card .hosp-delta.kpi-variacao-item {
        ...
        background: linear-gradient(180deg, #FFFFFF 0%, #F8FAFC 100%) !important;
        border: 1px solid #DCE3EC !important;
        box-shadow: inset 0 1px 0 rgba(255,255,255,0.90), 0 1px 2px rgba(15,23,42,0.08) !important;
    }
    .hosp-kpi-card .hosp-delta.variacao-positiva {
        color: #167A3B !important;
        border-color: rgba(22,122,59,0.20) !important;
    }
    .hosp-kpi-card .hosp-delta.variacao-negativa {
        color: #B42318 !important;
        border-color: rgba(180,35,24,0.20) !important;
    }
    .hosp-kpi-card .hosp-delta.variacao-neutra {
        color: #64748B !important;
    }
    .hosp-delta.pos { color: #167A3B; }
    .hosp-delta.neg { color: #B42318; }
    .hosp-delta.neutro { color: #64748B; }
    .hosp-chart-card {
        background: #FFFFFF;
        border: 1px solid rgba(121, 14, 9, 0.12);
        border-radius: 14px;
    ...
    .hosp-table-container {
        width: 100%;
        overflow-x: auto;
        border: 1px solid rgba(121,14,9,.13);
        border-radius: 10px;
        background: #FFFFFF;
        box-shadow: 0 12px 26px rgba(15,23,42,.052);
        margin: 8px 0 20px;
    }
    .hosp-table {
        width: 100%;
    ...
    .hosp-table tbody tr:nth-child(even) td {
        background: #FFFBFA;
    }
    .hosp-table tbody td.col-chave,
    .hosp-table thead th.col-chave {
        text-align: left;
        position: sticky;
        left: 0;
        z-index: 2;
    ...
    .hosp-table tbody tr.linha-total td {
        background: #5A0A06 !important;
        color: #FFFFFF !important;
        font-weight: 850;
    }
    .hosp-table tbody tr.linha-total .hosp-delta {
        color: #FFFFFF !important;
    }
    .hosp-table td.var.pos { color: #167A3B; }
    .hosp-table td.var.neg { color: #B42318; }
    .hosp-table td.var.neutro { color: #64748B; }
    .hosp-hier-toggle {
        display: none;
    }
    .hosp-table tr.hosp-child-row {
        display: none;
    }
    .hosp-expander {
        cursor: pointer;
    ...
    .hosp-table tr.hosp-child-row td {
        background: #FAFAFB !important;
        color: #475569;
        font-weight: 600;
    }
    .hosp-details summary {
        cursor: pointer;
        list-style: none;
        display: inline-flex;
        align-items: center;
    ...
    .hosp-details summary::-webkit-details-marker { display: none; }
    .hosp-details summary::before {
        content: "+";
        display: inline-flex;
        align-items: center;
        justify-content: center;
    ...
    .hosp-details[open] summary::before {
        content: "−";
    }
    .hosp-detail-wrap {
        margin: 8px 0 4px;
    ...
    .hosp-detail-table th:first-child,
    .hosp-detail-table td:first-child {
        text-align: left;
    }
    .hosp-detail-table td {
        padding: 6px;
...
@fragmento_dashboard
def render_bloco_hospitality() -> None:
    st.markdown(_hospitality_css(), unsafe_allow_html=True)
    st.markdown(
        build_visual_title_html(
            "HOSPITALITY",
            "hospitality",
            subtitle="INSTALAÇÃO E RECEITA DO CANAL HOTÉIS E HOSPITAIS"
        ),
        unsafe_allow_html=True
    )

    path_ativados = _resolver_arquivo_hospitality(HOSPITALITY_ATIVADOS_NAMES)
    path_receita = _resolver_arquivo_hospitality(
        HOSPITALITY_RECEITA_NAMES,
        colunas_obrigatorias=("SK_DATA", "TV_VL_TT_ASSINANTE")
    )

    if path_ativados is None:
        st.warning("Base de ativados Hospitality não encontrada.")
        return

    ativ_mtime = path_ativados.stat().st_mtime if path_ativados.exists() else None
    receita_mtime = path_receita.stat().st_mtime if path_receita is not None and path_receita.exists() else None
    df_hosp = load_hospitality_ativados_data(str(path_ativados), ativ_mtime)
    df_receita = (
        load_hospitality_receita_data(str(path_receita), receita_mtime)
        if path_receita is not None
        else pd.DataFrame(columns=["MES_ORDEM", "MES_ANO", "REGIONAL", "NM_PRODUTO", "NM_GRUPO", "RECEITA"])
    )

    if df_hosp.empty:
        st.warning("Não há dados válidos para Hospitality após o filtro do canal Hospitality PME.")
        return

    meses_ref = _hospitality_meses_referencia(df_hosp, df_receita)
    if not meses_ref:
        st.info("Sem meses disponíveis para montar a aba Hospitality.")
        return

    meses_labels = [str(item["MES_ANO"]) for item in meses_ref]
    mapa_mes_ordem = {str(item["MES_ANO"]): int(item["MES_ORDEM"]) for item in meses_ref}
    mes_default = meses_labels[-1]
    grupos_disp = sorted(set(df_hosp["NM_GRUPO"].dropna().astype(str).tolist()) | set(df_receita.get("NM_GRUPO", pd.Series(dtype=str)).dropna().astype(str).tolist()))
    grupos_opcoes = ["Todos"] + [g for g in grupos_disp if g and g != HOSP_NAO_INFORMADO]

    col_f1, col_f2, _ = st.columns([0.9, 0.9, 4.2], gap="medium")
    with col_f1:
        render_filter_label("MÊS")
        mes_sel = st.selectbox(
            "Mês Hospitality",
            options=meses_labels,
            index=meses_labels.index(mes_default),
            key="hospitality_mes_ref",
            label_visibility="collapsed",
        )
    with col_f2:
        render_filter_label("GRUPO")
        grupo_sel = st.selectbox(
            "Grupo Hospitality",
            options=grupos_opcoes,
            index=0,
            key="hospitality_grupo_ref",
            label_visibility="collapsed",
        )
    mes_ordem_sel = mapa_mes_ordem.get(mes_sel, int(meses_ref[-1]["MES_ORDEM"]))

    col_cards, col_bar, col_donut = st.columns([0.95, 1.75, 1.15], gap="medium")
    with col_cards:
        st.markdown(_hospitality_montar_card_qtd(df_hosp, HOSP_IND_INSTALACAO, mes_ordem_sel, grupo_sel), unsafe_allow_html=True)
        st.markdown('<div style="height:10px"></div>', unsafe_allow_html=True)
        st.markdown(_hospitality_montar_card_receita(df_receita, mes_ordem_sel, grupo_sel), unsafe_allow_html=True)
    with col_bar:
        st.markdown('<div class="hosp-chart-title">Instalação por Produto Atual</div>', unsafe_allow_html=True)
        st.plotly_chart(_hospitality_fig_produto(df_hosp, mes_ordem_sel, grupo_sel, HOSP_IND_INSTALACAO), width="stretch", config={"displayModeBar": False, "displaylogo": False})
    with col_donut:
        st.markdown('<div class="hosp-chart-title">Instalação por Grupo</div>', unsafe_allow_html=True)
        st.plotly_chart(_hospitality_fig_grupo(df_hosp, mes_ordem_sel, HOSP_IND_INSTALACAO), width="stretch", config={"displayModeBar": False, "displaylogo": False})

    st.markdown(
        build_visual_title_html(
            "INSTALAÇÕES - REGIONAL E PRODUTO",
            "mobile",
            "subsection-title",
            subtitle="Últimos 13 meses • clique no + da regional para abrir produto"
        ),
        unsafe_allow_html=True
    )
    html_inst_reg = _hospitality_tabela_hierarquica(
        df_hosp,
        "QTD",
        "REGIONAL",
        "PRODUTO_ATUAL",
        "REGIONAL",
        "hospitality-instalacao-regional",
        moeda=False,
        indicador=HOSP_IND_INSTALACAO,
    )
    if html_inst_reg:
        st.markdown(html_inst_reg, unsafe_allow_html=True)
    else:
        st.info("Sem dados de instalação por regional.")

    st.markdown(
        build_visual_title_html(
            "INSTALAÇÕES - PRODUTO MÊS A MÊS",
            "mobile",
            "subsection-title",
            subtitle="Quantidade por produto nos últimos 13 meses"
        ),
        unsafe_allow_html=True
    )
    html_inst_prod = _hospitality_tabela_simples(
        df_hosp,
        "QTD",
        "PRODUTO_ATUAL",
        "PRODUTO",
        "hospitality-instalacao-produto",
        moeda=False,
        indicador=HOSP_IND_INSTALACAO,
    )
    if html_inst_prod:
        st.markdown(html_inst_prod, unsafe_allow_html=True)
    else:
        st.info("Sem dados de instalação por produto.")
    ...
    st.markdown(
        build_visual_title_html(
            ...
            "money",
            "subsection-title",
            subtitle="Receita TV mês a mês por regional"
        ),
        unsafe_allow_html=True
    )
    html_rec_reg = _hospitality_tabela_simples(
        df_receita,
        "RECEITA",
        "REGIONAL",
        "REGIONAL",
        "hospitality-receita-regional",
        moeda=True,
    )
    if html_rec_reg:
        st.markdown(html_rec_reg, unsafe_allow_html=True)
    else:
        st.info("Sem dados de receita para a base Hospitality.")

    del df_hosp, df_receita
    gc.collect()
...
components.html(
    ...
    (function() {
      ...
      const TAB_ICONS = {
        ...
        HOSPITALITY: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.85" stroke-linecap="round" stroke-linejoin="round"><path d="M4 21V8.5A2.5 2.5 0 0 1 6.5 6H18a2 2 0 0 1 2 2v13"></path><path d="M3 21h18"></path><path d="M8 10h2"></path><path d="M14 10h2"></path><path d="M8 14h2"></path><path d="M14 14h2"></path><path d="M10.5 21v-3.5h3V21"></path><path d="M7.5 6V4a1 1 0 0 1 1-1h7a1 1 0 0 1 1 1v2"></path></svg>`,
...
st.markdown(
    ...
    <style>
    /* ============================================================
       EXECUTIVE VISUAL POLISH - abas, títulos, cards, filtros e tabelas
       ============================================================ */
    :root {
        --a9-red: #790E09;
        --a9-red-2: #A23B36;
    ...
    .block-container {
        background:
            radial-gradient(circle at 12% 0%, rgba(121, 14, 9, 0.045), rgba(121, 14, 9, 0) 30%),
            linear-gradient(180deg, #FAFBFC 0%, var(--a9-bg) 100%) !important;
    }
    ...
    /* 1. Abas: ocupam a largura toda, com ativo mais claro para leitura executiva. */
    .stTabs [data-baseweb="tab-list"] {
        width: 100% !important;
        ...
        box-shadow: 0 10px 26px rgba(16, 24, 40, 0.055), inset 0 1px 0 rgba(255, 255, 255, 0.92) !important;
        overflow: visible !important;
    }
    ...
    .stTabs [data-baseweb="tab"] {
        ...
        background: linear-gradient(180deg, #FFFFFF 0%, #F8FAFC 100%) !important;
        color: var(--a9-muted) !important;
        box-shadow: none !important;
        transform: none !important;
        transition: background 160ms ease, border-color 160ms ease, box-shadow 160ms ease, color 160ms ease !important;
    }
    ...
    .stTabs [data-baseweb="tab"] .tab-icon-badge {
        ...
        border-radius: 999px !important;
        color: var(--a9-red) !important;
        background: rgba(121, 14, 9, 0.070) !important;
        border: 1px solid rgba(121, 14, 9, 0.10) !important;
        box-shadow: none !important;
    }
    ...
    .stTabs [data-baseweb="tab"]:hover {
        ...
        box-shadow: 0 8px 18px rgba(121, 14, 9, 0.08) !important;
    }

    .stTabs [data-baseweb="tab"][aria-selected="true"],
    .stTabs [aria-selected="true"] {
        color: #FFFFFF !important;
        border-color: rgba(121, 14, 9, 0.70) !important;
        background: linear-gradient(135deg, #8E160F 0%, #5A0A06 100%) !important;
        box-shadow: 0 10px 22px rgba(121, 14, 9, 0.20), inset 0 1px 0 rgba(255,255,255,0.16) !important;
    }

    .stTabs [data-baseweb="tab"][aria-selected="true"] .tab-icon-badge,
    .stTabs [aria-selected="true"] .tab-icon-badge {
        color: #FFFFFF !important;
        background: rgba(255, 255, 255, 0.16) !important;
        border-color: rgba(255, 255, 255, 0.24) !important;
    }

    /* 2. Títulos: mesma voz visual em todas as seções. */
    .main-title {
    ...
    .card-title {
        font-family: 'Manrope', 'Segoe UI', sans-serif !important;
        background: linear-gradient(180deg, rgba(255,255,255,0.98) 0%, rgba(250,251,252,0.96) 100%) !important;
        border: 1px solid rgba(121, 14, 9, 0.12) !important;
        border-left: 4px solid var(--a9-red) !important;
        border-radius: 14px !important;
        box-shadow: 0 8px 20px rgba(16, 24, 40, 0.055) !important;
        color: var(--a9-ink) !important;
    ...
    .card-title .section-icon {
        ...
        color: #FFFFFF !important;
        background: linear-gradient(135deg, #8E160F 0%, #5A0A06 100%) !important;
        box-shadow: 0 5px 12px rgba(121,14,9,0.16) !important;
    }
    ...
    .subsection-title .section-icon svg,
    .card-title .section-icon svg {
    ...
    /* 4 e 5. Tabelas/variações: consistência, leitura limpa e cores mais sóbrias. */
    [class^="tabela-container-"],
    [class*=" tabela-container-"],
    .conv-table-wrap,
    .tabela-container-melhorada,
    ...
    table[class*="tabela"] th,
    .conv-table th {
        background: linear-gradient(180deg, #7B130E 0%, #5A0A06 100%) !important;
        color: #FFFFFF !important;
        text-shadow: none !important;
        box-shadow: none !important;
        border-right: 1px solid rgba(255,255,255,0.13) !important;
        border-bottom: 1px solid rgba(61,7,4,0.74) !important;
        font-family: 'Manrope', 'Segoe UI', sans-serif !important;
        font-weight: 800 !important;
        letter-spacing: 0.025em !important;
    }

    table[class*="tabela"] th.col-var,
    table[class*="tabela"] th.col-variacao,
    table[class*="tabela"] th.col-mom,
    table[class*="tabela"] th.col-yoy,
    table[class*="tabela"] th.th-pct,
    table[class*="tabela"] th[class*="pct"],
    table[class*="tabela"] th[class*="MOM"],
    table[class*="tabela"] th[class*="YOY"] {
        background: linear-gradient(180deg, #6B7280 0%, #4B5563 100%) !important;
    }

    table[class*="tabela"] td,
    .conv-table td {
        border-bottom: 1px solid #EEF0F3 !important;
    ...
    table[class*="tabela"] tbody tr:nth-child(even) td,
    .conv-table tbody tr:nth-child(even) td {
        background: #FAFBFC !important;
    }

    table[class*="tabela"] tbody tr:nth-child(odd) td,
    .conv-table tbody tr:nth-child(odd) td {
        background: #FFFFFF !important;
    }

    table[class*="tabela"] tbody tr:hover td,
    .conv-table tbody tr:hover td {
        background: #FFF8F7 !important;
    }

    table[class*="tabela"] tr[class*="total"] td,
    .conv-table tr.linha-total td {
        background: linear-gradient(180deg, #68110C 0%, #420604 100%) !important;
        color: #FFFFFF !important;
        font-weight: 800 !important;
    ...
    .variacao-positiva,
    .pct-positivo,
    td.col-var.status-positivo,
    td.col-variacao.percentual-positivo {
        color: var(--a9-good) !important;
        background: rgba(31, 122, 77, 0.080) !important;
        border-color: rgba(31, 122, 77, 0.16) !important;
    }
    ...
    .variacao-negativa,
    .pct-negativo,
    td.col-var.status-negativo,
    td.col-variacao.percentual-negativo {
        color: var(--a9-bad) !important;
        background: rgba(180, 35, 24, 0.078) !important;
    ...
    .variacao-neutra,
    .pct-neutro,
    td.col-var.status-neutro,
    td.col-variacao.percentual-neutro {
        color: var(--a9-neutral) !important;
        background: rgba(102, 112, 133, 0.075) !important;
        border-color: rgba(102, 112, 133, 0.14) !important;
    }
    ...
    /* 6. Ritmo vertical: blocos respiram igual, sem abrir buracos grandes. */
    div[data-testid="stVerticalBlock"] {
        gap: 0.68rem !important;
    }

    .element-container:has(.section-title),
    .element-container:has(.subsection-title) {
    ...
    .info-box {
        ...
        border: 1px solid rgba(121, 14, 9, 0.10) !important;
        background: linear-gradient(180deg, #FFFFFF 0%, #FFF8F6 100%) !important;
        box-shadow: 0 6px 16px rgba(16, 24, 40, 0.045) !important;
    }
...
st.markdown(
    """
    <style>
    /* VARIATION COLORS ABSOLUTE FINAL - fim real do arquivo */
    body table[class*="tabela"] tbody tr:not([class*="total"]) td[class*="positivo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):nth-child(even) td[class*="positivo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):nth-child(odd) td[class*="positivo"],
    body table[class*="tabela"] tbody tr:not([class*="total"]):hover td[class*="positivo"],
    ...
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.col-pct,
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo):hover td.col-pct {
        background: transparent !important;
        background-color: transparent !important;
        background-image: none !important;
        box-shadow: none !important;
    ...
    body table[class*="tabela-analitico-necessidade"] tbody tr.linha-realizado td.col-pct,
    body table[class*="tabela-analitico-necessidade"] tbody tr.linha-realizado:hover td.col-pct {
        background: transparent !important;
        background-color: transparent !important;
        background-image: none !important;
        box-shadow: none !important;
    ...
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.col-total-sem,
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo):hover td.col-total-sem {
        background: linear-gradient(180deg, rgba(47, 55, 71, 0.06) 0%, rgba(47, 55, 71, 0.025) 100%) !important;
        color: #1F2937 !important;
    ...
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.col-total-mes,
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo):hover td.col-total-mes {
        background: linear-gradient(180deg, rgba(47, 55, 71, 0.075) 0%, rgba(47, 55, 71, 0.03) 100%) !important;
        color: #1F2937 !important;
        -webkit-text-fill-color: #1F2937 !important;
        border-color: #FFFFFF !important;
    ...
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.col-pct.pct-positivo,
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo):hover td.col-pct.pct-positivo {
        color: #1B5E20 !important;
        -webkit-text-fill-color: #1B5E20 !important;
    }

    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.col-pct.pct-negativo,
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo):hover td.col-pct.pct-negativo {
        color: #B71C1C !important;
        -webkit-text-fill-color: #B71C1C !important;
    }

    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.col-pct.pct-neutro,
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo):hover td.col-pct.pct-neutro {
        color: #475569 !important;
        -webkit-text-fill-color: #475569 !important;
    }

    body table[class*="tabela-analitico-necessidade"] tbody tr.linha-grupo td,
    body table[class*="tabela-analitico-necessidade"] tbody tr.linha-grupo:hover td {
        background: linear-gradient(135deg, #5A0A06 0%, #3D0704 100%) !important;
        color: #FFFFFF !important;
        -webkit-text-fill-color: #FFFFFF !important;
        text-align: left !important;
    ...
    body table[class*="tabela-analitico-regional"] tbody td.col-pct::before,
    body table[class*="tabela-analitico-regional"] tbody td.pct-positivo::before,
    body table[class*="tabela-analitico-regional"] tbody td.pct-negativo::before,
    body table[class*="tabela-analitico-regional"] tbody td.pct-neutro::before {
        content: none !important;
        display: none !important;
    }

    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.col-pct::before,
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.pct-positivo::before,
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.pct-negativo::before,
    body table[class*="tabela-analitico-necessidade"] tbody tr:not(.linha-grupo) td.pct-neutro::before {
        content: none !important;
        display: none !important;
    }
...
st.markdown(
    ...
    <style media="not all">
    /* ============================================================
       A9 EXECUTIVE FINISH - padronização visual final
       Itens: tabelas, gráficos, filtros, títulos, tabelas largas e atualização.
       ============================================================ */
    .a9-update-strip {
        width: 100%;
        display: flex;
        ...
        margin: -6px 0 16px 0;
        padding: 10px 12px;
        border: 1px solid rgba(121, 14, 9, 0.10);
        border-radius: 14px;
    ...
    div[data-testid="stDateInput"] input:hover {
        border-color: rgba(121, 14, 9, 0.28) !important;
        box-shadow: 0 4px 12px rgba(121, 14, 9, 0.055) !important;
    }
    ...
    .card-title {
        ...
        border-left-color: #790E09 !important;
    ...
    div[data-testid="stPlotlyChart"] {
        ...
        border-radius: 14px !important;
        background: #FFFFFF !important;
        box-shadow: 0 6px 16px rgba(16, 24, 40, 0.040) !important;
    }

    body table[class*="tabela"],
    body .conv-table,
    body .hosp-table,
    body .ff-table-box table {
        width: 100% !important;
        border-collapse: separate !important;
        border-spacing: 0 !important;
        font-family: 'Manrope', 'Segoe UI', sans-serif !important;
    ...
    body table[class*="tabela"] thead th,
    body .conv-table thead th,
    body .hosp-table thead th,
    body .ff-table-box table thead th {
        padding: 7px 6px !important;
        font-size: clamp(10.4px, 0.70vw, 11.4px) !important;
        line-height: 1.08 !important;
        white-space: normal !important;
        word-break: normal !important;
        border-right: 1px solid rgba(255,255,255,0.14) !important;
        border-bottom: 1px solid rgba(61,7,4,0.70) !important;
    }

    body table[class*="tabela"] tbody td,
    body .conv-table tbody td,
    body .hosp-table tbody td,
    body .ff-table-box table tbody td {
        padding: 7px 6px !important;
        font-size: clamp(11.1px, 0.73vw, 12.2px) !important;
        line-height: 1.14 !important;
        border-bottom: 1px solid #EEF0F3 !important;
        border-right: 1px solid #F2F4F7 !important;
        box-shadow: none !important;
    ...
    body table[class*="tabela"] thead th:first-child,
    body table[class*="tabela"] tbody td:first-child,
    body .conv-table thead th:first-child,
    body .conv-table tbody td:first-child,
    body .hosp-table thead th:first-child,
    body .hosp-table tbody td:first-child {
        max-width: 220px !important;
        white-space: normal !important;
        overflow-wrap: anywhere !important;
    }

    body table[class*="tabela"] tbody tr:nth-child(even):not([class*="total"]) td,
    body .conv-table tbody tr:nth-child(even):not(.linha-total) td,
    body .hosp-table tbody tr:nth-child(even):not(.linha-total) td {
        background: #FAFBFC !important;
    }

    body table[class*="tabela"] tbody tr:nth-child(odd):not([class*="total"]) td,
    body .conv-table tbody tr:nth-child(odd):not(.linha-total) td,
    body .hosp-table tbody tr:nth-child(odd):not(.linha-total) td {
        background: #FFFFFF !important;
    }

    body table[class*="tabela"] tbody tr:not([class*="total"]):hover td,
    body .conv-table tbody tr:not(.linha-total):hover td,
    body .hosp-table tbody tr:not(.linha-total):hover td {
        background: #FFF8F7 !important;
    }

    body table[class*="tabela"] tbody td[class*="positivo"],
    body table[class*="tabela"] tbody td[class*="negativo"],
    body table[class*="tabela"] tbody td[class*="neutro"],
    body .conv-table tbody td[class*="positivo"],
    body .conv-table tbody td[class*="negativo"],
    body .conv-table tbody td[class*="neutro"] {
        background: transparent !important;
        background-image: none !important;
        box-shadow: none !important;
        font-weight: 600 !important;
    ...
    body .conv-table tbody tr.linha-total td,
    body .conv-table tbody tr.linha-total:hover td,
    body .hosp-table tbody tr.linha-total td,
    body .hosp-table tbody tr.linha-total:hover td {
        background: #5A0A06 !important;
        color: #FFFFFF !important;
        -webkit-text-fill-color: #FFFFFF !important;
        border-color: rgba(255,255,255,0.16) !important;
