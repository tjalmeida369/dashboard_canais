...
def criar_tabela_html_resumo_mensal_canal(
    ...
    largura_canal_pct = 10.0 if col_indicador else 11.4
    largura_indicador_pct = 8.5 if col_indicador else 0.0
    ...
    largura_variacao_pct = 5.8
    largura_ytd_meta_pct = 4.55
    ...
    if largura_meses_total < qtd_meses * 3.7:
        largura_variacao_pct = 5.45
        largura_ytd_meta_pct = 4.35
        largura_meses_total = 100.0 - largura_base_texto_pct - (qtd_variacoes * largura_variacao_pct) - (qtd_ytd_meta * largura_ytd_meta_pct)
    largura_mes_pct = max(largura_meses_total / qtd_meses, 3.4)
    larguras_colunas = []
    for coluna in colunas:
        if coluna == col_canal:
    ...
    <style>
        ...
        table.{table_id} {{
            ...
            min-width: 100%;
            table-layout: fixed;
            font-size: clamp(9.2px, 0.72vw, 11.0px);
            font-family: 'Manrope', 'Segoe UI', sans-serif;
            font-variant-numeric: tabular-nums;
        }}
        .{table_id} thead th {{
            ...
            overflow-wrap: anywhere;
            line-height: 1.0;
            font-size: clamp(8.3px, 0.66vw, 10.0px);
            text-transform: uppercase;
        }}
        .{table_id} thead th.col-canal {{
            position: sticky;
