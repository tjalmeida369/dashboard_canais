...
def render_bloco_analises_detalhadas_ativados(
    ...
    with aba_canal:
        ...
        if not top_gap.empty:
            ...
            tabela_canal_saida = pd.DataFrame({
                "Produto": tabela_canal["PRODUTO"],
                "Canal": tabela_canal["CANAL_PLAN"],
                "Métrica": tabela_canal["METRICA"],
                "Fonte": tabela_canal["FONTE"],
                "Valor": tabela_canal["VALOR_ANALISE"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                "ORC": tabela_canal["ORC"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                "Gap": tabela_canal["GAP"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                "Ating.": tabela_canal["ATING_PCT"].apply(_formatar_pct_delta_ativados),
            })
            st.dataframe(tabela_canal_saida, use_container_width=True, hide_index=True, height=260)
    ...
    with aba_regional:
        ...
        if not regional_ofensores.empty:
            ...
            tabela_saida = pd.DataFrame({
                ...
                "Canal": tabela["CANAL_PLAN"],
                "Regional": tabela["REGIONAL"],
                "Métrica": tabela["METRICA"],
                "Fonte": tabela["FONTE"],
                "Valor": tabela["VALOR_ANALISE"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                "ORC": tabela["ORC"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                "Gap": tabela["GAP"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                "Ating.": tabela["ATING_PCT"].apply(_formatar_pct_delta_ativados),
            })
            st.dataframe(tabela_saida, use_container_width=True, hide_index=True, height=420)
    ...
    with aba_conversao:
        ...
        if not conv_plot.empty:
            ...
            if not conv_scatter.empty:
                ...
                for produto_nome, df_prod in conv_scatter.groupby("PRODUTO", observed=True):
            ...
            tabela_conv_saida = pd.DataFrame({
                "Produto": tabela_conv["PRODUTO"],
                "Canal": tabela_conv["CANAL_PLAN"],
                "Regional": tabela_conv["REGIONAL"],
                "Venda Bruta": tabela_conv["VENDA BRUTA"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                "Ativados": tabela_conv["ATIVADOS"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                "Conversão": tabela_conv["CONVERSAO"].apply(_formatar_pct_simples_ativados),
                "p.p. MoM": tabela_conv["PP_MOM"].apply(lambda v: "-" if pd.isna(v) else f"{float(v) * 100:+.1f} p.p.".replace(".", ",")),
            })
            st.dataframe(tabela_conv_saida, use_container_width=True, hide_index=True, height=300)
        else:
        ...
        if modo_periodo == "YTD":
            ...
            if not serie_dia.empty:
                ...
                if not resumo_semana.empty:
                    resumo_semana_saida = pd.DataFrame({
                        "Semana": resumo_semana["SEMANA_LABEL"],
                        "Real": resumo_semana["REAL"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                        "ORC linear": resumo_semana["ORC"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                        "Gap": resumo_semana["GAP"].apply(lambda v: formatar_numero_brasileiro(v, 0)),
                        "Ating.": resumo_semana["ATING"].apply(_formatar_pct_delta_ativados),
                    })
                    col_sem1, col_sem2 = st.columns([1.15, 1], gap="large")
                    with col_sem1:
                        st.dataframe(resumo_semana_saida, use_container_width=True, hide_index=True, height=220)
                    with col_sem2:
                        ...
                        st.markdown(
                            _formatar_insights_ativados(
                                "Leitura semanal",
                                [
                                    f"Pior semana: {pior_semana['SEMANA_LABEL']} com gap de {formatar_numero_brasileiro(pior_semana['GAP'], 0)}.",
                                    f"Melhor semana: {melhor_semana['SEMANA_LABEL']} com gap de {formatar_numero_brasileiro(melhor_semana['GAP'], 0)}.",
                                    "Se as últimas semanas estão negativas, o problema é ritmo recente; se só a primeira foi ruim, o foco é recuperação do estoque acumulado.",
                                ],
                            ),
                            unsafe_allow_html=True
                        )
