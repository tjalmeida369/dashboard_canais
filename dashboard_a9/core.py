...
def load_migracoes_pme_data(path: str, file_mtime: float | None = None) -> pd.DataFrame:
    ...
    _ = file_mtime
    path_obj = Path(path)
    colunas_saida = ["REGIONAL", "MES_ANO", "DSC_PLANO_ATU", "QTDE", "VAL_PRODUTO_FINAL"]
    if not path_obj.exists():
        return pd.DataFrame(columns=colunas_saida)
    ...
    if path_obj.suffix.lower() == ".parquet":
        df_opt = _carregar_dataframe_preprocessado(
            ...
            text_cols=["REGIONAL", "MES_ANO", "DSC_PLANO_ATU"],
            numeric_cols=["QTDE", "VAL_PRODUTO_FINAL"],
            category_cols=["REGIONAL", "MES_ANO", "DSC_PLANO_ATU"],
            default_values={"DSC_PLANO_ATU": "SEM PLANO", "VAL_PRODUTO_FINAL": 0.0}
        )
        if df_opt.empty:
            return pd.DataFrame(columns=colunas_saida)
    ...
    df = (
        df.groupby(["REGIONAL", "MES_ANO", "DSC_PLANO_ATU"], as_index=False, observed=True, dropna=False)[["QTDE", "VAL_PRODUTO_FINAL"]]
        .sum()
    )
    compactar_colunas_categoricas(df, ["REGIONAL", "MES_ANO", "DSC_PLANO_ATU"])
...
def render_bloco_convergencia_tabelas() -> None:
    ...
    col_conv_produto, col_conv_produto_spacer = st.columns([1.05, 3.95], gap="medium")
