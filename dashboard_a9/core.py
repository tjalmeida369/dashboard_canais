...
def _montar_serie_diaria_gap_ativados(
    ...
    if canal != "Todos":
        df_dia = df_dia.loc[df_dia["CANAL_PLAN"].eq(canal)]
    if metrica != "Todos":
...
def _montar_serie_ytd_gap_ativados(
    ...
    if canal != "Todos":
        df_mes = df_mes.loc[df_mes["CANAL_PLAN"].eq(canal)]
    if metrica != "Todos":
...
def render_bloco_analises_detalhadas_ativados(
    ...
    col_f1, col_f2, col_f3, col_f4 = st.columns([1, 1.45, 1, 1], gap="medium")
    with col_f1:
    ...
    def _filtrar(df_filtro: pd.DataFrame) -> pd.DataFrame:
        ...
        if canal_sel != "Todos" and "CANAL_PLAN" in df_tmp.columns:
            df_tmp = df_tmp.loc[df_tmp["CANAL_PLAN"].eq(canal_sel)]
