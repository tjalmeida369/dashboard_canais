...
TEND_FUNIL_FIXA_FILE_PATH = resolver_arquivo_preprocessado("tend_funil_fixa.parquet", RAW_TEND_FUNIL_FIXA_FILE_PATH)
...
def load_funil_fixa_ecommerce_data(
    ...
    if path_obj.suffix.lower() == ".parquet":
        ...
        if not df_opt.empty:
            ...
            tem_qtde_tend_principal = (
                'QTDE_TEND' in df_opt.columns and
                float(normalizar_numerico_serie(df_opt['QTDE_TEND']).fillna(0.0).sum()) > 0
            )
            df_opt = _aplicar_qtde_tend_funil_fixa(df_opt)

            if not tem_qtde_tend_principal:
                tend_path_obj = (
                    Path(tend_path) if tend_path and DASHBOARD_PARQUET_ONLY else
                    Path(TEND_FUNIL_FIXA_FILE_PATH) if DASHBOARD_PARQUET_ONLY else
                    resolver_arquivo_dashboard(
                ...
                if DASHBOARD_PARQUET_ONLY and Path(tend_path_obj).suffix.lower() != ".parquet":
                    tend_path_obj = Path(TEND_FUNIL_FIXA_FILE_PATH)
    ...
    df['EH_TEND'] = 0
    tem_qtde_tend_principal = float(df['QTDE_TEND'].sum()) > 0
...
def render_visual_funil_fixa_ecommerce() -> None:
    ...
    tend_path = (
        Path(TEND_FUNIL_FIXA_FILE_PATH)
        if DASHBOARD_PARQUET_ONLY
        else resolver_arquivo_dashboard(
            TEND_FUNIL_FIXA_FILE_PATH,
            'tend_funil_ecom.xlsx',
    ...
    for diretorio in _hospitality_data_dirs():
        for nome in nomes:
            ...
            try:
                ...
                if set(colunas_obrigatorias).issubset(cabecalho_cols):
                    return path
            except Exception:
                continue
    return None
