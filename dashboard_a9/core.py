...
def criar_grafico_evolucao_convergencia(df_ref: pd.DataFrame, meses_ref: list[str]) -> go.Figure:
    ...
    fig.add_trace(
        go.Bar(
            ...
            cliponaxis=False,
            marker=dict(
                color="rgba(47, 55, 71, 0.18)",
                line=dict(color="rgba(47, 55, 71, 0.26)", width=1)
            ),
            width=0.62,
    ...
    fig.update_yaxes(
        title_text="Clientes totais",
        secondary_y=False,
        range=[0, max_total * 1.22 if max_total > 0 else 1],
        showgrid=True,
        gridcolor="rgba(47,55,71,0.08)",
        zeroline=False,
        tickfont=dict(size=11, color="#6B7280")
    )
    fig.update_yaxes(
        title_text="Clientes convergentes",
        secondary_y=True,
        range=[0, max_conv * 1.42 if max_conv > 0 else 1],
        showgrid=False,
        zeroline=False,
        tickfont=dict(size=11, color="#790E09")
    )
    fig.update_xaxes(
        type="category",
        categoryorder="array",
    ...
    fig.update_layout(
        ...
        paper_bgcolor="#FFFFFF",
        plot_bgcolor="#FFFFFF",
        margin=dict(l=28, r=34, t=38, b=28),
        hovermode="x unified",
        bargap=0.25,
        legend=dict(
            orientation="h",
        ...
        hoverlabel=dict(bgcolor="#FFFFFF", bordercolor="rgba(121,14,9,0.22)", font_size=12)
    )
    return fig
...
def render_bloco_migracoes_pme() -> None:
    ...
    col_mig_canal_global, col_mig_canal_spacer = st.columns([1.10, 2.40], gap="medium")
