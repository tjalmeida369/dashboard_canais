...
def render_bloco_analises_detalhadas_ativados(
    ...
    with aba_canal:
        ...
        with col_k4:
            st.markdown(
                _formatar_card_gap_ativados(
                    "Principal ofensor",
                    pior_canal,
                    "Canal e métrica com maior gap absoluto",
                    "alerta"
                ),
                unsafe_allow_html=True
            )
        ...
        st.markdown(
            build_visual_title_html("GAP DE ATIVADOS/INSTALAÇÃO POR CANAL", "bar-chart", "subsection-title"),
            unsafe_allow_html=True
        )
        top_gap = canal_gap_principal.copy()
    ...
    with aba_regional:
        gap_regional_total = (
            float(pd.to_numeric(regional_gap_principal["GAP"], errors="coerce").fillna(0.0).sum())
            if not regional_gap_principal.empty else 0.0
        )
        regionais_negativas = 0
        maior_recuperacao = "-"
        if not regional_gap_principal.empty:
            regionais_negativas = int(regional_gap_principal.loc[regional_gap_principal["GAP"] < 0, "REGIONAL"].nunique())
            row_melhor_reg = regional_gap_principal.sort_values("GAP", ascending=False).iloc[0]
        ...
        st.markdown(
            build_visual_title_html("OFENSORES DE ATIVADOS/INSTALAÇÃO POR REGIONAL", "table", "subsection-title"),
            unsafe_allow_html=True
        )
        if not regional_gap_principal.empty:
            top_reg = regional_gap_principal.sort_values("GAP", ascending=True).head(18).copy()
            top_reg["LABEL"] = (
            ...
            tabela = regional_gap_principal.sort_values("GAP", ascending=True).head(45).copy()
            ...
            st.dataframe(tabela_saida, use_container_width=True, hide_index=True, height=330)

            regional_dimensoes_com_orc = regional_dimensoes.loc[
                pd.to_numeric(regional_dimensoes["ORC"], errors="coerce").fillna(0.0) > 0
            ].copy()
            ...
            if not heat_reg.empty:
                ordem_reg = (
                    regional_gap_principal.groupby("REGIONAL", observed=True)["GAP"]
                    .sum()
