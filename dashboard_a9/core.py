...
SESSION_CACHE_MAX_TEXT_CHARS = 240_000
...
def renderizar_html_otimizado(html: object, markdown_kwargs: dict | None = None) -> None:
    ...
    for estilo in estilos:
        estilo_chave = hashlib.blake2b(estilo.encode("utf-8", errors="ignore"), digest_size=12).hexdigest()
        if estilo_chave not in _RUN_CSS_RENDERED:
            _ST_MARKDOWN_ORIGINAL(estilo, unsafe_allow_html=True)
            _RUN_CSS_RENDERED.add(estilo_chave)
...
BASE_PRINCIPAL_MENSAL_FILE_PATH = resolver_arquivo_preprocessado("base_principal_mensal.parquet", PRIMARY_BASE_FILE_PATH)
RUNTIME_BASE_FILE_PATH = BASE_PRINCIPAL_MENSAL_FILE_PATH if Path(BASE_PRINCIPAL_MENSAL_FILE_PATH).exists() else PRIMARY_BASE_FILE_PATH
...
def _formatar_data_atualizacao_dashboard(valor) -> str:
    try:
        data_ref = pd.to_datetime(valor, errors="coerce")
    except Exception:
        data_ref = pd.NaT
    if pd.isna(data_ref):
        return "--/--"
    return pd.Timestamp(data_ref).strftime("%d/%m")

def _normalizar_texto_atualizacao_dashboard(valor) -> str:
...
def _mtime_arquivo_dashboard(path_ref: str | Path | None) -> float | None:
    try:
        path_obj = Path(path_ref)
        return path_obj.stat().st_mtime if path_obj.exists() else None
    except Exception:
        return None

def _data_maxima_parquet_dashboard(
    path_ref: str | Path | None,
    *,
    colunas_data: tuple[str, ...],
    filtro_indicador: tuple[str, ...] = tuple(),
    filtro_produto: tuple[str, ...] = tuple(),
    coluna_indicador: str | None = None,
    coluna_produto: str = "COD_PLATAFORMA",
    somente_qtde_real: bool = True,
    usar_mtime_fallback: bool = False,
) -> str:
    path_obj = Path(path_ref) if path_ref else None
    if path_obj is None or not path_obj.exists():
        return "--/--"
    try:
        df_ref = pd.read_parquet(path_obj)
        if df_ref.empty:
            return _formatar_data_atualizacao_dashboard(datetime.fromtimestamp(path_obj.stat().st_mtime)) if usar_mtime_fallback else "--/--"

        mask = pd.Series(True, index=df_ref.index)
        if somente_qtde_real and "QTDE" in df_ref.columns:
            mask &= pd.to_numeric(df_ref["QTDE"], errors="coerce").fillna(0).gt(0)
        if filtro_indicador:
            col_ind = coluna_indicador
            if col_ind is None:
                col_ind = next((col for col in ("DSC_INDICADOR", "INDICADOR_CANONICO", "INDICADOR") if col in df_ref.columns), None)
            if col_ind in df_ref.columns:
                ind_norm = df_ref[col_ind].map(_normalizar_texto_atualizacao_dashboard)
                termos = tuple(_normalizar_texto_atualizacao_dashboard(item) for item in filtro_indicador)
                mask &= ind_norm.map(lambda valor: any(termo and termo in valor for termo in termos))
        if filtro_produto and coluna_produto in df_ref.columns:
            prod_norm = df_ref[coluna_produto].map(_normalizar_texto_atualizacao_dashboard)
            produtos = {_normalizar_texto_atualizacao_dashboard(item) for item in filtro_produto}
            mask &= prod_norm.isin(produtos)

        df_filtrado = df_ref.loc[mask] if len(mask) == len(df_ref) else df_ref
        for coluna_data in colunas_data:
            if coluna_data not in df_filtrado.columns:
                continue
            serie_data = pd.to_datetime(df_filtrado[coluna_data], errors="coerce")
            serie_data = serie_data[serie_data.notna()]
            if not serie_data.empty:
                return _formatar_data_atualizacao_dashboard(serie_data.max())

        if usar_mtime_fallback:
            return _formatar_data_atualizacao_dashboard(datetime.fromtimestamp(path_obj.stat().st_mtime))
        return "--/--"
    except Exception:
        if usar_mtime_fallback:
            return _formatar_data_atualizacao_dashboard(datetime.fromtimestamp(path_obj.stat().st_mtime))
...
def montar_itens_atualizacao_dashboard(mtimes_ref: tuple[tuple[str, float | None], ...]) -> tuple[tuple[str, str], ...]:
    _ = mtimes_ref
    itens = [
        (
            "Pedidos",
            _data_maxima_parquet_dashboard(
                PEDIDOS_FILE_PATH,
                colunas_data=("DAT_MOVIMENTO2", "DATA_DIA"),
                filtro_indicador=("PEDIDOS",),
            ),
        ),
        (
            "Cotações",
            _data_maxima_parquet_dashboard(
                COTACOES_FILE_PATH,
                colunas_data=("DATA_CRIACAO_COTACAO", "DAT_MOVIMENTO2", "DATA_DIA"),
                somente_qtde_real=False,
            ),
        ),
        (
            "Instalados Fixa",
            _data_maxima_parquet_dashboard(
                RUNTIME_BASE_FILE_PATH,
                colunas_data=("DAT_MOVIMENTO2", "DATA_DIA"),
                filtro_indicador=("INSTAL",),
                filtro_produto=("FIXA",),
            ),
        ),
        (
            ...
            _data_maxima_parquet_dashboard(
                RUNTIME_BASE_FILE_PATH,
                colunas_data=("DAT_MOVIMENTO2", "DATA_DIA"),
                filtro_indicador=("GROSS LIQ",),
                filtro_produto=("CONTA", "MOVEL", "MÓVEL"),
            ),
        ),
        (
            "Ligações",
            _data_maxima_parquet_dashboard(
                LIGACOES_FILE_PATH,
                colunas_data=("DATA_DIA", "DAT_MOVIMENTO2"),
            ),
        ),
        (
            "Backlog",
            _data_maxima_parquet_dashboard(
                ...
                colunas_data=("DATA_DIA", "DAT_MOVIMENTO2", "DATA"),
                somente_qtde_real=False,
                usar_mtime_fallback=True,
            ),
        ),
        (
            "Desativados",
            _data_maxima_parquet_dashboard(
                DESATIVADOS_FILE_PATH,
                colunas_data=("DAT_MOVIMENTO2", "DATA_DIA"),
            ),
        ),
        (
            "Convergência",
            _data_maxima_parquet_dashboard(
                CONVERGENCIA_FILE_PATH,
                colunas_data=("DATA_DIA", "DAT_MOVIMENTO", "DAT_MOVIMENTO2"),
                somente_qtde_real=False,
                usar_mtime_fallback=True,
            ),
        ),
...
def render_barra_atualizacao_dashboard() -> None:
    paths_ref = [
        PEDIDOS_FILE_PATH,
        COTACOES_FILE_PATH,
        RUNTIME_BASE_FILE_PATH,
    ...
    mtimes_ref = tuple((str(path), _mtime_arquivo_dashboard(path)) for path in paths_ref)
    itens = montar_itens_atualizacao_dashboard(mtimes_ref)
    texto = "Atualizados em: " + "  •  ".join(
        f"{label}: {data_ref}" for label, data_ref in itens if data_ref and data_ref != "--/--"
    )
    if not texto.strip() or texto.strip() == "Atualizados em:":
        return
    texto_html = escape(f"{texto}  •  ")
    renderizar_html_otimizado(f"""
    <style>
        .a9-update-ticker {{
            width: 100%;
            overflow: hidden;
            ...
            border-radius: 999px;
            background: linear-gradient(135deg, rgba(255,255,255,0.94), rgba(250,246,245,0.92));
            box-shadow: 0 8px 22px rgba(121, 14, 9, 0.055);
        }}
        .a9-update-ticker-track {{
            display: flex;
            width: max-content;
            min-width: 200%;
            animation: a9-update-ticker-scroll 38s linear infinite;
        }}
        .a9-update-ticker span {{
            display: inline-flex;
            align-items: center;
            padding: 7px 22px;
            color: #4B5563;
        ...
        .a9-update-ticker strong {{
            color: #790E09;
            font-weight: 900;
        }}
        @keyframes a9-update-ticker-scroll {{
            from {{ transform: translateX(0); }}
            to {{ transform: translateX(-50%); }}
        }}
        @media (prefers-reduced-motion: reduce) {{
            .a9-update-ticker-track {{ animation: none; }}
        }}
    </style>
    <div class="a9-update-ticker" aria-label="Datas de atualização das bases">
        <div class="a9-update-ticker-track">
            <span><strong>{texto_html}</strong></span>
            <span><strong>{texto_html}</strong></span>
        </div>
    </div>
    """)
...
def montar_serie_grafico_migracoes_pme(
    ...
    df_serie["_ORDEM_TIPO"] = df_serie["TIPO"].map({"REALIZADO": 0, "TENDENCIA": 1}).fillna(0)
    df_serie = df_serie.sort_values(
        by=["MES_ANO", "_ORDEM_TIPO"],
        key=lambda serie: (
            serie.map(lambda valor: pd.Timestamp(mes_ano_para_data(str(valor))))
            if serie.name == "MES_ANO"
            else serie
        )
    ).drop(columns=["_ORDEM_TIPO"], errors="ignore").reset_index(drop=True)
    return df_serie
...
@st.cache_data(show_spinner=False, max_entries=4, persist="disk")
def cached_tabela_html_resumo_mensal_canal(df_fmt_json: str, df_num_json: str, table_id: str) -> str:
    return criar_tabela_html_resumo_mensal_canal(
        desserializar_dataframe_cache(df_fmt_json),
        desserializar_dataframe_cache(df_num_json),
        table_id
    )
...
@st.cache_data(show_spinner=False, max_entries=3, persist="disk")
def cached_tabela_html_backlog_canais(df_fmt_json: str, df_num_json: str, table_id: str) -> str:
    return criar_tabela_html_backlog_canais(
        desserializar_dataframe_cache(df_fmt_json),
        desserializar_dataframe_cache(df_num_json),
        table_id
    )
...
@st.cache_data(show_spinner=False, max_entries=3, persist="disk")
def cached_tabela_html_migracoes_regionais(df_fmt_json: str, df_num_json: str, table_id: str) -> str:
    return criar_tabela_html_migracoes_regionais(
        desserializar_dataframe_cache(df_fmt_json),
        desserializar_dataframe_cache(df_num_json),
        table_id
    )
...
def render_bloco_convergencia_tabelas() -> None:
    ...
    col_conv_produto, col_conv_mes, col_conv_canal, col_conv_regional = st.columns([1.0, 1.0, 1.35, 1.35], gap="medium")
    ...
    with col_conv_mes:
        ...
        mes_tabela_convergencia_sel = st.selectbox(
            "Mês das tabelas de convergência",
    ...
    mes_tabela_canal_convergencia_sel = mes_tabela_convergencia_sel
    idx_mes_conv_canal = (
...
def render_bloco_backlog_fixa_pme() -> None:
    ...
    if tabela_backlog_fmt.empty:
        ...
        renderizar_html_otimizado(
            cached_tabela_html_backlog_canais(
                serializar_dataframe_cache(tabela_backlog_fmt),
                serializar_dataframe_cache(tabela_backlog_num),
                "tabela-funil-fixa-backlog-fixa-pme"
            )
        )
...
def render_bloco_migracoes_pme() -> None:
    ...
    if not opcoes_mes_mom_migracoes:
        ...
        col_mig_mes_mom, col_mig_regional, col_mig_spacer = st.columns([0.85, 0.95, 1.70], gap="medium")
        with col_mig_mes_mom:
            ...
            mes_migracoes_mom_ref = st.selectbox(
                ...
                index=idx_mes_migracoes_padrao,
    ...
    if not opcoes_mes_mom_migracoes:
        col_mig_regional, col_mig_spacer = st.columns([0.95, 2.55], gap="medium")
    with col_mig_regional:
        render_filter_label("Regional")
    ...
    if tabela_migracoes_fmt.empty:
        ...
        renderizar_html_otimizado(
            cached_tabela_html_migracoes_regionais(
                serializar_dataframe_cache(tabela_migracoes_fmt),
                serializar_dataframe_cache(tabela_migracoes_num),
                "tabela-analitico-migracoes-pme"
            )
        )
...
def render_bloco_cotacoes_funil_movel() -> None:
    ...
    if df_cotacoes_tabela_movel is not None and not getattr(df_cotacoes_tabela_movel, "empty", True):
        ...
        if tabela_cotacoes_fmt.empty:
            ...
            renderizar_html_otimizado(
                cached_tabela_html_backlog_canais(
                    serializar_dataframe_cache(tabela_cotacoes_fmt),
                    serializar_dataframe_cache(tabela_cotacoes_num),
                    "tabela-funil-movel-cotacoes-valor-mensal"
                )
            )
...
file_path = str(RUNTIME_BASE_FILE_PATH)
...
render_barra_atualizacao_dashboard()
try:
