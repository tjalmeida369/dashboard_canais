...
def render_bloco_convergencia_tabelas() -> None:
    ...
    df_convergencia_fixa = df_convergencia.loc[
    ...
    meses_convergencia = ordenar_meses_convergencia(
        df_convergencia_fixa['mes_ano'].dropna().unique().tolist()
    )
    ...
    if 'QTDE_CNPJ8' in df_convergencia_fixa.columns:
        canais_base_conv = (
        ...
        canais_base_conv = sorted(
            df_convergencia_fixa['CANAL_PLAN'].dropna().astype(str).str.strip().unique().tolist()
        )
