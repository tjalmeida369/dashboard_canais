...
DU_DATA_PATH = (
    Path(__file__).resolve().parents[1]
    / "dados_preprocessados"
    / "dias_uteis_corporativos.parquet"
)
...
@st.cache_data(show_spinner=False, max_entries=2)
def _load_du_lookup(path: str, modified_at: float | None) -> dict[tuple[int, str, str], float]:
    del modified_at
    if not Path(path).exists():
        return {}
    frame = pd.read_parquet(
        path,
        columns=["ANO_MES", "PRODUTO", "TIPO_DU", "DIAS_UTEIS_EQUIVALENTES"],
    )
    frame["ANO_MES"] = pd.to_numeric(frame["ANO_MES"], errors="coerce")
    frame["DIAS_UTEIS_EQUIVALENTES"] = pd.to_numeric(
        frame["DIAS_UTEIS_EQUIVALENTES"], errors="coerce"
    )
    frame = frame.dropna(subset=["ANO_MES", "DIAS_UTEIS_EQUIVALENTES"])
    return {
        (int(row.ANO_MES), str(row.PRODUTO).upper(), str(row.TIPO_DU).upper()): float(
            row.DIAS_UTEIS_EQUIVALENTES
        )
        for row in frame.itertuples(index=False)
        if float(row.DIAS_UTEIS_EQUIVALENTES) > 0
    }


def _forecast_du(product: str, month_ref: pd.Timestamp) -> float:
    month = pd.Timestamp(month_ref)
    key_month = int(month.year * 100 + month.month)
    du_type = "GERAL" if str(product).upper() == "CONTA" else "INSTALACAO"
    modified_at = DU_DATA_PATH.stat().st_mtime if DU_DATA_PATH.exists() else None
    value = _load_du_lookup(str(DU_DATA_PATH), modified_at).get(
        (key_month, str(product).upper(), du_type)
    )
    if value and value > 0:
        return float(value)
    return float(max(len(pd.bdate_range(month.replace(day=1), month + pd.offsets.MonthEnd(0))), 1))
...
def _build_product_table(frame: pd.DataFrame, product: str) -> str:
    ...
    current_month_ref = pd.Timestamp(
        current.iloc[-1]["MES_REF"] if not current.empty else months.iloc[-1]["MES_REF"]
    )
    previous_month_ref = current_month_ref - pd.offsets.MonthBegin(1)
    ...
    current_du = _forecast_du(product, current_month_ref)
    previous_du = _forecast_du(product, previous_month_ref)
    metrics["MOM_DU"] = (
        (current_values / current_du)
        / (metrics["MES_ANTERIOR_REAL"] / previous_du).replace(0, pd.NA)
        - 1.0
    ) * 100.0
    ...
    comparison_columns = [
        ...
        ("MOM_DU", "MoM<br>D.U.", "forecast-variation", True),
    ...
    colgroup = (
        '<colgroup><col style="width:9.5%;">'
        + ''.join('<col style="width:3.45%;">' for _ in month_labels)
        + ''.join(
            f'<col style="width:{4.8 if is_percentage else 4.1}%;">'
