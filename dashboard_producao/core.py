...
DASHBOARD_LEGACY_MOBILITY_DIR = Path(
    r"C:\Users\F270665\OneDrive - Claro SA\Documentos\Extração_VDI\FÍSICOS_MOBILIDADE"
)
...
def load_cotacoes_data(
    ...
    if path_obj.suffix.lower() == ".parquet" or ".csv" in suffixes:
        ...
        if colunas_agregadas.issubset(set(df_cot_opt.columns)):
            ...
            df_cot_opt["CANAL_PLAN"] = df_cot_opt["CANAL_PLAN"].astype(str).str.strip().map(normalizar_canal_plan)
...
def load_pedidos_dashboard_data(path: str, file_mtime: float | None = None) -> pd.DataFrame:
    ...
    df_ped["CANAL_PLAN"] = df_ped["CANAL_PLAN"].astype(str).str.strip().map(normalizar_canal_plan)
...
def criar_tabela_html_funil_cotacoes(
    ...
    col_mom = "MoM" if "MoM" in colunas else (colunas[-1] if len(colunas) >= 1 else "")
    colunas_meses = [col for col in colunas if col not in {col_etapa, col_mom}]
    ...
    largura_final_pct = 7.5
    largura_meses_pct = max((100.0 - largura_etapa_pct - largura_final_pct) / qtd_meses, 4.0)
    larguras = [largura_etapa_pct] + [largura_meses_pct] * len(colunas_meses) + [largura_final_pct]
...
required_cols_base = {'REGIONAL', 'CANAL_PLAN', 'dat_tratada', 'QTDE'}
if not required_cols_base.issubset(set(getattr(df, "columns", []))):
    fallback_base_path = Path(PRIMARY_BASE_FILE_PATH)
    if fallback_base_path.suffix.lower() == ".parquet" and fallback_base_path.exists():
        try:
            df = pd.read_parquet(fallback_base_path)
        except Exception:
            pass
